...
\begin{tikzpicture}[
  ...
  >=Stealth,
  source/.style={rectangle, rounded corners=4pt, draw=none, fill=blue!15, align=center, minimum width=30mm, minimum height=12mm, inner sep=4pt},
  collector/.style={rectangle, rounded corners=6pt, draw=black!40, line width=1pt, fill=orange!12, align=center, minimum width=50mm, minimum height=22mm, inner sep=6pt},
  processor/.style={rectangle, rounded corners=3pt, draw=none, fill=yellow!20, align=center, minimum width=22mm, minimum height=8mm, inner sep=3pt, font=\footnotesize},
  backend/.style={rectangle, rounded corners=4pt, draw=none, align=center, minimum width=30mm, minimum height=12mm, inner sep=4pt},
  arr/.style={-{Latex}, line width=1pt, draw=black!60}
]

% Application sources (left)
...
% Signal labels next to sources
\node[font=\footnotesize, text=black!60, right=1mm of app1.east, anchor=west] (sig1) {traces, metrics, logs};
\node[font=\footnotesize, text=black!60, right=1mm of app2.east, anchor=west] (sig2) {traces, metrics};
\node[font=\footnotesize, text=black!60, right=1mm of app3.east, anchor=west] (sig3) {traces, logs};

% OTel Collector (center)
\node[collector, right=45mm of app2] (collector) {\textbf{OTel Collector}};

% Processors inside collector (below label)
...
% Backends (right)
\node[backend, fill=green!15, right=40mm of collector, yshift=24mm] (prom) {\textbf{Prometheus}\\Metrics};
\node[backend, fill=purple!15, right=40mm of collector, yshift=8mm] (jaeger) {\textbf{Jaeger}\\Traces};
\node[backend, fill=cyan!15, right=40mm of collector, yshift=-8mm] (elastic) {\textbf{Elasticsearch}\\Logs};
\node[backend, fill=red!12, right=40mm of collector, yshift=-24mm] (grafana) {\textbf{Grafana}\\Dashboards};

% Arrows: sources -> collector
\draw[arr] (sig1.east) -- (collector.west |- app1);
\draw[arr] (sig2.east) -- (collector.west);
\draw[arr] (sig3.east) -- (collector.west |- app3);

% Arrows: collector -> backends
\draw[arr] (collector.east |- prom) -- (prom.west);
\draw[arr] (collector.east |- jaeger) -- (jaeger.west);
\draw[arr] (collector.east |- elastic) -- (elastic.west);
\draw[arr] (collector.east |- grafana) -- (grafana.west);
...
% --- Palette is defined globally in macros.tex (llmblue, llmgray, ...) ---

% --- Central TRACE panel (defined first so other nodes can reference it) ---
\node[boxwide, fill=llmgray!8] (trace) {%
...
\node[box, fill=llmgreen!12, above=22mm of trace.north, xshift=24mm] (retr) {%
...
% --- Legend / Notes panel ---
\node[rectangle, rounded corners=5pt, draw=none, fill=llmgray!6,
...
\node[rectangle, rounded corners=4pt, draw=none, fill=llmgray!10,

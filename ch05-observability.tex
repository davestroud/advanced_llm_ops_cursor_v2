...
\centering
\begin{llmfigbox}
% Three-layer telemetry stack. Color encodes the abstraction level:
%   blue   = infra-level signals (system)
%   green  = control / quality signals (model)
%   orange = decision-relevant lineage (pipeline)
\begin{tikzpicture}[
  layer/.style={minimum width=60mm, minimum height=16mm, font=\small, inner sep=4pt, align=center},
  note/.style={llm/label, text width=40mm}
]

\node[llm/box/blue,   layer]                                  (system)
  {\textbf{System Telemetry}\\[3pt]\footnotesize GPU/CPU util., memory, latency, throughput};
\node[llm/box/green,  layer, below=of system,  yshift=-4mm]   (model)
  {\textbf{Model Telemetry}\\[3pt]\footnotesize Tokens/request, hallucination rate, factuality checks};
\node[llm/box/orange, layer, below=of model,   yshift=-4mm]   (pipeline)
  {\textbf{Pipeline Telemetry}\\[3pt]\footnotesize Prompt traces, retrieval hits, multi-agent orchestration};

\draw[llm/arrow] (system.south) -- (model.north);
\draw[llm/arrow] (model.south)  -- (pipeline.north);
...
Retrieval-Augmented Generation (RAG) introduces unique observability needs, as illustrated in Figure~\ref{fig:ch05_telemetry_layers}. Beyond standard recall@K or mean reciprocal rank (MRR), practitioners track:
\begin{itemize}
...
\end{enumerate}
Frameworks such as RAGAS~\cite{ragas_eacl2024} and ARES~\cite{ares_naacl2024} formalize groundedness (faithfulness), context relevance, and evidence attribution; annotated corpora like RAGTruth~\cite{ragtruth} can support supervised detectors. A best practice is to combine model-graded scores (e.g., faithfulness) with retrieval metrics (e.g., MRR/NDCG) and to track attribution fidelity (evidence–answer span overlap) as a first-class signal. Table~\ref{tab:ch05_rag_metrics_taxonomy} provides a comprehensive taxonomy of RAG-specific metrics with target values and monitoring recommendations.
...
\section{Advanced Instrumentation and Logging for LLM Applications}\index{instrumentation!LLM}
...
\subsection{Standardizing Telemetry: OpenTelemetry and OpenMetrics}\index{OpenTelemetry}\index{OpenMetrics}\label{sec:otel-openmetrics}
...
Fig.~\ref{fig:ch05_otel_collector_hub} illustrates how the OTel Collector acts as a central hub, receiving traces, metrics, and logs from application services and routing them to specialized backends after processing.

\begin{figure}[t]
\centering
\begin{llmfigbox}
% OTel Collector hub: blue = data sources (apps), orange = central control
% (collector with internal processors), green/purple/gray = downstream backends
% specialized by signal type. Dashed processor outline distinguishes them
% as collector-internal stages rather than first-class services.
\begin{tikzpicture}[
  node distance=10mm and 16mm,
  source/.style   ={llm/box/blue,   minimum width=32mm, minimum height=13mm, font=\footnotesize, align=center},
  collector/.style={llm/box/orange, minimum width=52mm, minimum height=24mm, font=\small,        align=center},
  processor/.style={llm/box/gray,   minimum width=22mm, minimum height=8mm,  font=\footnotesize, inner sep=3pt, align=center},
  backend/.style  ={minimum width=32mm, minimum height=13mm, font=\footnotesize, align=center},
  signal/.style   ={llm/label, font=\footnotesize}
]

\node[source] (app1) {LLM Inference\\Service};
\node[source, below=8mm of app1] (app2) {Retrieval\\Service};
\node[source, below=8mm of app2] (app3) {Tool / Agent\\Service};

\node[signal, right=1mm of app1.east, anchor=west] (sig1) {traces, metrics, logs};
\node[signal, right=1mm of app2.east, anchor=west] (sig2) {traces, metrics};
\node[signal, right=1mm of app3.east, anchor=west] (sig3) {traces, logs};

\node[collector, right=45mm of app2] (collector) {\textbf{OTel Collector}};

\node[processor, below=2mm of collector.north] (p1) {Batching};
\node[processor, right=4mm of p1] (p2) {Filtering};
\node[processor, right=4mm of p2] (p3) {Sampling};

\node[llm/box/green,  backend, right=40mm of collector, yshift=24mm]  (prom)    {\textbf{Prometheus}\\Metrics};
\node[llm/box/purple, backend, right=40mm of collector, yshift=8mm]   (jaeger)  {\textbf{Jaeger}\\Traces};
\node[llm/box/blue,   backend, right=40mm of collector, yshift=-8mm]  (elastic) {\textbf{Elasticsearch}\\Logs};
\node[llm/box/orange, backend, right=40mm of collector, yshift=-24mm] (grafana) {\textbf{Grafana}\\Dashboards};

\draw[llm/arrow] (sig1.east) -- (collector.west |- app1);
\draw[llm/arrow] (sig2.east) -- (collector.west);
\draw[llm/arrow] (sig3.east) -- (collector.west |- app3);

\draw[llm/arrow] (collector.east |- prom)    -- (prom.west);
\draw[llm/arrow] (collector.east |- jaeger)  -- (jaeger.west);
\draw[llm/arrow] (collector.east |- elastic) -- (elastic.west);
\draw[llm/arrow] (collector.east |- grafana) -- (grafana.west);
\end{tikzpicture}
\end{llmfigbox}
\caption{The OpenTelemetry Collector serves as a centralized telemetry hub for LLM observability. Application services emit traces, metrics, and logs via OTel SDKs; the Collector processes them (batching, filtering, sampling) and routes each signal type to the appropriate backend. This architecture decouples instrumentation from storage, enabling teams to add or swap backends without modifying application code.}
\label{fig:ch05_otel_collector_hub}
\end{figure}
...
\begin{codebox}{Prometheus scrape and recording rules configuration for LLM-specific observability, covering inference latency percentiles (TTFT), token throughput, groundedness and citation quality aggregations, cost-per-request tracking, and retrieval quality metrics. These recording rules pre-compute common aggregations to enable fast dashboard queries and alerting on LLM-aware SLOs.}{lst:ch05_prometheus_config}
\begin{minted}{yaml}
# prometheus.yml -- scrape three LLM services every 10s.
global:
  scrape_interval: 15s
  evaluation_interval: 15s
  external_labels: { cluster: 'ishtar-production', environment: 'production' }
  ...
  - job_name: 'ishtar-llm-inference'
    ...
    static_configs:
      - targets: ['ishtar-llm-inference:8000']
        labels: { service: 'llm-inference', component: 'inference' }
  - job_name: 'ishtar-retrieval'
    scrape_interval: 10s
    static_configs:
      - targets: ['ishtar-retrieval:8080']
        labels: { service: 'retrieval', component: 'rag' }
  - job_name: 'ishtar-tools'
    scrape_interval: 10s
    static_configs:
      - targets: ['ishtar-tools:9000']
        labels: { service: 'tools', component: 'orchestration' }

rule_files: ['llm_recording_rules.yml']
alerting:
  alertmanagers: [{ static_configs: [{ targets: ['alertmanager:9093'] }] }]
---
# llm_recording_rules.yml -- pre-computed aggregations for fast dashboards
# and alerting. Four groups: latency, quality, cost, retrieval_quality.
groups:
  - name: llm_latency
    interval: 30s
    rules:
      ...
      # ... analogous :p95 and :p99 records ...
      - { record: ishtar:tokens_per_second:avg,
          expr: 'rate(ishtar_tokens_total[5m])
                 / rate(ishtar_requests_total[5m])' }

  - name: llm_quality      # groundedness mean+p95, citation_coverage, toxicity
    interval: 1m
    rules:
      - { record: ishtar:groundedness:mean,
          expr: 'avg(ishtar_groundedness_score) by (service, prompt_version)' }
      - { record: ishtar:toxicity:rate,
          expr: 'sum(rate(ishtar_toxicity_flags_total[5m])) by (service)
                 / sum(rate(ishtar_requests_total[5m])) by (service)' }
      # ... groundedness:p95 and citation_coverage:mean records ...

  - name: llm_cost          # USD/request and USD/1k-tokens per service
    interval: 1m
    rules:
      - { record: ishtar:cost_per_request:avg,
          expr: 'sum(rate(ishtar_cost_usd_total[5m])) by (service)
                 / sum(rate(ishtar_requests_total[5m])) by (service)' }
      - { record: ishtar:cost_per_1k_tokens:avg,
          expr: '1000 * sum(rate(ishtar_cost_usd_total[5m])) by (service)
                      / sum(rate(ishtar_tokens_total[5m])) by (service)' }

  - name: retrieval_quality # latency:p95, recall_at_k, mrr (full set in repo)
    interval: 1m
    rules:
      - { record: ishtar:retrieval_latency:p95,
          expr: 'histogram_quantile(0.95,
                  sum(rate(ishtar_retrieval_latency_ms_bucket[5m]))
                    by (le, index_version))' }
\end{minted}
\end{codebox}
...
\begin{codebox}{JSON Schema definition for standardized LLM telemetry, specifying required fields (trace ID, span ID, prompt template version, token counts, latency breakdown) and optional enrichment (retrieval metadata, cost estimates, safety signals). Schema validation at the collector prevents telemetry drift and ensures consistent observability across pipeline components.}{lst:ch05_telemetry_schema}
\begin{minted}{json}
{
  ...
  "version": "1.2.0",
  "type": "object",
  "required": ["trace_id", "span_id", "conversation_id",
               "prompt_template_id", "prompt_version",
               "token_counts", "latency_breakdown"],
  "properties": {
    ...
    "prompt_telemetry": {
      ...
      "properties": {
        ...
        "version":             { "type": "string",
                                 "pattern": "^\\d+\\.\\d+\\.\\d+$" },
        "system_message_hash": { "type": "string" },
        "user_query_hash":     { "type": "string" },
        "decoding_params": {
          "type": "object",
          "properties": {
    ...
    "latency_breakdown": {
      ...
      "properties": {
        ...
        "tool_io_latency_ms": { "type": "number" }
      }
    },

    "error_type": {
      "type": "string",
      "enum": ["provider_timeout", "rate_limit", "tool_schema_mismatch",
               "parser_failure", "retrieval_timeout", "model_error",
               "unknown"]
    },
    "timestamp": { "type": "string", "format": "date-time" }

    /* Optional sub-objects (full schema in companion repo):
       - retrieval_metadata: index_version, top_k, retrieved_doc_ids[],
         retrieval_scores[], reranker_scores[], retrieval_latency_ms.
       - cost_estimates:    input_cost_usd, output_cost_usd, total_cost_usd.
       - safety_signals:    toxicity_score (0..1), hallucination_flags[],
         jailbreak_detected, policy_violations[]. */
  }
}
\end{minted}
\end{codebox}
...
\begin{codebox}{OpenTelemetry instrumentation for LLM inference, retrieval, and tool calls with proper span creation, attribute recording, and context propagation. Each function creates structured spans with LLM-specific attributes (token counts, latency breakdown, cost estimates, safety signals), enabling end-to-end tracing across multi-hop pipelines and precise attribution of latency and cost to individual pipeline stages.}{lst:ch05_opentelemetry}
\begin{minted}{python}
import hashlib, time
...
# One-time setup: tag every span with service identity, then ship spans
# to an OTel Collector over OTLP/gRPC in batched mode.
...
def instrument_llm_call(model_name, prompt_template_id, prompt_version,
                        user_query, decoding_params,
    ...
    with tracer.start_as_current_span("llm.call", attributes={
        ...
        # Hash the prompt instead of storing it raw (privacy by design).
        span.set_attribute("llm.prompt_hash",
            hashlib.sha256(user_query.encode()).hexdigest()[:16])

        t0 = time.time()
        response = call_llm_api(model_name, user_query, decoding_params)
        total_latency_ms = (time.time() - t0) * 1000

        # Token counts, latency breakdown (TTFT/TPOT/total), cost, and
        # optional safety signals all become span attributes; an error
        # response is recorded via record_exception + ERROR status so the
        # collector can route on it. (Full attribute set in companion repo.)
        for k, v in {
            "llm.input_tokens":     response.get("input_tokens", 0),
            "llm.output_tokens":    response.get("output_tokens", 0),
            "llm.ttft_ms":          response.get("ttft_ms", 0),
            "llm.tpot_ms":          response.get("tpot_ms", 0),
            "llm.total_latency_ms": total_latency_ms,
            "llm.cost_usd":         estimate_cost(response.get("input_tokens", 0),
...
# instrument_retrieval(index_version, query, top_k) and
# instrument_tool_call(tool_name, tool_version, arguments) follow the same
# pattern: open a span ("retriever.search" / "tool.invoke.<name>"), record
# inputs (hashed query / arg size), measure latency, attach result-derived
# attributes (doc_count, recall_at_k / success), and set ERROR status on
# failure. See companion repo for the full implementation.
\end{minted}
\end{codebox}
...
Quality & Phoenix evaluators \cite{arize_phoenix_docs} + LLM-as-a-judge \cite{zheng2023judge} & Did quality regress? (faithfulness, groundedness, toxicity/refusal rates) and which cohorts/routes are affected? \\
...
\section{Tracing Complex Prompt Flows and Multi-Agent Interactions}\index{tracing!prompt flows}
...
\centering
\begin{llmfigbox}
% Span taxonomy. Color carries the role of each span class:
%   gray   = trace envelope (correlation context)
%   blue   = data carrier (LLM call / retrieval)
%   green  = retrieval (data acquisition)
%   orange = control-relevant span (tools, guardrails)
%   purple = parser / external structure
\begin{tikzpicture}[
  node distance=12mm and 18mm,
  every node/.style={align=left},
  header/.style={font=\bfseries\ttfamily\small},
  spanbox/.style ={inner sep=5mm, text width=48mm,           minimum height=16mm, font=\footnotesize},
  spanwide/.style={inner sep=5mm, text width=0.28\linewidth, minimum height=20mm, font=\footnotesize},
]

\node[llm/box/gray, spanwide] (trace) {%
...
\node[llm/box/blue,  spanbox, above=22mm of trace.north, xshift=-24mm] (llm) {%
  \header{\texttt{llm.call}}\\[2mm]
  model/version; temperature, $p$; TTFT; tokens/s; token counts
};
\node[llm/box/green, spanbox, above=22mm of trace.north, xshift=24mm] (retr) {%
  \header{\texttt{retriever.search}}\\[2mm]
  index/version; \texttt{topK}; latency; hit set (doc IDs); re-ranker scores
};

\node[llm/box/orange, spanbox, below=22mm of trace.south, xshift=-48mm] (tool) {%
  \header{\texttt{tool.invoke.<name>}}\\[2mm]
  schema version; payload bytes; retries/backoff; downstream status
};
\node[llm/box/purple, spanbox, below=22mm of trace.south] (parser) {%
  \header{\texttt{parser.jsonschema}}\\[2mm]
  schema ID; failure class: \texttt{missing\_field}, \texttt{type\_mismatch}
};
\node[llm/box/orange, spanbox, below=22mm of trace.south, xshift=48mm] (guard) {%
  \header{\texttt{guard.rail}}\\[2mm]
  policy ID; triggers (jailbreak, toxicity); action (block/sanitize/route)
};

\draw[llm/arrow] (trace.north) .. controls +(0,8mm)  and +(-8mm,0) .. (llm.south);
\draw[llm/arrow] (trace.north) .. controls +(0,8mm)  and +(8mm,0)  .. (retr.south);
\draw[llm/arrow] (trace.south) .. controls +(0,-8mm) and +(-8mm,0) .. (tool.north);
\draw[llm/arrow] (trace.south) .. controls +(0,-8mm) and +(0,0)    .. (parser.north);
\draw[llm/arrow] (trace.south) .. controls +(0,-8mm) and +(8mm,0)  .. (guard.north);

\node[llm/box/gray, below=20mm of tool.south, text width=0.92\linewidth, inner sep=5mm, anchor=north, font=\footnotesize, align=left] (legend) {%
...
\node[llm/box/gray, above=4mm of llm.north, anchor=south, inner sep=5mm, font=\small\bfseries] (title) {%
  Span Taxonomy \& Naming
};
\end{tikzpicture}
\end{llmfigbox}
...
Because decoding is streamed, record TTFT, token cadence (tokens/s series), and finalization time as span attributes or span events. Fine-grained events (e.g., "first citation emitted," "tool decision chosen") make it possible to link subjective user experience to specific inflection points in the waterfall. Figure~\ref{fig:ch05_span_taxonomy} illustrates the recommended span naming conventions that enable systematic observability across LLM pipeline components.
...
Without these observability foundations, it is impractical to debug RAG hallucinations, optimize retrieval strategies, diagnose multi-agent coordination failures, or validate that agent graphs produce correct outputs. The chapters in Part III assume these observability capabilities are in place, building on them to cover advanced RAG architectures (Chapter~\ref{ch:rag}) and sophisticated multi-agent orchestration patterns (Chapter~\ref{ch:multiagent}).
...
\section{Real-Time Dashboards and Live Metrics}\index{dashboards!real-time}
...
A useful organizing principle is a \emph{three-tier} layout, as summarized in Table~\ref{tab:ch05_three_tier_layout}:
...
\subsubsection{Continuous RAGAS Integration}\index{RAGAS!continuous evaluation}
The RAGAS framework~\cite{ragas_eacl2024} provides a suite of reference-free metrics---faithfulness, answer relevancy, and context recall---that can be computed continuously on production traffic without requiring ground-truth labels. In a production deployment, a sampled fraction (typically 1--5\%) of live requests is routed through RAGAS evaluators asynchronously. Results are persisted alongside traces and exported to Prometheus, where recording rules compute rolling aggregates:

\begin{itemize}
    \item \textbf{Faithfulness} measures the fraction of answer claims supported by retrieved context. Track rolling 1-hour and 24-hour means with confidence intervals; alert when the 1-hour mean drops below the 24-hour baseline by more than one standard deviation.
    \item \textbf{Answer relevancy} measures whether the response addresses the user's question. Low relevancy with high faithfulness suggests the retriever is fetching correct but off-topic documents---a retrieval configuration issue rather than a generation problem.
    \item \textbf{Context recall} estimates how much of the required information was present in the retrieved context. Declining recall often precedes faithfulness drops, making it a leading indicator for retrieval degradation.
\end{itemize}

By tracking these three metrics simultaneously, operators can localize quality issues to specific pipeline components: low context recall points to retriever problems; low faithfulness with adequate recall points to generation problems; low relevancy points to query understanding or routing issues.

\subsubsection{Failure Patterns and Alert Thresholds}\index{quality!failure patterns}
Quality metrics exhibit three characteristic failure patterns, each requiring different alert configurations and response strategies:

\begin{enumerate}
    \item \textbf{Sustained drop.} A gradual decline over hours or days, often caused by embedding drift, index staleness, or slow prompt degradation. Detect with a slope test on the 24-hour rolling mean: alert when the regression coefficient exceeds a pre-registered threshold for two consecutive evaluation windows.
    \item \textbf{Sudden cliff.} An abrupt drop (e.g., faithfulness falling from 0.88 to 0.65 within minutes), typically caused by a deployment event, index rebuild failure, or upstream model change. Detect with a change-point detector (e.g., CUSUM or Bayesian online change-point detection) on the 15-minute rolling mean. This pattern demands immediate investigation and usually correlates with a deployment marker in the annotated timeline.
    \item \textbf{Gradual drift.} A slow, persistent decline over weeks that escapes individual alert windows but compounds into material degradation. Detect by comparing weekly aggregates against a locked baseline; schedule automated re-evaluation on golden query sets monthly to catch drift that evades per-window thresholds.
\end{enumerate}

\subsubsection{Concrete Rollback Triggers}
Quality-driven rollback should be governed by explicit, pre-registered thresholds rather than ad hoc judgment. Recommended triggers include:

\begin{itemize}
    \item If faithfulness drops below 0.7 for 15 consecutive minutes, trigger automatic rollback to the previous prompt/model version.
    \item If answer relevancy drops below 0.65 for 30 consecutive minutes, escalate to on-call and initiate controlled traffic shifting to a known-good configuration.
    \item If context recall drops below 0.6 for 1 hour, trigger retrieval index investigation and switch to a fallback retriever (e.g., BM25) while the primary index is diagnosed.
    \item If the unsupported-claim rate exceeds 5\% for two consecutive 15-minute windows, enable constrained decoding mode and reduce retriever fan-out.
\end{itemize}

These thresholds should be calibrated against historical data from golden query replays and adjusted as the system matures. Encode them as Prometheus alerting rules (analogous to Listing~\ref{lst:ch05_alerting_rules}) so that rollback is automated rather than dependent on human detection.
...
\section{Alerts, Incident Response, and Resilience}\index{incident response}\index{alerting}
...
Incident response playbooks should define rollback strategies (e.g., revert prompt versions, fail over to smaller models) and escalation criteria for human review. Public prompt-injection and jailbreak failures in deployed assistants underscore the need for rapid tracing, guardrail patching, and adversarial replay before full restoration \cite{owasp_llm}.

\medskip
\noindent\textbf{SLO-aligned, multi-window alerting.}
Beyond these core principles, effective alerting in LLM systems is \emph{SLO-driven} and \emph{multi-window}. Attach each alert to an explicit SLI with an error budget (e.g., P99 TTFT, unsupported-claim rate, refusal rate), and use burn-rate detectors over short and long windows (e.g., 5\,min and 1\,h) so pages fire quickly for severe regressions while suppressing noise from brief blips. Token/cost anomalies should be normalized by route, tenant, and content type; this prevents high-variance cohorts from drowning out true incidents and surfaces abuse patterns (e.g., prompt amplification) early, as shown in Table~\ref{tab:ch05_alert_types_thresholds}.
...
\begin{codebox}{Prometheus alerting rules for LLM SLOs covering latency spikes (P99 TTFT), throughput degradation, groundedness and hallucination quality regressions, cost anomaly detection, retrieval quality degradation, and safety violations (toxicity and refusal rate anomalies). Each alert includes severity labels, descriptive annotations, and concrete mitigation steps tied to operational runbooks.}{lst:ch05_alerting_rules}
\begin{minted}{yaml}
# alerting_rules.yml -- LLM SLO alerts. One representative rule per group;
# the rest follow the same shape (alert / expr / for / labels / annotations
# with summary, description, runbook_url, mitigation). Full set in repo.
  ...
  - name: llm_latency_alerts
    ...
    rules:
      - alert: LLMLatencySpike
        ...
        for: 5m
        labels: { severity: critical, component: latency }
        annotations:
          summary: "P99 TTFT exceeds 1000ms"
          description: "P99 TTFT is {{ $value }}ms for {{ $labels.service }}.
            Check retrieval latency and inference performance."
          runbook_url: "https://runbooks.ishtar.ai/latency-spike"
      # + LLMThroughputDegradation: tokens/req < 10 for 10m (warning).

  - name: llm_quality_alerts
    interval: 1m
    rules:
      - alert: LLMHallucinationSpike    # critical: hallucinations / requests > 5%
        expr: 'sum(rate(ishtar_hallucination_flags_total[30m])) by (service)
               / sum(rate(ishtar_requests_total[30m])) by (service) > 0.05'
        for: 15m
        labels: { severity: critical, component: safety }
        annotations:
      ...
      # + LLMGroundednessDegradation: avg(groundedness) < 0.80 for 30m.

  - name: llm_cost_alerts
    interval: 5m
    rules:
      - alert: LLMCostAnomaly           # warning: cost ratio vs 24h ago > 2x
        expr: '(sum(rate(ishtar_cost_usd_total[1h])) by (service)
               / sum(rate(ishtar_cost_usd_total[1h] offset 24h)) by (service))
               > 2.0'
        for: 1h
        labels: { severity: warning, component: cost }
        annotations: { summary: "Cost per hour increased 2x",
          mitigation: "throttle long prompts; enable caching; review routing" }

  - name: retrieval_quality_alerts
    interval: 1m
    rules:
      - alert: RetrievalRecallDegradation
        expr: 'avg(ishtar_recall_at_k{k="10"}) by (index_version) < 0.75'
        for: 1h
        labels: { severity: critical, component: retrieval }
        annotations: { summary: "Recall@10 below 0.75 -- rebuild index or
          re-evaluate embedding model" }
      # + RetrievalLatencySpike: p95 retrieval latency > 250ms for 10m.

  - name: safety_alerts                  # toxicity + refusal-rate anomalies
    interval: 1m
    rules:
      - alert: LLMToxicitySpike         # critical: toxicity / requests > 10%
        expr: 'sum(rate(ishtar_toxicity_flags_total[1h])) by (service)
               / sum(rate(ishtar_requests_total[1h])) by (service) > 0.10'
        for: 15m
        labels: { severity: critical, component: safety }
        annotations:
          summary: "Toxicity rate exceeds 10%"
          mitigation: "enable strict filters; rollback prompt/model;
                       disable risky tools; route to safe-mode"
      # + LLMRefusalRateAnomaly: refusals / requests > 15% for 30m (warning).
\end{minted}
\end{codebox}
...
\subsection{End-to-End Incident Narrative: Hallucination Spike}
\label{subsec:ch05-incident-narrative}

\IshtarVignette{
\textbf{Incident: Hallucination spike following knowledge base update.}

\medskip
\noindent\textbf{T+0 min --- Detection.}
A Grafana alert fires: the rolling 15-minute hallucination rate for the \texttt{/analysis} route has exceeded the 5\% SLO threshold, reaching 8.2\%. The alert links to a Prometheus recording rule (\texttt{ishtar:hallucination:rate}) and includes an OpenTelemetry exemplar trace ID. The on-call engineer opens the Grafana dashboard and confirms the spike is localized to the \texttt{conflict\_reports} slice---other slices remain within normal bounds.

\medskip
\noindent\textbf{T+3 min --- Trace-based diagnosis.}
The engineer clicks the exemplar link, which opens the problematic trace in Jaeger. The waterfall shows normal latency for the \texttt{llm.call} span but an anomalous pattern in \texttt{retriever.search}: the retrieval latency has increased from a baseline P50 of 45\,ms to 180\,ms, and the \texttt{retrieval.recall\_at\_k} attribute has dropped from 0.89 to 0.52. The \texttt{retrieval.index\_version} attribute shows \texttt{v2024-08-15}, whereas the deployment marker indicates a knowledge base update was pushed 22 minutes earlier. The trace also reveals that the RAGAS faithfulness score attached to the synthesis span is 0.41---far below the 0.85 target.

\medskip
\noindent\textbf{T+7 min --- Root cause identification.}
Cross-referencing the index version with the deployment log reveals that the knowledge base update included a re-chunking of conflict-zone documents with a new chunk size (512 tokens instead of the previous 256). The new chunks are too large for the embedding model's effective context window, causing embedding quality to degrade for this document subset. The embeddings are technically valid but semantically drifted: cosine similarity between the new chunk embeddings and the pinned reference set has dropped from 0.91 to 0.72---tripping the embedding drift threshold that was set but not yet wired to an automatic rollback.

\medskip
\noindent\textbf{T+10 min --- Stabilization.}
The engineer executes the pre-approved playbook: (1) rolls back the retrieval index to \texttt{v2024-08-14} using the index snapshot stored in the artifact registry; (2) confirms via golden query replay that faithfulness has recovered to 0.87 on the conflict\_reports slice; (3) verifies that the hallucination rate on the Grafana dashboard has returned below 3\% within two evaluation windows.

\medskip
\noindent\textbf{T+30 min --- Eradication and prevention.}
The team re-chunks the affected documents with the original 256-token chunk size, regenerates embeddings, and validates them against the reference set before re-deploying. Post-incident, three improvements are implemented:
\begin{itemize}[leftmargin=1.5em, itemsep=2pt]
    \item The embedding drift alert is wired to an automatic index rollback trigger (previously it was advisory-only).
    \item The CI pipeline for knowledge base updates now includes a mandatory embedding drift check: new embeddings must maintain cosine similarity $\geq$ 0.85 with the pinned reference set before promotion.
    \item The failing conflict-zone queries from this incident are captured and added to the regression test suite, ensuring that future index updates are tested against these specific cases.
\end{itemize}

\medskip
\noindent\textbf{Observability tools used:} Grafana (alert and dashboard), Prometheus (hallucination rate metric and recording rules), Jaeger (trace waterfall and span attributes), RAGAS (faithfulness scoring on synthesis span), OTel Collector (telemetry routing and exemplar binding), golden query replay (recovery validation).
}
...
Table~\ref{tab:ch05_troubleshooting_runbook} provides a quick-reference runbook mapping common symptoms to the metrics that diagnose them, their likely root causes, and recommended first-response actions.

\begin{table}[t]
\centering
\small
\caption{Troubleshooting runbook maps observable symptoms to diagnostic metrics, likely causes, and first-response actions. This table serves as a quick reference during incidents, enabling operators to move from symptom to action without lengthy investigation.}
\label{tab:ch05_troubleshooting_runbook}
\setlength{\tabcolsep}{5pt}
\renewcommand{\arraystretch}{1.3}
\rowcolors{2}{gray!8}{white}
\begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{2.4cm}>{\raggedright\arraybackslash}p{2.4cm}>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X@{}}
\toprule
\rowcolor{gray!15}
\textbf{Symptom} & \textbf{Check Metric} & \textbf{Likely Cause} & \textbf{Action} \\
\midrule
High latency & TTFT P99; queue depth; GPU utilization & Overloaded GPU; batch size too large; KV-cache pressure & Scale up replicas; reduce batch size; enable KV-cache eviction \\
\addlinespace[2pt]
Low quality scores & Faithfulness score; context recall & Embedding drift; stale index; prompt regression & Re-index knowledge base; check embedding model version; rollback prompt \\
\addlinespace[2pt]
Cost spike & Cost per request; tokens per request & Verbose outputs; retrieval over-fetching; model tier misconfiguration & Cap max tokens; reduce \texttt{topK}; verify routing rules \\
\addlinespace[2pt]
High hallucination rate & Unsupported-claim rate; citation coverage & Missing evidence in retrieval; context window overflow & Increase retriever fan-out; enable passage deduplication; reduce context cap \\
\addlinespace[2pt]
Elevated refusal rate & Refusal rate per route; guardrail trigger count & Overly strict guardrails; prompt template regression & Review guardrail thresholds; rollback prompt version; check safety policy \\
\addlinespace[2pt]
Retrieval timeout & Retriever P95 latency; index health & Index corruption; shard imbalance; network partition & Rebuild index; rebalance shards; failover to BM25 fallback \\
\addlinespace[2pt]
Token amplification & Tokens per turn; tool call count & Verbosity loop; agent oscillation; tool storms & Cap max steps per request; add early-exit heuristics; reduce tool budget \\
\bottomrule
\end{tabularx}
\end{table}
...
\section{Cost Attribution and Unit Economics}\index{cost attribution}\index{unit economics}
\label{sec:ch05-cost-attribution}

Observability is incomplete without cost visibility. In LLM systems, inference costs scale with token consumption, and a single feature or user cohort can dominate spend without any visible signal in latency or error-rate dashboards. Cost attribution ties observability metrics to billing, enabling teams to answer not just ``is the system healthy?'' but ``is the system \emph{economical}?''

\subsection{Token Counting and Cost Calculation}
\label{subsec:ch05-token-cost-calc}

Every request should carry a cost estimate computed from token counts and the per-token pricing of the model used. For self-hosted models, cost is derived from GPU-seconds consumed; for API-based models, it maps directly to the provider's pricing schedule. The telemetry schema (Table~\ref{tab:ch05_telemetry_schema}) already includes \texttt{token\_counts} and \texttt{cost\_estimates} fields---the key is to populate them consistently and use them as first-class dimensions in dashboards and alerts.

Cost per request is calculated as:
\[
\text{cost}_{\text{request}} = \text{tokens}_{\text{input}} \times p_{\text{input}} + \text{tokens}_{\text{output}} \times p_{\text{output}},
\]
where $p_{\text{input}}$ and $p_{\text{output}}$ are the per-token prices for the model tier used. For self-hosted deployments, substitute GPU-second pricing: $\text{cost}_{\text{request}} = t_{\text{GPU}} \times p_{\text{GPU/s}}$, where $t_{\text{GPU}}$ is the wall-clock GPU time consumed by the request.

\subsection{Attribution Dimensions}
\label{subsec:ch05-attribution-dimensions}

Cost attribution becomes actionable when it is broken down by multiple dimensions simultaneously. Table~\ref{tab:ch05_cost_attribution} shows a representative cost breakdown for an \ishtar{}-style deployment.

\begin{table}[t]
\centering
\small
\caption{Cost attribution by dimension reveals spending patterns invisible in aggregate metrics. Breaking costs down by model tier, feature, and user tier enables targeted optimization---for example, discovering that a summarization feature consumes 40\% of spend while serving only 15\% of requests.}
\label{tab:ch05_cost_attribution}
\setlength{\tabcolsep}{6pt}
\renewcommand{\arraystretch}{1.3}
\rowcolors{2}{gray!8}{white}
\begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{2.8cm}>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}p{2cm}>{\raggedright\arraybackslash}p{2.2cm}@{}}
\toprule
\rowcolor{gray!15}
\textbf{Dimension} & \textbf{Example Segments} & \textbf{Cost Share} & \textbf{Optimization Lever} \\
\midrule
\textbf{By model tier} & GPT-4 class (70B); GPT-3.5 class (13B); Distilled (8B) & 60\% / 25\% / 15\% & Route simple queries to cheaper tiers \\
\addlinespace[2pt]
\textbf{By feature} & Search; Chat; Summarization; Fact-check & 20\% / 35\% / 30\% / 15\% & Cache high-repeat summarization; batch fact-check \\
\addlinespace[2pt]
\textbf{By user tier} & Free; Pro; Enterprise & 10\% / 45\% / 45\% & Enforce per-tier token budgets; throttle free tier \\
\addlinespace[2pt]
\textbf{By request type} & Short QA ($<$200 tokens); Long analysis ($>$2k tokens) & 15\% / 85\% & Context trimming; prompt compression for long requests \\
\bottomrule
\end{tabularx}
\end{table}

\subsection{Integration with OpenTelemetry Metrics}
\label{subsec:ch05-cost-otel-integration}

To enable cost attribution, augment existing OTel spans and metrics with custom attributes that carry cost-relevant metadata. Add the following attributes to every inference span:

\begin{itemize}
    \item \texttt{llm.model\_tier}: the pricing tier of the model used (e.g., \texttt{premium}, \texttt{standard}, \texttt{lightweight}).
    \item \texttt{llm.feature}: the application feature that originated the request (e.g., \texttt{search}, \texttt{chat}, \texttt{summarize}).
    \item \texttt{llm.user\_tier}: the billing tier of the requesting user or tenant.
    \item \texttt{llm.cost\_usd}: the computed cost estimate for this request.
\end{itemize}

These attributes flow through the OTel Collector to Prometheus, where recording rules (analogous to those in Listing~\ref{lst:ch05_prometheus_config}) aggregate cost by dimension. Grafana dashboards can then display cost breakdowns per feature, per tenant, and per model tier, with burn-rate alerts when any dimension exceeds its budget.

\BestPracticeBox{
\textbf{Cost-aware observability principles:}
\begin{itemize}[leftmargin=1.5em, itemsep=3pt]
    \item Attach cost estimates to every inference span so cost is queryable alongside latency and quality.
    \item Break cost down by model tier, feature, and user tier to identify optimization opportunities.
    \item Set per-dimension cost budgets with burn-rate alerts; treat cost overruns as incidents requiring the same response discipline as latency or safety regressions.
    \item Monitor cost per unit of value (e.g., cost per successful fact-check, cost per satisfied user) rather than cost per request alone---this aligns engineering optimization with business outcomes.
    \item Review cost attribution dashboards weekly; flag features where cost grows faster than usage as candidates for caching, model routing, or prompt compression.
\end{itemize}
}

...
\end{tikzpicture}
\end{llmfigbox}
\caption{Continuous bias audits turn fairness from a launch checkbox into a steady-state operational metric. The cycle collects evaluation data, measures disparate outcomes, reports against thresholds, triggers remediation when thresholds are exceeded, and feeds monitoring back into the next model update.}
...
% Labels on arrows
\node[font=\footnotesize, above=0.1cm] at ($(ingest)!0.5!(detect)$) {NER scan};
\node[font=\footnotesize, above=0.1cm] at ($(detect)!0.5!(anon)$) {classify};
\node[font=\footnotesize, above=0.1cm] at ($(anon)!0.5!(process)$) {sanitized};
\node[font=\footnotesize, right=0.1cm] at ($(process)!0.5!(retain)$) {TTL policy};
\node[font=\footnotesize, below=0.1cm] at ($(retain)!0.5!(delete)$) {crypto-erase};
\end{tikzpicture}
\end{llmfigbox}
\caption{Treating privacy as a lifecycle (collect $\to$ minimize $\to$ retain $\to$ erase) makes GDPR/CCPA obligations enforceable in code rather than policy. Data flows from ingestion through PII detection and anonymization, a retention management layer enforces TTL policies, and crypto-erasure ensures records are irrecoverable once the retention window closes.}
...
\draw[arrow] (safe) -- node[right, font=\footnotesize] {Yes} (deliver);
\draw[arrow] (safe) -- node[above, font=\footnotesize] {No} (severity);
\draw[arrow] (severity) -- node[above left, font=\footnotesize] {Critical} (p1);
\draw[arrow] (severity) -- node[right, font=\footnotesize] {Moderate} (p2);
\draw[arrow] (severity) -- node[above right, font=\footnotesize] {Low} (p34);

% Escalation chain annotation
\node[font=\footnotesize\itshape, text=gray!60!black, below=0.3cm of p1, text width=2.5cm, align=center] {Ethics officer $\rightarrow$ Legal $\rightarrow$ Executive};
\node[font=\footnotesize\itshape, text=gray!60!black, below=0.3cm of p2, text width=2.5cm, align=center] {Content moderator $\rightarrow$ Ethics officer};
\node[font=\footnotesize\itshape, text=gray!60!black, below=0.3cm of p34, text width=2.5cm, align=center] {Batch review $\rightarrow$ Weekly report};
\end{tikzpicture}
\end{llmfigbox}
\caption{Classifying safety incidents at ingress keeps the vast majority of cases on automated paths and routes only the residual to human review. Each severity tier carries a defined response SLA, required action, and escalation chain; critical (P1) incidents trigger immediate human takeover plus notification of the ethics officer, legal team, and executive leadership.}

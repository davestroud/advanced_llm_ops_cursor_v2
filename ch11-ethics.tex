...
\begin{algorithm}[t]
\caption{Bias audit pipeline systematically measures fairness across demographic dimensions. For each dimension (gender, race, age), the algorithm generates test prompts, runs inference for all groups, computes sentiment or outcome scores, and flags disparities exceeding a threshold. This structured approach enables continuous bias monitoring and supports fairness validation before deployment.}\label{alg:ch11_bias_audit}
\begin{algorithmic}[1]

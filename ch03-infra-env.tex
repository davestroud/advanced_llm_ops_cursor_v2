...
LLMs are increasingly able to generate deployable IaC templates directly from natural language descriptions, thereby reducing the steep learning curve traditionally associated with infrastructure automation~\cite{srivatsa2024-iac-llm}.
...
This linear dependence means that long-context or high-concurrency serving can easily become memory-bound. For example, a single 13B-parameter model (like LLaMA-13B with 40 layers and 40 heads of dimension 128) will require on the order of 1–2 GB of GPU memory per 1024 tokens of context \textit{per request} in FP16 precision.\footnote{Applying the same scaling, a batch of eight concurrent requests at 1024-token contexts therefore requires roughly 10--15\,GB just for KV caches.} Caching and memory management strategies (see \S\ref{sec:infra-hardware-bench}) are therefore central to efficient operations.

\IshtarVignette{%
\textbf{Worked Example: From SLA to Hardware Selection for \ishtar{}.}\index{SLA!hardware mapping}\index{capacity planning!worked example}

Suppose the \ishtar{} deployment must meet the following operational contract: \emph{P99 TTFT $< 500$\,ms} and \emph{sustain 100 concurrent requests} for a 13B-parameter model with average context length $T = 2048$ tokens and average output length $N_{\text{out}} = 256$ tokens in FP16.

\medskip\noindent\textbf{Step 1: Memory budget.} Model weights require $M_{\text{params}} = 13 \times 10^9 \times 2 \approx 26$\,GB. The KV-cache for batch size $B$ at sequence length $T = 2048$ with $L = 40$ layers, $H = 40$ heads, $d_h = 128$, and $\alpha = 2$ bytes (FP16) is $M_{\text{KV}} \approx 2 \times 40 \times B \times 40 \times 128 \times 2048 \times 2 \approx 1.68$\,GB per request. For $B = 100$ concurrent requests, KV-cache alone requires $\sim$168\,GB.

\medskip\noindent\textbf{Step 2: Hardware feasibility.}
\begin{itemize}[leftmargin=1.5em, itemsep=2pt]
  \item \textbf{3$\times$ A100-80GB} (240\,GB total): 26\,GB weights + 168\,GB KV-cache $=$ 194\,GB $<$ 240\,GB. Feasible, with $\sim$46\,GB headroom for activations and overhead. Each A100 delivers $\sim$2\,TB/s memory bandwidth; at this batch size, estimated TTFT $\approx 350$--$450$\,ms (within SLA).
  \item \textbf{2$\times$ H100-80GB} (160\,GB total): 26 + 168 $=$ 194\,GB $>$ 160\,GB. \emph{Does not fit.} Would require INT8 quantization ($\alpha = 1$), halving KV-cache to $\sim$84\,GB and weights to $\sim$13\,GB for a total of $\sim$97\,GB. With H100's 3.35\,TB/s bandwidth, estimated TTFT $\approx 200$--$300$\,ms (well within SLA).
  \item \textbf{8$\times$ L4-24GB} (192\,GB total): Marginal fit, but L4 memory bandwidth ($\sim$300\,GB/s each) is insufficient for the prefill-phase compute at this batch size. Estimated TTFT $\approx 1200$--$1500$\,ms, far exceeding the 500\,ms SLA.
\end{itemize}

\medskip\noindent\textbf{Conclusion.} The L4 configuration fails the latency SLA despite fitting in aggregate memory. The A100 configuration works in FP16 but leaves limited headroom. The H100 configuration with INT8 quantization offers the best latency margin and headroom for traffic spikes. This analysis demonstrates that memory capacity, memory bandwidth, and compute throughput must all be evaluated jointly against the SLA---no single metric suffices.
}

\subsection{GPU Architectures and Choices}\index{GPU!architecture}
\label{sec:infra-hardware-gpu}

NVIDIA GPUs remain the dominant hardware for LLM workloads in 2024–2025. The NVIDIA L4 offers excellent per-watt efficiency for smaller models, embeddings, and lightweight inference; the A100 was the mainstream workhorse for both training and inference in the early 2020s; and the H100 (Hopper architecture) introduces new low-precision math (FP8), significantly higher memory bandwidth, and other architectural improvements targeting Transformer models. Table~\ref{tab:ch03_gpucompare} compares key specifications and typical performance characteristics of these GPU accelerators. 
...
\begin{table}[t]
  ...
  \begin{tabularx}{\linewidth}{@{}X S[table-format=2.0] S[table-format=5.0] S[table-format=1.2] >{\raggedright\arraybackslash}p{38mm}@{}}
    \toprule
    \rowcolor{gray!15}
    \textbf{Processor} & {\textbf{Memory (GB)}} & {\textbf{Tokens/s (13B)}} & {\textbf{Price/hr (\$)}} & \textbf{Best Use Case} \\
    \midrule
    \rowcolor{blue!8}
    NVIDIA L4     & 24 & 2000 & 0.35 & Low-power, low-latency inference; embeddings \\
    \addlinespace[2pt]
    \rowcolor{orange!10}
    NVIDIA A100   & 40 & 6500 & 2.90 & High-load inference; some training/fine-tuning \\
    \rowcolor{orange!10}
    NVIDIA A100   & 80 & 8000 & 3.80 & Larger models; multi-model serving \\
    \addlinespace[2pt]
    \rowcolor{green!10}
    NVIDIA H100   & 80 & \textcolor{green!70!black}{\textbf{12000}} & 5.00 & Cutting-edge throughput; FP8 acceleration \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\begin{codebox}{Python script for calculating cost per token based on GPU hardware configuration and utilization levels, with multi-GPU comparison support. This utility enables infrastructure teams to model the cost implications of different GPU choices and utilization targets before committing to provisioning decisions, making the cost-throughput trade-off explicit and data-driven.}{lst:ch03_cost_calculation}
\begin{minted}{python}
...
def calculate_cost_per_token(gpu: GPUConfig,
                             utilization_percent: float = 100.0) -> CostResult:
    if not 0 <= utilization_percent <= 100:
        raise ValueError("Utilization must be between 0 and 100")
    # Effective throughput scales linearly with utilization;
    # cost-per-token = (USD/sec) / (tokens/sec).
    ...
    return CostResult(
        cost_per_1k_tokens_usd=cost_per_token * 1000.0,
        cost_per_second_usd=cost_per_second,
        effective_throughput_tokens_per_sec=effective_throughput,
        utilization_percent=utilization_percent,
    )

def compare_gpu_options(gpus, utilization_percent: float = 100.0):
    return {g.name: calculate_cost_per_token(g, utilization_percent)
            for g in gpus}

# ... (driver block: defines A100-80GB ($3.80/h, 8k tok/s) and
#      H100-80GB ($5.00/h, 12k tok/s); prints cost/1k tokens at
#      100% and 50% utilization. Full source in companion repo.) ...
\end{minted}
\end{codebox}
...
\begin{figure}[t]
  \centering
  \begin{llmfigbox}
  % Throughput-vs-batch-size with semantic shading: green = rapid gain,
  % orange = diminishing returns, gray = saturation. Uses palette colors
  % from macros.tex so phases stay grayscale-distinct.
  \begin{tikzpicture}
    \begin{axis}[llm/chart,
      height=10cm,
      xlabel={Batch Size $B$},
      ...
      \addplot[fill=llmgreen!15,  draw=none, domain=1:8,    area legend] {throughput(x)} \closedcycle;
      \addplot[fill=llmorange!15, draw=none, domain=8:40,   area legend] {throughput(x)} \closedcycle;
      \addplot[fill=llmgray!15,   draw=none, domain=40:140, area legend] {throughput(x)} \closedcycle;

      \addplot[color=llmblue, line width=1.4pt, smooth, domain=1:140, samples=150] {throughput(x)};

      \addplot[dashed, color=black!40, line width=0.6pt] coordinates {(32,0)  (32,8500)};
      \addplot[dashed, color=black!40, line width=0.6pt] coordinates {(64,0)  (64,10500)};
      \addplot[dashed, color=black!40, line width=0.6pt] coordinates {(128,0) (128,11000)};

      \node[circle, fill=llmgreen,  draw=white, line width=1.5pt, inner sep=3pt] at (axis cs:4,3500)  {};
      \node[font=\footnotesize\bfseries, text=llmgreen,  above=5pt, right=8pt] at (axis cs:4,3500)  {3$\times$ gain};

      \node[circle, fill=llmorange, draw=white, line width=1.5pt, inner sep=3pt] at (axis cs:32,8500) {};
      \node[font=\footnotesize\bfseries, text=llmorange, above=5pt] at (axis cs:32,8500) {Diminishing};

      \node[circle, fill=llmgray,   draw=white, line width=1.5pt, inner sep=3pt] at (axis cs:64,10500){};
      \node[font=\footnotesize\bfseries, text=llmgray,   above=5pt] at (axis cs:64,10500){Saturation};

      \node[font=\footnotesize\bfseries, text=llmgreen,  align=center, right=10pt] at (axis cs:4,2000)   {Rapid\\gain};
      \node[font=\footnotesize\bfseries, text=llmorange, align=center]             at (axis cs:24,6000)  {Diminishing\\returns};
      \node[font=\footnotesize\bfseries, text=llmgray,   align=center]             at (axis cs:100,11000){Saturation\\region};

      \node[font=\footnotesize, text=black!60, below=4pt] at (axis cs:32,0)  {32};
      \node[font=\footnotesize, text=black!60, below=4pt] at (axis cs:64,0)  {64};
      \node[font=\footnotesize, text=black!60, below=4pt] at (axis cs:128,0) {128};
    \end{axis}
  \end{tikzpicture}
  \end{llmfigbox}
...
Beyond raw compute, two techniques significantly impact performance and cost in LLM serving: caching and quantization. As illustrated in Figure~\ref{fig:ch03_batchtradeoff}, throughput gains from increasing batch size eventually saturate as memory or kernel overhead becomes the bottleneck.
...
\begin{table}[t]
  ...
  \begin{tabularx}{\linewidth}{@{}X S[table-format=1.2] S[table-format=5.0] S[table-format=1.3]@{}}
    \toprule
    \rowcolor{gray!15}
    \textbf{Accelerator} & {\textbf{Hourly Price (\$)}} & {\textbf{Throughput (tok/s)}} & {\textbf{Cost/1M tok (\$)}} \\
    \midrule
    \rowcolor{orange!10}
    NVIDIA A100-80GB & 3.80 & 8000 & {\textcolor{orange!70!black}{\textbf{$\sim$0.13}}} \\
    \rowcolor{blue!8}
    NVIDIA H100-80GB & 5.00 & 12000 & {$\sim$0.09} \\
    \addlinespace[2pt]
    \rowcolor{blue!8}
    Google TPU v4     & 3.00 & 9000  & {$\sim$0.093} \\
    \rowcolor{green!10}
    Google TPU v5e-8  & {8.00\tnote{a}} & \textcolor{green!70!black}{\textbf{48000}} & {\textcolor{green!70!black}{\textbf{$\sim$0.046}}} \\
    \bottomrule
  \end{tabularx}
  \begin{tablenotes}
  \footnotesize
...
\subsection{End-to-End Deployment Cost Scenario}\index{cost!deployment scenario}\index{cost!end-to-end comparison}
\label{sec:infra-cost-scenario}

To make the cost analysis fully concrete, we now present an end-to-end deployment scenario for \ishtar{}'s 70B-parameter summarization model\index{cost!70B model deployment}. The operational requirement is to sustain \textbf{100 requests per second} with a \textbf{P99 TTFT target of 500\,ms}, using FP16 precision where possible and INT8 quantization where memory constraints require it. Table~\ref{tab:ch03_e2e_cost} compares three hardware configurations under these constraints.

\begin{table}[t]
\centering
\small
\caption{End-to-end deployment cost comparison for \ishtar{}'s 70B model at 100 req/s with P99 TTFT $<$ 500\,ms. The H100 configuration offers the best balance of cost and latency; the A100 configuration provides a lower-risk FP16 path at higher cost; the L4 configuration cannot meet the latency SLA at this scale. Costs assume on-demand cloud pricing (early 2025) and sustained utilization at 80\%.}
\label{tab:ch03_e2e_cost}
\setlength{\tabcolsep}{5pt}
\renewcommand{\arraystretch}{1.3}
\rowcolors{2}{gray!8}{white}
\begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{28mm} S[table-format=4.0] S[table-format=5.0] S[table-format=4.0] S[table-format=1.2]@{}}
\toprule
\rowcolor{gray!15}
\textbf{Configuration} & {\textbf{Monthly Cost (\$)}} & {\textbf{Throughput (tok/s)}} & {\textbf{TTFT P99 (ms)}} & {\textbf{Cost/1M tok (\$)}} \\
\midrule
\rowcolor{orange!10}
3$\times$ A100-80GB (FP16, tensor parallel) &
8208 &
{$\sim$24\,000} &
{$\sim$420} &
0.13 \\
\addlinespace[2pt]
\rowcolor{green!10}
2$\times$ H100-80GB (INT8, tensor parallel) &
\textcolor{green!70!black}{\textbf{7200}} &
{$\sim$30\,000} &
{\textcolor{green!70!black}{\textbf{$\sim$280}}} &
\textcolor{green!70!black}{\textbf{0.09}} \\
\addlinespace[2pt]
\rowcolor{blue!8}
8$\times$ L4-24GB (INT8, pipeline parallel) &
\textcolor{blue!70!black}{\textbf{2016}} &
{$\sim$10\,000} &
{\textcolor{red!70!black}{\textbf{$\sim$1\,400}}} &
0.08 \\
\bottomrule
\end{tabularx}
\vspace{3pt}
\footnotesize
\textit{Note:} Monthly costs assume 720 hours at on-demand rates (A100: \$3.80/hr, H100: \$5.00/hr, L4: \$0.35/hr) with 80\% average utilization. Throughput and TTFT estimates are based on published benchmarks for 70B models with continuous batching enabled.
\end{table}

Several observations emerge from this comparison:

\begin{itemize}
  \item \textbf{Cost-optimized profile (L4):}\index{cost!optimization} The L4 configuration achieves the lowest cost per million tokens (\$0.08), but its P99 TTFT of $\sim$1{,}400\,ms far exceeds the 500\,ms target. This configuration is suitable only for batch or asynchronous workloads where latency is not a binding constraint---for example, overnight bulk summarization jobs.

  \item \textbf{Latency-optimized profile (H100):}\index{latency!optimization} The H100 configuration meets the TTFT target with substantial margin (280\,ms vs.\ 500\,ms) while also delivering the highest throughput. Despite requiring INT8 quantization, this configuration provides headroom for traffic spikes and leaves capacity for future context-length increases. It represents the best choice when latency SLAs are firm and the quality impact of INT8 quantization is acceptable.

  \item \textbf{Balanced profile (A100):}\index{cost!balanced profile} The A100 configuration runs in native FP16 (avoiding quantization quality risk) and meets the TTFT target, but at the highest monthly cost. This is the conservative choice when quantization has not been validated against the application's evaluation suite, or when the team prefers to avoid the operational complexity of INT8 calibration.
\end{itemize}

\IshtarVignette{%
For the \ishtar{} production deployment, the team selected the \textbf{2$\times$ H100 configuration} after validating that INT8 quantization produced less than 1\% degradation on their conflict-reporting evaluation suite. The 280\,ms TTFT provides sufficient headroom for the 500\,ms SLA, and the \$7{,}200/month cost fits within the project's infrastructure budget. The L4 configuration was retained for the nightly batch re-indexing pipeline, where latency is irrelevant and cost per token is the primary concern. This mixed-hardware strategy---latency-sensitive workloads on H100, throughput-sensitive batch workloads on L4---exemplifies the ``right accelerator for the right job'' principle discussed throughout this chapter.
}
...
\begin{codebox}{Reusable Terraform module for provisioning GPU-enabled EKS node groups with configurable instance types, autoscaling limits, Kubernetes taints and labels for workload isolation, and least-privilege IAM roles. This module encapsulates the GPU infrastructure pattern so that teams can stamp out consistent node groups across environments without duplicating configuration or risking security policy drift.}{lst:ch03_terraform_module}
\begin{minted}{terraform}
# modules/gpu-node-group/main.tf -- reusable GPU EKS node group.
...
# Taint defaults isolate GPU workloads; labels target llm-inference pods.
variable "taints" {
  type    = list(object({ key = string, value = string, effect = string }))
  default = [{ key = "gpu", value = "true", effect = "NO_SCHEDULE" }]
}
variable "labels" {
  type    = map(string)
  default = { nodeType = "gpu", workload = "llm-inference" }
}
...
resource "aws_eks_node_group" "gpu_nodes" {
  ...
  ami_type        = "AL2_x86_64_GPU"   # ships NVIDIA drivers
  disk_size       = 100
  ...
  remote_access   { ec2_ssh_key = var.ssh_key_name }

  dynamic "taint" {
    for_each = var.taints
    content { key = taint.value.key; value = taint.value.value
...
# Least-privilege node role: assumed by EC2; attaches the three EKS-managed
# policies required for worker registration, CNI, and ECR pulls.
...
output "node_group_arn" { value = aws_eks_node_group.gpu_nodes.arn }
output "node_group_id"  { value = aws_eks_node_group.gpu_nodes.id  }
\end{minted}
\end{codebox}
...
\begin{codebox}{GitHub Actions workflow implementing a full Infrastructure-as-Code CI/CD pipeline with Terraform format checking, validation, TFLint linting, Checkov security scanning, and multi-environment promotion from dev to production with plan-artifact handoff. This pipeline ensures that every infrastructure change is validated, security-scanned, and promoted through environments with human approval gates, preventing misconfigurations from reaching production GPU clusters.}{lst:ch03_iac_pipeline}
\begin{minted}{yaml}
...
jobs:
  # 1) Validate: fmt, init -backend=false, validate, TFLint.
  validate:
  ...
  # 2) Security: Checkov scan against the Terraform sources.
  security-scan:
    runs-on: ubuntu-latest
    needs: validate
    steps:
  ...
  # 3) Dev plan on PR; uploads tfplan artifact for the apply step.
  plan-dev:
    runs-on: ubuntu-latest
    ...
    steps:
      ...
      - working-directory: ./infrastructure
        run: |
          terraform init -backend-config="bucket=ishtar-terraform-state" \
                         -backend-config="key=dev/terraform.tfstate"
          terraform plan -var-file="environments/dev.tfvars" \
                         -out=tfplan -no-color
      - uses: actions/upload-artifact@v4
        with: { name: terraform-plan-dev, path: infrastructure/tfplan }

  # 4) Apply Dev on push to main, gated by the `dev` GitHub environment.
  apply-dev:
    runs-on: ubuntu-latest
    needs: plan-dev
    ...
    steps:
      ...
      - uses: actions/download-artifact@v4
        with: { name: terraform-plan-dev }
      # ... (configure-aws-credentials + setup-terraform identical to plan-dev) ...
      - working-directory: ./infrastructure
        run: |
          terraform init -backend-config="bucket=ishtar-terraform-state" \
                         -backend-config="key=dev/terraform.tfstate"
  ...
  # 5/6) plan-prod and apply-prod mirror plan-dev/apply-dev with prod-scoped
  #      AWS_*_PROD secrets, the `production` environment (manual approval
  #      gate), and the bucket=ishtar-terraform-state-prod / key=prod/...
  #      backend. Full workflow in companion repo.
\end{minted}
\end{codebox}
...
\begin{codebox}{Kubernetes Deployment and Service manifests for an LLM inference service with GPU resource requests, node-selector and taint-based scheduling for GPU isolation, pod anti-affinity for high availability across hosts, extended readiness probes that account for model loading time, and a non-root security context. This production-grade manifest demonstrates the operational patterns needed to reliably serve large language models on shared GPU clusters.}{lst:ch03_k8s_deployment}
\begin{minted}{yaml}
apiVersion: apps/v1
kind: Deployment
metadata:
  ...
  labels: { app: ishtar-llm, component: inference, version: v1.3.0 }
spec:
  replicas: 3
  selector:
    matchLabels: { app: ishtar-llm, component: inference }
  template:
    metadata:
      labels: { app: ishtar-llm, component: inference, version: v1.3.0 }
    spec:
      # Schedule onto GPU nodes (taint set by the Terraform module above).
      nodeSelector: { nodeType: gpu, workload: llm-inference }
      tolerations:
      - { key: "gpu", operator: "Equal", value: "true", effect: "NoSchedule" }

      # Spread replicas across hosts for HA.
      affinity:
        podAntiAffinity:
          ...
          - weight: 100
            podAffinityTerm:
              labelSelector:
                matchExpressions:
                - { key: app, operator: In, values: [ishtar-llm] }
      ...
      - name: inference-server
        ...
        resources:
          requests: { nvidia.com/gpu: 1, memory: "40Gi", cpu: "8"  }
          limits:   { nvidia.com/gpu: 1, memory: "45Gi", cpu: "16" }
        env:
        - { name: MODEL_NAME,    value: "llama-2-13b-chat"   }
        - { name: MODEL_PATH,    value: "/models/llama-2-13b" }
        - { name: MAX_BATCH_SIZE, value: "8" }
        - { name: PORT,           value: "8000" }
        # Readiness gives the model 120s to load before serving traffic.
        livenessProbe:
      ...
      volumes:
      - name: model-storage
        persistentVolumeClaim: { claimName: ishtar-models-pvc }
      securityContext: { runAsNonRoot: true, runAsUser: 1000, fsGroup: 1000 }

---
apiVersion: v1
kind: Service
metadata: { name: ishtar-llm-inference, namespace: production }
spec:
  selector: { app: ishtar-llm, component: inference }
...
When models exceed the memory of a single GPU, serving requires model parallelism, pipeline parallelism, or hybrid approaches. Frameworks such as DeepSpeed-Inference and Megatron-LM~\cite{megatron_lm_2019} inference mode support partitioning across GPUs and nodes, overlapping communication and compute to reduce latency. These techniques are essential for serving multi-hundred-billion parameter models, though most organizations prefer quantization or distillation to fit models onto single accelerators where possible.

\subsection{Summary}\label{sec:serving-summary}

The landscape of LLM serving is rapidly evolving. By combining robust serving frameworks (e.g., TGI, vLLM, TensorRT) with novel methods (e.g., Smoothie routing~\cite{Guha2024Smoothie}, KV cache optimizations, speculative decoding), LLMOps practitioners can achieve significant gains in both performance and cost-efficiency. What is cutting-edge today often becomes standard within a year—continuous awareness of new serving methods is therefore critical for engineering competitive, scalable LLM systems. Table~\ref{tab:ch03_servingcompare} summarizes the key trade-offs among major serving frameworks to guide framework selection.
...
\begin{tikzpicture}[
  font=\footnotesize,
  box/.style={rectangle, rounded corners=4pt, draw=black!70, line width=0.9pt, align=center},
  header/.style={font=\small\bfseries, inner sep=4pt, text=black!90},
  res/.style={rectangle, rounded corners=3pt, draw=black!70, line width=0.8pt,
              minimum height=9mm, font=\footnotesize\bfseries, align=center, text width=18mm},
  arrow/.style={llm/arrow},
  arrowDashed/.style={llm/arrow/dashed},
  cloud/.style={box, fill=llmblue!15, draw=llmblue!80!black, line width=1pt,
                minimum width=44mm, minimum height=56mm, inner sep=6pt},
  vpc/.style={box, fill=llmblue!18, draw=llmblue!75!black, line width=0.8pt, inner sep=5pt},
  onprem/.style={box, fill=llmpurple!15, draw=llmpurple!75!black, line width=1pt,
                 minimum width=44mm, minimum height=56mm, inner sep=6pt},
  k8s/.style={box, fill=llmgreen!18, draw=llmgreen!75!black, line width=0.8pt, inner sep=5pt},
  lb/.style={res, fill=llmorange!25, draw=llmorange!75!black},
  gw/.style={res, fill=llmgreen!25, draw=llmgreen!75!black},
  gpu/.style={res, fill=llmpurple!25, draw=llmpurple!75!black, text width=14mm},
  store/.style={res, fill=llmorange!25, draw=llmorange!75!black}
]

% ================== SECTION HEADERS (above box tops at y=50mm) ==================
\node[header, font=\normalsize\bfseries] at (-62mm, 56mm) {Cloud-Native};
\node[header, font=\normalsize\bfseries] at (63mm, 56mm) {Hybrid};
\node[header, font=\normalsize\bfseries] at (5mm, -34mm) {Multi-Cluster / Multi-Region};

% ================== (A) CLOUD-NATIVE (Top Left) ==================
\node[cloud] (cloudA) at (-62mm, 22mm) {};
\node[font=\footnotesize\bfseries, text=black!65] at (cloudA.north west)
  [anchor=north west, xshift=2mm, yshift=-2mm] {Cloud};
\node[vpc, minimum width=40mm, minimum height=50mm] (vpcA) at (cloudA.center) {};
\node[font=\footnotesize\bfseries, text=black!65] at (vpcA.north west)
  [anchor=north west, xshift=2mm, yshift=-1.5mm] {VPC};
\node[lb] (apigwA) at ($(vpcA.north) + (0, -10mm)$) {API\\Gateway};
\node[k8s, minimum width=36mm, minimum height=14mm] (k8sA) at ($(vpcA.center) + (0, 5mm)$) {K8s Cluster};
\node[store] (objA) at ($(vpcA.center) + (0, -8mm)$) {Object\\Store};
\node[gpu] (gpuA1) at ($(vpcA.south) + (-10mm, 7mm)$) {GPU};
\node[gpu] (gpuA2) at ($(vpcA.south) + (10mm, 7mm)$) {GPU};

\draw[arrow] (apigwA) -- (k8sA);
\draw[arrow] (k8sA) -- (objA);

% ================== (B) HYBRID (Top Right) ==================
% On-Premise block (shifted left to create room for connection nodes)
\node[onprem] (premB) at (15mm, 22mm) {};
\node[font=\footnotesize\bfseries, text=black!65] at (premB.north west)
  [anchor=north west, xshift=2mm, yshift=-2mm] {On-Premise};
\node[k8s, minimum width=36mm, minimum height=14mm] (k8sBprem) at ($(premB.center) + (0, 12mm)$)
  {K8s Cluster};
\node[store] (vecB) at ($(premB.center) + (0, -4mm)$) {Vector DB};
\node[gpu] (gpuB1) at ($(premB.south) + (-10mm, 7mm)$) {GPU};
\node[gpu] (gpuB2) at ($(premB.south) + (10mm, 7mm)$) {GPU};

% Cloud block (shifted right to create room for connection nodes)
\node[cloud] (cloudB) at (112mm, 22mm) {};
\node[font=\footnotesize\bfseries, text=black!65] at (cloudB.north west)
  [anchor=north west, xshift=2mm, yshift=-2mm] {Cloud};
\node[vpc, minimum width=40mm, minimum height=50mm] (vpcB) at (cloudB.center) {};
\node[font=\footnotesize\bfseries, text=black!65] at (vpcB.north west)
  [anchor=north west, xshift=2mm, yshift=-1.5mm] {VPC};
\node[lb] (apigwB) at ($(vpcB.north) + (0, -10mm)$) {API\\Gateway};
\node[k8s, minimum width=36mm, minimum height=14mm] (k8sBcloud) at ($(vpcB.center) + (0, 5mm)$)
  {K8s Cluster};
\node[gpu] (gpuB3) at ($(vpcB.south) + (-10mm, 7mm)$) {GPU};
\node[gpu] (gpuB4) at ($(vpcB.south) + (10mm, 7mm)$) {GPU};

% Connection nodes - narrower (14mm) and well-spaced in the 53mm gap
\node[gw, text width=14mm] (vpnL) at (46mm, 22mm) {VPN\\Gateway};
\node[gw, text width=14mm] (vpnR) at (76mm, 22mm) {Direct\\Connect};

\draw[arrow] (k8sBprem.east) -- (vpnL.west);
\draw[arrow] (vpnR.east) -- (k8sBcloud.west);
\draw[arrowDashed] (vpnL) -- (vpnR);
\draw[arrow] (k8sBprem) -- (vecB);
\draw[arrow] (apigwB) -- (k8sBcloud);

% ================== (C) MULTI-CLUSTER (Bottom Center) ==================
\node[cloud, minimum width=95mm, minimum height=52mm] (cloudC) at (5mm, -65mm) {};
\node[font=\footnotesize\bfseries, text=black!65] at (cloudC.north west)
  [anchor=north west, xshift=2mm, yshift=-2mm] {Multi-Cloud/Region};
\node[lb] (glbC) at ($(cloudC.north) + (0, -12mm)$) {Global\\LB/DNS};

% Wider VPC spacing (+-26mm) gives 20mm gap for failover label
\node[vpc, minimum width=36mm, minimum height=36mm] (vpcC1) at ($(cloudC.center) + (-26mm, -6mm)$) {};
\node[font=\footnotesize\bfseries, text=black!65] at (vpcC1.north west)
  [anchor=north west, xshift=2mm, yshift=-1.5mm] {Region A};
\node[k8s, minimum width=32mm, minimum height=14mm] (k8sC1) at ($(vpcC1.center) + (0, 8mm)$)
  {K8s Cluster};
\node[gpu] (gpuC1) at ($(vpcC1.south) + (-9mm, 7mm)$) {GPU};
\node[gpu] (gpuC2) at ($(vpcC1.south) + (9mm, 7mm)$) {GPU};

\node[vpc, minimum width=36mm, minimum height=36mm] (vpcC2) at ($(cloudC.center) + (26mm, -6mm)$) {};
\node[font=\footnotesize\bfseries, text=black!65] at (vpcC2.north west)
  [anchor=north west, xshift=2mm, yshift=-1.5mm] {Region B};
\node[k8s, minimum width=32mm, minimum height=14mm] (k8sC2) at ($(vpcC2.center) + (0, 8mm)$)
  {K8s Cluster};
\node[gpu] (gpuC3) at ($(vpcC2.south) + (-9mm, 7mm)$) {GPU};
\node[gpu] (gpuC4) at ($(vpcC2.south) + (9mm, 7mm)$) {GPU};

\draw[arrow] (glbC.south west) ++(2mm,0) -- (k8sC1.north);
\draw[arrow] (glbC.south east) ++(-2mm,0) -- (k8sC2.north);
\draw[arrowDashed] (k8sC1) --
  node[font=\footnotesize\bfseries, align=center, midway, below=1mm, text=black!80]
  {failover} (k8sC2);

% ================== LEGEND (Bottom) ==================
\node[box, fill=white, draw=black!40, rounded corners=4pt, line width=0.8pt,
      minimum width=190mm, minimum height=10mm, inner sep=4pt] (legend)
  at (20mm, -98mm) {};

\node[cloud, minimum width=8mm, minimum height=6mm, inner sep=1pt] (L1) at ($(legend.west) + (8mm, 0)$) {};
\node[font=\footnotesize, right=2mm of L1.east, anchor=west] {Cloud};
\node[vpc, minimum width=8mm, minimum height=6mm, inner sep=1pt] (L2) at ($(L1.east) + (18mm, 0)$) {};
\node[font=\footnotesize, right=2mm of L2.east, anchor=west] {VPC};
\node[onprem, minimum width=8mm, minimum height=6mm, inner sep=1pt] (L3) at ($(L2.east) + (16mm, 0)$) {};
\node[font=\footnotesize, right=2mm of L3.east, anchor=west] {On-Prem};
\node[k8s, minimum width=8mm, minimum height=6mm, inner sep=1pt] (L4) at ($(L3.east) + (22mm, 0)$) {};
\node[font=\footnotesize, right=2mm of L4.east, anchor=west] {K8s};
\node[gpu, minimum width=8mm, minimum height=6mm, text width=6mm, inner sep=1pt] (L5) at ($(L4.east) + (16mm, 0)$) {};
\node[font=\footnotesize, right=2mm of L5.east, anchor=west] {GPU};
\node[lb, minimum width=8mm, minimum height=6mm, text width=6mm, inner sep=1pt] (L6) at ($(L5.east) + (16mm, 0)$) {};
\node[font=\footnotesize, right=2mm of L6.east, anchor=west] {API GW};
\node[gw, minimum width=8mm, minimum height=6mm, text width=6mm, inner sep=1pt] (L7) at ($(L6.east) + (20mm, 0)$) {};
\node[font=\footnotesize, right=2mm of L7.east, anchor=west] {VPN};
\node[store, minimum width=8mm, minimum height=6mm, text width=6mm, inner sep=1pt] (L8) at ($(L7.east) + (16mm, 0)$) {};
\node[font=\footnotesize, right=2mm of L8.east, anchor=west] {Storage};
...
A representative cloud-native architecture is illustrated in Fig.~\ref{fig:ch03_cloud_native_deployment}. Figure~\ref{fig:ch03_deploy_patterns_balanced} contrasts cloud-native, hybrid, and multi-cluster deployment patterns to illustrate the operational and cost trade-offs inherent in each approach.
...
\begin{figure}[tb]
  ...
  \begin{tikzpicture}[
    ...
    cloud/.style={llm/box/purple, ellipse, minimum width=32mm, minimum height=11mm, font=\footnotesize\bfseries, align=center, text width=30mm},
    ingress/.style={llm/box/blue, minimum width=36mm, minimum height=11mm, font=\footnotesize\bfseries, align=center, text width=34mm},
    security/.style={llm/box/orange, fill=llmorange!18, draw=llmorange!70, minimum width=24mm, minimum height=10mm, font=\footnotesize\bfseries, align=center, text width=22mm},
    k8s/.style={llm/box/orange, fill=llmorange!25, draw=llmorange!70, minimum width=28mm, minimum height=10mm, font=\footnotesize\bfseries, align=center},
    gpu/.style={llm/box/orange, fill=llmorange!12, draw=llmorange!50, minimum width=15mm, minimum height=9mm, font=\footnotesize, align=center},
    managed/.style={llm/box/green, fill=llmgreen!15, draw=llmgreen!70, minimum width=24mm, minimum height=9mm, font=\footnotesize, align=center, text width=22mm},
    az/.style={llm/lane, inner sep=6pt, minimum height=34mm},
    legendsw/.style={rectangle, draw=black!40, minimum width=6mm, minimum height=4mm},
  ]
    % Top: Cloud provider
    \node[cloud] (cp) {Cloud Provider\\(AWS/Azure/GCP)};

    % IAM and Security groups flank the API gateway
    \node[ingress, below=6mm of cp] (api) {API Gateway\\Load Balancer};
    \node[security, left=8mm of api]  (iam) {IAM};
    \node[security, right=8mm of api] (sg)  {Security\\Groups};

    % AZ-1 container
    \node[k8s, below=12mm of api, xshift=-30mm] (k8s1) {K8s Cluster};
    \node[gpu, below=4mm of k8s1, xshift=-10mm]  (g11) {GPU\\Node};
    \node[gpu, right=2mm of g11]                   (g12) {GPU\\Node};
    \node[gpu, right=2mm of g12]                   (g13) {GPU\\Node};
    \node[gpu, below=1mm of g11, xshift=0mm, text=black!40] (g14) {GPU\\Node};
    \node[gpu, right=2mm of g14, text=black!40]             (g15) {GPU\\Node};
    \node[llm/label, below=2mm of g14.south east, anchor=north, xshift=2mm, font=\footnotesize\bfseries, text=llmorange!80!black] (as) {Auto-Scale Reserve};
    \node[fit=(k8s1)(g14)(g15)(as), az, label={[font=\footnotesize\bfseries,text=llmblue!70!black]above:AZ-1}] (az1) {};

    % AZ-2 container
    \node[k8s, right=18mm of k8s1] (k8s2) {K8s Cluster};
    \node[gpu, below=4mm of k8s2, xshift=-7mm]  (g21) {GPU\\Node};
    \node[gpu, right=2mm of g21]                 (g22) {GPU\\Node};
    \node[fit=(k8s2)(g21)(g22), az, label={[font=\footnotesize\bfseries,text=llmblue!70!black]above:AZ-2}] (az2) {};

    % "Private Subnets" outer lane
    \node[fit=(az1)(az2), llm/lane, inner sep=9pt, label={[font=\footnotesize\bfseries,text=llmblue!70!black]below:Private Subnets}] (priv) {};

    % Managed services column (right)
    \node[managed, right=12mm of az2, yshift=12mm] (sqs) {SQS\\Pub/Sub};
    \node[managed, below=2mm of sqs]               (vdb) {Vector\\DB};
    \node[managed, below=2mm of vdb]               (obj) {Object\\Store};
    \node[managed, below=2mm of obj]               (brk) {Bedrock\\OpenAI};
    \node[fit=(sqs)(brk), llm/lane, inner sep=5pt, dash pattern=on 1pt off 1pt] (mgsbox) {};
    \node[llm/label, right=2mm of mgsbox.east, font=\footnotesize\bfseries, text=llmpurple!70!black, align=center] {DR Region\\via IaC};

    % ------- Arrows -------
    \draw[llm/arrow] (cp)  -- (api);
    \draw[llm/arrow] (iam) -- (api);
    \draw[llm/arrow] (sg)  -- (api);
    \draw[llm/arrow] (api) -- (k8s1);
    \draw[llm/arrow] (api) -- (k8s2);

    \draw[llm/arrow/dashed] (k8s1.east) to[bend left=5]  (sqs.west);
    \draw[llm/arrow/dashed] (k8s1.east) to[bend right=5] (vdb.west);
    \draw[llm/arrow/dashed] (k8s1.east) to[bend right=15](obj.west);
    \draw[llm/arrow/dashed] (k8s1.east) to[bend right=20](brk.west);
    \draw[llm/arrow/dashed] (k8s2.east) to[bend left=10] (sqs.west);
    \draw[llm/arrow/dashed] (k8s2.east) to[bend left=5]  (vdb.west);
    \draw[llm/arrow/dashed] (k8s2.east) to[bend right=5] (obj.west);
    \draw[llm/arrow/dashed] (k8s2.east) to[bend right=10](brk.west);
    % A small upward arrow from the Auto-Scale label to the reserve row
    % indicates that the reserve nodes can be expanded on demand.
    \draw[{Latex[length=2mm,width=1.4mm]}-,
          line width=0.6pt, color=llmorange!80!black, dashed]
      (g14.south -| as.north) -- (as.north);

    % Legend
    \node[llm/label, below=10mm of priv.south west, anchor=north west, font=\footnotesize\bfseries] (klbl) {Key Features:};
    \node[legendsw, fill=llmorange!18, draw=llmorange!50, below=3mm of klbl.south west, anchor=north west] (le1) {};
    \node[llm/label, right=2mm of le1, font=\footnotesize] {GPU Nodes (Auto-scaling)};
    \node[legendsw, fill=llmgreen!15,  draw=llmgreen!70,  below=2mm of le1]  (le2) {};
    \node[llm/label, right=2mm of le2, font=\footnotesize] {Managed Services};
    \node[legendsw, fill=llmorange!18, draw=llmorange!70, below=2mm of le2]  (le3) {};
    \node[llm/label, right=2mm of le3, font=\footnotesize] {Security \& IAM};
  \end{tikzpicture}
  \end{llmfigbox}
...
\begin{itemize}
  ...
  \item \textbf{H100 GPUs (cloud, AWS)} operate in an autoscaling group (0–4 instances). These instances activate during spikes, doubling throughput to $\sim$250--300 tokens/s per stream due to FP8 precision and HBM3 bandwidth. Although their hourly price is higher, cost-per-token is $\sim$50\% lower than A100s when fully utilized~\cite{fabricatedknowledge,lambda}.
\end{itemize}
...
\begin{tikzpicture}[
  ...
  gpu/.style ={llm/box/purple, fill=llmpurple!18, draw=llmpurple!70, minimum width=20mm, minimum height=9mm,  font=\footnotesize, align=center},
  gpuH/.style={llm/box/purple, fill=llmpurple!30, draw=llmpurple!80, minimum width=24mm, minimum height=9mm,  font=\footnotesize\bfseries, align=center},
  gpuA/.style={llm/box/purple, fill=llmpurple!10, draw=llmpurple!50, minimum width=24mm, minimum height=9mm,  font=\footnotesize, align=center},
  embed/.style={llm/box/orange, fill=llmorange!18, draw=llmorange!70, minimum width=32mm, minimum height=13mm, font=\footnotesize\bfseries, align=center, text width=30mm},
  iac/.style ={llm/box/blue,   minimum width=58mm, minimum height=11mm, font=\footnotesize\bfseries, align=center, text width=54mm},
  pods/.style={llm/box/orange, fill=llmorange!18, draw=llmorange!70, minimum width=58mm, minimum height=11mm, font=\footnotesize\bfseries, align=center, text width=54mm},
  vpn/.style ={llm/box/green,  fill=llmgreen!15,  draw=llmgreen!70,  minimum width=18mm, minimum height=10mm, font=\footnotesize\bfseries, align=center},
  group/.style={llm/lane, inner sep=7pt},
  legendsw/.style={rectangle, draw=black!40, minimum width=6mm, minimum height=4mm},
]
  % Left: on-prem L4 cluster
  \node[gpu] (l41) {L4 GPU};
  \node[gpu, right=4mm of l41] (l42) {L4 GPU};
  \node[embed, below=6mm of $(l41)!0.5!(l42)$] (embed) {Embeddings /\\NER Tagging};
  \node[fit=(l41)(l42)(embed), group,
        label={[font=\footnotesize\bfseries,text=black!70]above:On-Prem (L4 Cluster)}] (onprem) {};

  % Right: AWS EKS cluster
  \node[iac,  right=32mm of l41, xshift=8mm] (tf) {Terraform + ArgoCD + K8s Autoscaling};
  \node[pods, below=3mm of tf] (pods) {Model Serving Pods (TGI / vLLM)};

  % Autoscaling node group
  \node[gpuH, below=5mm of pods.south, xshift=-15mm] (h1) {H100};
  \node[gpuH, right=4mm of h1] (h2) {H100};
  \node[llm/label, font=\footnotesize\bfseries, above=0.5mm of h1.north west, anchor=south west, xshift=-4mm] {Autoscaling Node Group};
  \node[fit=(h1)(h2), group, inner sep=5pt] (asng) {};

  % Baseline node group
  \node[gpuA, below=4mm of h1] (a1) {A100};
  \node[gpuA, right=4mm of a1]  (a2) {A100};
  \node[llm/label, font=\footnotesize, above=0.5mm of a1.north west, anchor=south west, xshift=-4mm] {Baseline Node Group};
  \node[fit=(a1)(a2), group, inner sep=5pt] (bng) {};

  \node[fit=(tf)(pods)(asng)(bng), group, inner sep=8pt,
        label={[font=\footnotesize\bfseries,text=llmblue!70!black]above:AWS EKS Cluster}] (eks) {};

  % VPN bridge
  \node[vpn] (vpn) at ($(onprem.east)!0.5!(eks.west)$) {VPN};

  \draw[llm/arrow] (l41) -- (embed);
  \draw[llm/arrow] (l42) -- (embed);
  \draw[llm/arrow] (onprem.east) -- (vpn.west);
  \draw[llm/arrow] (vpn.east) -- (eks.west);
  \draw[llm/arrow] (tf) -- (pods);

  % Legend
  \node[llm/label, below=6mm of bng.south west, anchor=north west, font=\footnotesize, xshift=-40mm] (k1) {};
  \node[legendsw, fill=llmpurple!10, draw=llmpurple!50, right=0mm of k1.west] (le1) {};
  \node[llm/label, right=2mm of le1, font=\footnotesize] (le1t) {L4 / A100 GPU};
  \node[legendsw, fill=llmpurple!30, draw=llmpurple!80, right=8mm of le1t] (le2) {};
  \node[llm/label, right=2mm of le2, font=\footnotesize] (le2t) {H100 GPU};
  \node[legendsw, fill=llmorange!18, draw=llmorange!70, right=8mm of le2t] (le3) {};
  \node[llm/label, right=2mm of le3, font=\footnotesize] (le3t) {Serving Pods};
  \node[legendsw, fill=llmblue!15,   draw=llmblue!70,   right=8mm of le3t] (le4) {};
  \node[llm/label, right=2mm of le4, font=\footnotesize] {IaC / Control Plane};
\end{tikzpicture}
\end{llmfigbox}
...
\textbf{Benchmark Cost-Per-Token} & For each GPU/TPU candidate, benchmark tokens/sec and compute cost per 1k tokens~\cite{fabricatedknowledge,lambda}. A more expensive hourly instance may be cheaper per output due to higher throughput. \\

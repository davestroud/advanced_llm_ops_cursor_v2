...
\begin{lstlisting}[
  float=htbp,
  style=springerfloat,
  caption={Reusable Terraform module for provisioning GPU-enabled EKS node groups with configurable instance types, autoscaling limits, Kubernetes taints and labels for workload isolation, and least-privilege IAM roles. This module encapsulates the GPU infrastructure pattern so that teams can stamp out consistent node groups across environments without duplicating configuration or risking security policy drift.},
...
\begin{lstlisting}[
  float=htbp,
  style=springerfloat,
  caption={Kubernetes Deployment and Service manifests for an LLM inference service with GPU resource requests, node-selector and taint-based scheduling for GPU isolation, pod anti-affinity for high availability across hosts, extended readiness probes that account for model loading time, and a non-root security context. This production-grade manifest demonstrates the operational patterns needed to reliably serve large language models on shared GPU clusters.},

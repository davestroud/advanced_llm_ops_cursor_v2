...
% Lanes
\node[lane, fill=llmblue!8, minimum width=0.95\linewidth, minimum height=28mm] (inglane) {};
\node[lane, fill=llmgreen!8, minimum width=0.95\linewidth, minimum height=32mm, below=9mm of inglane] (querylane) {};
...
\node[box, fill=llmorange!15, anchor=west] (sources) at ([xshift=7mm]inglane.west) {Sources\\\footnotesize (news, NGO,\\social, health)};
\node[box, fill=llmpurple!15, right=7mm of sources] (ingest) {Ingestion\\\footnotesize parse \& normalize};
\node[box, fill=llmpurple!15, right=7mm of ingest] (scrub) {Safety preprocessing\\\footnotesize PII/coord scrubbing};
\node[box, fill=llmpurple!15, right=7mm of scrub] (chunkembed) {Chunk \& embed\\\footnotesize batch GPU/CPU};
\node[box, fill=llmpurple!15, right=7mm of chunkembed] (docstore) {Doc store\\\footnotesize text + metadata};
\node[box, fill=llmpurple!15, right=7mm of docstore] (vec) {Vector index\\\footnotesize FAISS/HNSW};
...
% Query lane nodes
\node[box, fill=llmgreen!15, anchor=west] (user) at ([xshift=7mm]querylane.west) {Journalist\\\footnotesize query/UI};
\node[box, fill=llmgreen!15, right=7mm of user] (gateway) {API gateway\\\footnotesize auth \& rate limits};
\node[box, fill=llmgreen!15, right=7mm of gateway] (controller) {Orchestrator\\\footnotesize agent controller};
\node[box, fill=llmorange!15, right=7mm of controller] (retr) {Retriever\\\footnotesize ANN + filters};
\node[box, fill=llmgreen!15, right=7mm of retr] (llm) {LLM serving\\\footnotesize vLLM/TGI};
\node[box, fill=llmgreen!15, right=7mm of llm] (agents) {Post-processing\\\footnotesize verify \& safety};
\node[box, fill=llmgreen!15, right=7mm of agents] (resp) {Answer\\\footnotesize citations + disclaimers};
...
\addplot[color=blue!70!black, mark=square*, mark options={fill=blue!50}]
...
\begin{axis}[
    ...
    tick label style={font=\footnotesize},
    legend style={at={(0.5,-0.38)}, anchor=north, legend columns=2, font=\footnotesize},
    every axis plot/.append style={thick, mark size=2.5pt},
]
\addplot[color=red!70!black, mark=triangle*, mark options={fill=red!50}]
    coordinates {(1,62) (2,78) (3,83) (4,85) (5,89) (6,93)};
\addlegendentry{Accuracy (\%)}
\end{axis}
\end{tikzpicture}
\end{llmfigbox}
\caption{Latency fell and accuracy climbed monotonically across \ishtar{}'s four deployment phases, showing that systematic LLMOps practices compound rather than trade off. The left axis tracks latency (driven down by infrastructure optimizations and autoscaling) while the right axis tracks accuracy (driven up by RAG, evaluation gates, and monitoring)---each phase contributed measurable gains in both directions.}

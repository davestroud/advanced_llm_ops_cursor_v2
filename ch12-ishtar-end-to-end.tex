...
\begin{lstlisting}[
  float=t,
  style=springerfloat,
  caption={Terraform module interface for provisioning the Ishtar Kubernetes cluster. The module abstracts region, node sizing, GPU autoscaling bounds, and OIDC configuration into a declarative interface that enables reproducible multi-region deployments.},
...
\begin{lstlisting}[
  float=t,
  style=springerfloat,
  caption={Kubernetes Deployment manifest for the Ishtar LLM serving pod. The configuration pins GPU workloads to dedicated nodes via node selectors and tolerations, and requests a single GPU resource per replica for inference.},
...
\begin{lstlisting}[
  float=t,
  style=springerfloat,
  caption={Ishtar production prompt template enforcing source-grounded answering with inline citations. The system instructions mandate citation format, conflict disclosure, and privacy constraints that enable both automated evaluation and human verification.},
...
\begin{lstlisting}[
  float=t,
  style=springerfloat,
  caption={Agent orchestration workflow implementing the Ishtar controller pattern. The state-machine pipeline sequences retrieval, synthesis, verification, safety, and optional translation agents with explicit handoffs at each stage.},
...
\begin{lstlisting}[
  float=t,
  style=springerfloat,
  caption={CI/CD workflow configuration for the Ishtar release pipeline. The workflow runs unit tests, executes offline evaluation with quality gates, and performs a canary rollout to Kubernetes only when all gates pass.},

...
\centering
\begin{llmfigbox}
% Style contract (macros.tex):
%   purple = external actors (sources, journalist)
%   blue   = data-handling workers and stores (ingest, embed, retriever, LLM)
%   green  = control / orchestration plane (gateway, orchestrator)
%   orange = final callout (answer with citations + disclaimers)
% Lanes use subtle palette tints (blue for background, green for interactive).
\begin{tikzpicture}[
  font=\small,
  ext/.style    ={llm/box/purple, font=\footnotesize\bfseries, minimum width=22mm},
  data/.style   ={llm/box/blue,   font=\footnotesize\bfseries, minimum width=22mm},
  ctrl/.style   ={llm/box/green,  font=\footnotesize\bfseries, minimum width=22mm},
  callout/.style={llm/box/orange, font=\footnotesize\bfseries, minimum width=22mm},
  bglane/.style ={draw=none, rounded corners=6pt, fill=#1!7,
                  minimum width=0.95\linewidth},
  flow/.style   ={llm/arrow},
  loop/.style   ={llm/arrow/dashed},
  note/.style   ={font=\footnotesize\bfseries, text=black!80, align=left},
]

% Lanes (background bands; semantics carried by node colors and shapes)
\node[bglane=llmblue,  minimum height=28mm] (inglane) {};
\node[bglane=llmgreen, minimum height=32mm, below=9mm of inglane] (querylane) {};
\node[note, anchor=west] at ([xshift=5mm]inglane.north west)
      {Continuous ingestion \& indexing (background)};
\node[note, anchor=west] at ([xshift=5mm]querylane.north west)
      {Online query handling (interactive path)};

% Ingestion lane (left to right)
\node[ext,  anchor=west]                     (sources)    at ([xshift=7mm]inglane.west)
                                                          {Sources\\\footnotesize (news, NGO,\\social, health)};
\node[data, right=7mm of sources]            (ingest)     {Ingestion\\\footnotesize parse \& normalize};
\node[data, right=7mm of ingest]             (scrub)      {Safety preprocessing\\\footnotesize PII/coord scrubbing};
\node[data, right=7mm of scrub]              (chunkembed) {Chunk \& embed\\\footnotesize batch GPU/CPU};
\node[data, right=7mm of chunkembed]         (docstore)   {Doc store\\\footnotesize text + metadata};
\node[data, right=7mm of docstore]           (vec)        {Vector index\\\footnotesize FAISS/HNSW};

\draw[flow] (sources)    -- (ingest);
\draw[flow] (ingest)     -- (scrub);
\draw[flow] (scrub)      -- (chunkembed);
\draw[flow] (chunkembed) -- (docstore);
\draw[flow] (chunkembed) -- (vec);

% Query lane (left to right)
\node[ext,     anchor=west]                  (user)       at ([xshift=7mm]querylane.west)
                                                          {Journalist\\\footnotesize query/UI};
\node[ctrl,    right=7mm of user]            (gateway)    {API gateway\\\footnotesize auth \& rate limits};
\node[ctrl,    right=7mm of gateway]         (controller) {Orchestrator\\\footnotesize agent controller};
\node[data,    right=7mm of controller]      (retr)       {Retriever\\\footnotesize ANN + filters};
\node[data,    right=7mm of retr]            (llm)        {LLM serving\\\footnotesize vLLM/TGI};
\node[data,    right=7mm of llm]             (agents)     {Post-processing\\\footnotesize verify \& safety};
\node[callout, right=7mm of agents]          (resp)       {Answer\\\footnotesize citations + disclaimers};

\draw[flow] (user)       -- (gateway);
\draw[flow] (gateway)    -- (controller);
\draw[flow] (controller) -- (retr);
\draw[flow] (retr)       -- (llm);
\draw[flow] (llm)        -- (agents);
\draw[flow] (agents)     -- (resp);

% Retrieval dependency (dashed): route through the gap between lanes so it does
% not visually cross llm / agents / resp on its way to the vector index.
\coordinate (gap) at ($(inglane.south)!0.5!(querylane.north)$);
\draw[loop] (retr.north) -- (retr.north |- gap)
                         -- (vec.south |- gap)
                         -- (vec.south);
...
\end{tikzpicture}
\end{llmfigbox}
\caption{Splitting \ishtar{} into a background ingestion lane and an interactive query lane lets each plane be scaled, monitored, and governed independently---the foundation of a production-ready LLMOps system. External actors (purple) feed data workers and stores (blue); the orchestration plane (green) authenticates and routes each query through retrieval, LLM serving, and post-processing before returning a citation-anchored answer (orange). The dashed link to the vector index marks the only cross-plane dependency, and observability (traces, metrics, audit logs) spans every component.}
...
\begin{codebox}{Terraform module interface for provisioning the Ishtar Kubernetes cluster. The module abstracts region, node sizing, GPU autoscaling bounds, and OIDC configuration into a declarative interface that enables reproducible multi-region deployments.}{lst:ch12_terraform_module}
...
\begin{codebox}{Kubernetes Deployment manifest for the Ishtar LLM serving pod. The configuration pins GPU workloads to dedicated nodes via node selectors and tolerations, and requests a single GPU resource per replica for inference.}{lst:ch12_k8s_deployment}
\begin{minted}{yaml}
apiVersion: apps/v1
kind: Deployment
metadata:
...
\begin{codebox}{Ishtar production prompt template enforcing source-grounded answering with inline citations. The system instructions mandate citation format, conflict disclosure, and privacy constraints that enable both automated evaluation and human verification.}{lst:ch12_prompt_template}
\begin{minted}{text}
...
\begin{codebox}{Agent orchestration workflow implementing the Ishtar controller pattern. The state-machine pipeline sequences retrieval, synthesis, verification, safety, and optional translation agents with explicit handoffs at each stage.}{lst:ch12_agent_orchestration}
\begin{minted}{python}
...
\begin{codebox}{CI/CD workflow configuration for the Ishtar release pipeline. The workflow runs unit tests, executes offline evaluation with quality gates, and performs a canary rollout to Kubernetes only when all gates pass.}{lst:ch12_cicd_workflow}
\begin{minted}{yaml}
name: ishtar-release
on: [push]
jobs:
...
\begin{itemize}
    ...
    \item \textbf{RAG quality metrics} (e.g., faithfulness/answer relevance via RAGAS)~\cite{ragas_eacl2024},
    \item \textbf{safety regression suites} (prompt injection, sensitive-data requests, refusal correctness) aligned with OWASP LLM guidance~\cite{OWASP2024LLMTop10}.
\end{itemize}
...
\subsection{On-call runbook}
\label{sec:ishtar-runbook}

Table~\ref{tab:ch12_runbook} codifies the most common \ishtar{} incidents into a structured runbook\index{runbook}\index{incident response!runbook} that on-call engineers can follow to detect, diagnose, resolve, and prevent recurrence.

\begin{table}[t]
\centering
\small
\caption{On-call runbook for common \ishtar{} incidents. Each row provides a structured response path from detection signal to prevention strategy, enabling rapid and consistent incident handling.}
\label{tab:ch12_runbook}
\setlength{\tabcolsep}{4pt}
\renewcommand{\arraystretch}{1.3}
\rowcolors{2}{gray!8}{white}
\begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{20mm}>{\raggedright\arraybackslash}p{20mm}>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}p{22mm}@{}}
\toprule
\rowcolor{gray!15}
\textbf{Incident} & \textbf{Detection} & \textbf{Diagnosis} & \textbf{Resolution} & \textbf{Prevention} \\
\midrule
Retrieval failure (no relevant docs) & Faithfulness $< 0.5$ & Check index status, embedding model version, source freshness & Re-index corpus; verify embedding pipeline; restore from snapshot & Automated freshness checks; index health probes \\
LLM timeout & TTFT $> 5$\,s & Check GPU utilization, inference queue depth, prompt length & Scale up GPU nodes; drain queue; reduce batch size & Autoscaling thresholds; prompt length budgets \\
Safety filter false positive & High rejection rate ($>15\%$) & Review blocked examples; check filter version and thresholds & Tune filter sensitivity; add allowlist entries & Regular filter calibration with domain experts \\
Citation hallucination & Source verification fails in audit & Trace retrieval pipeline; check reranker output & Fix retrieval ranking; add citation validation gate & Automated citation-entailment checks \\
Cost spike & \$/hour $> 2\times$ baseline & Check token usage by route; identify cache misses & Rate-limit high-cost routes; optimize prompts; enable caching & Budget alerts; per-route cost dashboards \\
\bottomrule
\end{tabularx}
\end{table}
...
Table~\ref{tab:ch12_ishtar_metrics} summarizes representative outcomes observed after full operationalization.
...
\subsection{Metrics evolution across deployment phases}
\label{sec:ishtar-metrics-evolution}

Fig.~\ref{fig:ch12_metrics_trend} shows how key metrics evolved as \ishtar{} progressed through successive deployment phases.\index{metrics!trend analysis} Each phase introduced an operational improvement---RAG integration, evaluation gates, autoscaling, monitoring, and full production hardening---and the cumulative effect is visible in both latency reduction and accuracy improvement.

\begin{figure}[t]
\centering
\begin{llmfigbox}
% Style contract (macros.tex):
%   llm/chart sets fonts, grid, tick, and axis-line weights book-wide.
%   Per-series colors are pinned to llmblue (latency) and llmorange (accuracy)
%   so the two axes remain distinguishable in grayscale via line style + marker.
\begin{tikzpicture}
\begin{axis}[
    llm/chart,
    xlabel={Deployment Phase},
    ylabel={Latency (ms)},
    axis y line*=left,
    ymin=500, ymax=3500,
    xtick={1,2,3,4,5,6},
    xticklabels={Baseline,{+RAG},{+Eval\\Gates},{+Auto-\\scaling},{+Moni-\\toring},{Full\\Prod.}},
    xticklabel style={font=\footnotesize, align=center},
    ylabel style={font=\small\bfseries, color=llmblue},
    legend style={at={(0.5,-0.28)}, anchor=north, legend columns=2,
                  font=\footnotesize, draw=black!40, fill=white,
                  cells={anchor=west}},
]
\addplot[color=llmblue, solid, mark=square*, mark size=1.8pt,
         mark options={fill=llmblue!60}]
    coordinates {(1,3200) (2,2400) (3,2200) (4,1600) (5,1400) (6,1100)};
\addlegendentry{p90 Latency (ms)}
\end{axis}
\begin{axis}[
    llm/chart,
    axis y line*=right,
    ...
    ymin=55, ymax=100,
    xtick={1,2,3,4,5,6},
    ylabel style={font=\small\bfseries, color=llmorange},
    legend style={at={(0.5,-0.38)}, anchor=north, legend columns=2,
                  font=\footnotesize, draw=black!40, fill=white,
                  cells={anchor=west}},
]
\addplot[color=llmorange, dashed, dash pattern=on 4pt off 2pt,
         mark=triangle*, mark size=2pt, mark options={fill=llmorange!60, solid}]
    coordinates {(1,62) (2,78) (3,83) (4,85) (5,89) (6,93)};
\addlegendentry{Accuracy (\%)}
\end{axis}
\end{tikzpicture}
\end{llmfigbox}
\caption{Latency fell and accuracy climbed monotonically across \ishtar{}'s deployment phases, showing that systematic LLMOps practices compound rather than trade off. The left axis tracks p90 latency (blue, solid; driven down by infrastructure optimizations and autoscaling) while the right axis tracks accuracy (orange, dashed; driven up by RAG, evaluation gates, and monitoring)---each phase contributed measurable gains in both directions.}
\label{fig:ch12_metrics_trend}
\end{figure}
...
\paragraph{Cost breakdown by component.}\index{cost!breakdown}
Table~\ref{tab:ch12_cost_breakdown} provides an illustrative cost breakdown for \ishtar{} in a representative monthly operating window. These figures are approximate and vary with traffic volume, model choice, and infrastructure provider; the key insight is the relative allocation across components and the optimization levers available at each tier.

\begin{table}[t]
\centering
\small
\caption{Illustrative cost breakdown by component for \ishtar{} at steady-state operation. LLM inference dominates total cost; optimization levers exist at every tier to reduce spend without compromising quality.}
\label{tab:ch12_cost_breakdown}
\setlength{\tabcolsep}{5pt}
\renewcommand{\arraystretch}{1.3}
\rowcolors{2}{gray!8}{white}
\begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{30mm}>{\raggedright\arraybackslash}p{18mm}>{\raggedright\arraybackslash}p{20mm}>{\raggedright\arraybackslash}X@{}}
\toprule
\rowcolor{gray!15}
\textbf{Component} & \textbf{\% of Total} & \textbf{Monthly Est.} & \textbf{Optimization Lever} \\
\midrule
\rowcolor{orange!10}
\textbf{LLM Inference} & \textcolor{orange!70!black}{\textbf{55\%}} & \textcolor{orange!70!black}{\textbf{\$4{,}400}} & Quantization, KV-cache optimization, prompt compression, model routing \\
\rowcolor{blue!8}
Retrieval (embeddings + search) & 20\% & \$1{,}600 & Index optimization, embedding caching, smaller embedding models \\
\rowcolor{blue!8}
Safety / verification pipeline & 10\% & \$800 & Smaller judge models, classifier cascades, sampling-based verification \\
\rowcolor{blue!8}
Infrastructure (K8s, storage) & 10\% & \$800 & Right-sizing, spot instances, scale-to-zero for non-critical services \\
\rowcolor{green!10}
Monitoring / observability & \textcolor{green!70!black}{\textbf{5\%}} & \$400 & Sampling strategies, log retention policies, metric aggregation \\
\bottomrule
\end{tabularx}
\end{table}
...
Implementing \ishtar{} end-to-end yielded lessons that generalize to other production LLM systems.\index{lessons learned} We consolidate them into eight themes.

\paragraph{1.\ Data quality is the foundation.}
RAG amplifies the impact of ingestion quality: noisy or duplicated data pollutes retrieval and degrades faithfulness. Early investment in cleaning, normalization, and source curation yields downstream gains across every metric.

\paragraph{2.\ Modular architecture enables velocity and safety.}
Clear interfaces between ingestion, retrieval, serving, and agents allow independent iteration, safer component swaps, and parallel team development. Modularity also reduces vendor lock-in as serving engines and vector databases evolve rapidly---FAISS offers speed with engineering overhead for metadata, while managed vector DBs\index{vector database} trade cost for operational simplicity.

\paragraph{3.\ Prompts and configurations require governance.}
Minor prompt edits can cause major regressions (e.g., dropped citations). Treat prompts as code: version, test, canary, and rollback. The same discipline applies to retrieval parameters, tool permissions, and safety thresholds.

\paragraph{4.\ Observability is non-negotiable.}
Multi-stage systems demand fine-grained attribution: is latency caused by retrieval, inference queueing, or verification loops? Without traces and semantic quality metrics, failures cannot be debugged and regressions go unnoticed.

\paragraph{5.\ Agentic verification improves quality but increases complexity.}
Multi-agent verification substantially reduces hallucinations and improves citation fidelity, but only if end-to-end tests cover inter-agent failure modes and citation consistency. Teams must budget for this additional testing surface.

\paragraph{6.\ User-facing design drives trust.}
Citations, calibrated uncertainty, and consistent tone matter as much as raw correctness. Users judge the system on perceived reliability, not internal accuracy scores.

\paragraph{7.\ Continuous improvement is the default mode.}
Deployment begins the iterative loop: production feedback, audits, and domain-expert review should directly drive backlog prioritization. Every incident should produce a regression test; every user complaint should inform evaluation criteria.

\paragraph{8.\ Domain expertise and ethical vigilance are ongoing requirements.}
Journalistic norms and domain review identify subtle failures (framing, missing context) that automated metrics miss. Safety policies and filters require continuous calibration to avoid both under- and over-blocking. Design for adaptability: crisis contexts change rapidly, and the system must evolve safely.
...
RAG-specific evaluation (citation precision\index{precision!citation}, faithfulness\index{faithfulness}, and evidence coverage\index{evidence coverage}) should evolve from ad hoc audits to standardized, automated suites (e.g., RAGAS)~\cite{ragas_eacl2024}.

...
\begin{itemize}
    ...
    \item \textbf{Integrating retrieval-augmented generation (RAG) pipelines\index{RAG!pipeline}} \parencite{Borzunov2023Petals} (to supplement the model with relevant external knowledge retrieved at query time).
...
\addlinespace[2pt]
\textbf{Knowledge Integration} & Static training data; periodic retraining & Dynamic retrieval-augmented generation (RAG); continuous knowledge injection without full retraining \parencite{Borzunov2023Petals,Zheng2024sglang} \\
\addlinespace[2pt]
...
\PitfallBox{Understanding these mathematical foundations is not merely academic---it is the basis for every operational decision covered in this book. The parameter memory equation determines how many GPUs you must provision (and therefore your infrastructure cost). The KV-cache formula governs how many concurrent users your system can serve before running out of memory. The cost-per-token model translates hardware choices into business economics. Without internalizing these relationships, teams risk over-provisioning (wasting budget), under-provisioning (violating latency SLAs), or choosing hardware configurations that cannot physically serve their workload. The equations below give practitioners the quantitative vocabulary to reason about capacity planning\index{capacity planning}, cost optimization\index{cost!optimization}, and scaling decisions\index{scaling} before committing real resources.}
...
these ideas to increase throughput under realistic workloads \cite{Kwon2023vLLM}.
...
The equations above define memory and compute requirements, but visual representations help solidify how these components interact. Figure~\ref{fig:ch02_transformer_block} illustrates the core Transformer block architecture, showing how attention mechanisms and feed-forward networks contribute to memory and compute costs. Figure~\ref{fig:ch02_quadratic_attn} demonstrates the quadratic scaling of attention complexity with sequence length, motivating the use of efficient attention variants and retrieval strategies for long contexts.

\begin{figure}[t]
    \centering
    \begin{llmfigbox}
    % Transformer block schematic. Semantic color mapping per book contract:
    %   blue   = data carrier (input/output token stream)
    %   orange = compute hotspot / decision-relevant block (attention)
    %   green  = control/normalization step (residual + LayerNorm)
    %   purple = external/auxiliary computation (FFN)
    \begin{tikzpicture}[
      node distance=10mm,
      tfblock/.style={minimum width=42mm, font=\footnotesize\bfseries},
      tflabel/.style={llm/label}
    ]
        \node[llm/box/blue, tfblock]                          (input)    {Input (tokens)};
        \node[llm/box/orange, tfblock, below=of input]        (mha)      {Multi-Head Attention};
        \node[llm/box/green, tfblock, below=10mm of mha]      (addnorm1) {Add \& Norm};
        \node[llm/box/purple, tfblock, below=10mm of addnorm1](ffn)      {Feed-Forward Network};
        \node[llm/box/green, tfblock, below=10mm of ffn]      (addnorm2) {Add \& Norm};
        \node[llm/box/blue, tfblock, below=12mm of addnorm2]  (output)   {Hidden States / Next Block};

        \draw[llm/arrow] (input)    -- (mha);
        \draw[llm/arrow] (mha)      -- (addnorm1);
        \draw[llm/arrow] (addnorm1) -- (ffn);
        \draw[llm/arrow] (ffn)      -- (addnorm2);
        \draw[llm/arrow] (addnorm2) -- (output);

        \draw[llm/arrow/dashed] ($(input.south west)+(1mm,-1mm)$)
              -- ++(0,-22mm) -| (addnorm1.west);
        \draw[llm/arrow/dashed] ($(addnorm1.south west)+(1mm,-1mm)$)
              -- ++(0,-17mm) -| (addnorm2.west);

        \node[tflabel, anchor=west] at ($(mha.east)+(1.2,0)$)
              {Self-attention\\(KV cache affects memory)};
        \node[tflabel, anchor=west] at ($(ffn.east)+(1.2,0)$)
              {$d \rightarrow d_{ff} \rightarrow d$};
        \node[tflabel, anchor=west] at ($(addnorm2.east)+(1.2,0)$)
              {Residual + LayerNorm};
    \end{tikzpicture}
    \end{llmfigbox}
...
\begin{figure}[t]
    \centering
    \begin{llmfigbox}
    % Quadratic attention complexity. Uses llm/chart + llm/cycle for
    % book-wide chart styling and grayscale-safe line/mark distinction.
    \begin{tikzpicture}
        \begin{axis}[llm/chart, llm/cycle,
            ...
            \addplot+[mark=none, smooth, domain=0:4096, samples=200]
                {(x^2)/(4096^2)};
...
\begin{figure}[t]
    \centering
    \begin{llmfigbox}
    % Parameter scaling over time (single series, log-y).
    \begin{tikzpicture}
      \begin{axis}[llm/chart, llm/cycle,
    ...
    \end{tikzpicture}
    \end{llmfigbox}
    \caption{Model size has grown roughly two orders of magnitude per half-decade, constraining single-GPU inference for the largest families and forcing LLMOps teams toward sharding, quantization, and distillation. The log-scale trajectory also explains why memory and cost---not accuracy---now dominate the production decision space.}
...
The trade-off between time-to-first-token (TTFT) and overall throughput is central to serving optimization, as illustrated in Figure~\ref{fig:ch02_ttft_throughput}. By varying batch sizes, systems can amortize initialization overhead across multiple requests, but must balance this against latency targets and GPU memory constraints.

% ------------------ Throughput vs TTFT Plot ------------------
\begin{figure}[t]
    \centering
    \begin{llmfigbox}
    % Throughput-vs-TTFT family. Constants: N = 120 tokens, tau1 = 0.02 s/token.
    \begin{tikzpicture}
        \begin{axis}[llm/chart, llm/cycle,
            xlabel={TTFT $\tau_0$ (s)},
            ...
            \addplot+[mark=none, smooth, domain=0:2.0, samples=200] {(1*120) / (x + 120*0.02)};
            \addlegendentry{$B{=}1$};

            \addplot+[mark=none, smooth, domain=0:2.0, samples=200] {(4*120) / (x + 120*0.02)};
            \addlegendentry{$B{=}4$};

            \addplot+[mark=none, smooth, domain=0:2.0, samples=200] {(8*120) / (x + 120*0.02)};
            \addlegendentry{$B{=}8$};

            \addplot+[mark=none, smooth, domain=0:2.0, samples=200] {(16*120) / (x + 120*0.02)};
            \addlegendentry{$B{=}16$};
        \end{axis}
    \end{tikzpicture}
    \end{llmfigbox}
...
Table~\ref{tab:ch02_pipeline_components} summarizes the key stages of an LLMOps pipeline, their core activities, and the artifacts produced at each step. Listing~\ref{lst:ch02_pipeline_config} shows a complete pipeline configuration that orchestrates all stages from data acquisition to monitoring.

\begin{codebox}{Complete LLMOps pipeline configuration orchestrating seven stages from data acquisition through monitoring, including model selection with fallback, LoRA fine-tuning, prompt versioning with A/B testing, evaluation gating, Kubernetes deployment with autoscaling, and feedback-loop alerting. This single declarative artifact codifies the entire operational lifecycle and enables reproducible, auditable deployments.}{lst:ch02_pipeline_config}
\begin{minted}{yaml}
...
\begin{codebox}{Chain-of-thought prompt template with few-shot examples demonstrating structured multi-step reasoning for conflict analysis. By decomposing complex queries into fact extraction, perspective comparison, implication assessment, and conclusion synthesis, this template steers the model toward more transparent and verifiable outputs in high-stakes domains.}{lst:ch02_prompt_cot}
\begin{minted}{yaml}
...
\begin{codebox}{Tool calling schema with semantic versioning, access-control validation, rate limiting, tenant isolation, and built-in monitoring thresholds. This schema-driven approach treats each tool as a governed API contract, enabling safe model-initiated invocations with auditability, backward compatibility guarantees, and operational alerting on error rates and latency.}{lst:ch02_tool_schema}
\begin{minted}{json}
{
...
\subsubsection{Tool Calling Failure Modes}\index{tool calling!failure modes}

While tool calling significantly extends the capabilities of LLM-powered systems, it introduces failure modes that do not exist in pure text-generation pipelines. Understanding these modes is essential for designing robust operational safeguards.

\begin{itemize}
  \item \textbf{Argument injection\index{tool calling!argument injection}:} The LLM crafts malicious or unexpected arguments to tool calls, analogous to SQL injection\index{SQL injection} in traditional software. For example, a model might embed shell metacharacters or path-traversal sequences in a file-retrieval tool argument, potentially accessing resources outside the intended scope. Mitigation requires strict input validation against the tool's JSON Schema, allowlisting of argument values where feasible, and sandboxing\index{sandboxing} tool execution environments so that even a successful injection cannot escalate privileges.

  \item \textbf{Schema-breaking changes\index{tool calling!schema breaking}:} When tool schemas evolve (e.g., a required parameter is renamed or a field type changes), models trained or prompted against older schemas may generate calls using the stale format. These calls may fail silently or produce incorrect results. In multi-model or multi-tenant environments, different model versions may expect different schema versions simultaneously, making backward compatibility critical.

  \item \textbf{Tool selection errors\index{tool calling!selection error}:} The model picks the wrong tool for the task---for example, calling a geocoding API when the user asked for a population statistic. Selection errors are particularly insidious because the tool call itself may succeed, returning syntactically valid but semantically irrelevant data that the model then weaves into a confident-sounding answer. Monitoring tool-selection accuracy as a first-class metric is essential.

  \item \textbf{Cascading failures\index{tool calling!cascading failure}:} In agentic pipelines where one tool's output feeds into another tool's input, a single tool failure can trigger a chain of downstream errors. A timeout in a database lookup, for instance, may cause a summarization tool to receive empty input, which in turn causes a citation tool to hallucinate references. Circuit breakers\index{circuit breaker}, retry budgets, and graceful degradation paths (e.g., returning a partial answer with an explicit ``information unavailable'' flag) help contain blast radius.

  \item \textbf{Hallucinated tool calls\index{tool calling!hallucinated}:} The model invents tool names that do not exist in the registered tool catalog. This occurs more frequently when the model is presented with a large tool catalog and must select from many options, or when the system prompt insufficiently constrains the model's generation. Strict tool-name validation at the orchestration layer---rejecting any call whose name does not match a registered tool---is a simple but essential safeguard.
\end{itemize}

\PitfallBox{Tool calling failures are often \emph{silent}: the system returns HTTP~200, the model produces fluent text, yet the underlying data is wrong because the wrong tool was called, the arguments were malformed, or a cascading failure corrupted intermediate results. Treat tool-call validation as a safety-critical control, not an optimization.}

\subsubsection{Tool Schema Versioning Best Practices}\index{tool calling!versioning}

As tool ecosystems grow, schema versioning becomes a critical operational concern. Unlike model weights (which are immutable once deployed), tool schemas evolve continuously as backend APIs change, new parameters are added, and deprecated features are removed.

\begin{itemize}
  \item \textbf{Semantic versioning\index{semantic versioning}:} Apply semantic versioning (MAJOR.MINOR.PATCH) to every tool schema. Increment MAJOR for breaking changes (removed fields, type changes), MINOR for backward-compatible additions, and PATCH for documentation or metadata updates. Include the version string in the schema itself (as shown in Listing~\ref{lst:ch02_tool_schema}) so that both the model and the orchestration layer can reference it.

  \item \textbf{Backward compatibility windows:} When introducing a breaking schema change, maintain the old version for a defined grace period (e.g., 2--4 weeks). During this window, the orchestration layer accepts calls in both old and new formats, translating old-format calls to the new backend. This prevents hard cutover failures when models have cached or fine-tuned against the previous schema.

  \item \textbf{Deprecation workflows\index{tool calling!deprecation}:} Mark deprecated tools or parameters with explicit metadata (\texttt{"deprecated": true, "sunset\_date": "2025-06-01"}) and emit warnings in logs when they are invoked. Track deprecation-warning rates in dashboards to gauge migration progress before removing the old schema.

  \item \textbf{Contract testing\index{contract testing}:} Maintain automated contract tests (referenced in the schema's \texttt{contract\_tests} field) that verify the tool's behavior against its declared inputs and outputs. Run these tests in CI/CD pipelines whenever the schema or the backend implementation changes. Contract tests serve as the operational guarantee that the schema accurately describes the tool's behavior.
\end{itemize}

\BestPracticeBox{Treat tool schemas as \emph{public APIs} with the same rigor applied to external-facing REST endpoints: version them semantically, test them contractually, deprecate them gracefully, and never introduce breaking changes without a migration path. In multi-model deployments, schema compatibility matrices---mapping which model versions support which schema versions---prevent silent integration failures.}

\subsubsection{Tool Calling vs.\ Retrieval: When to Use Which}\index{tool calling!vs.\ retrieval}

Tool calling and Retrieval-Augmented Generation (RAG)\index{RAG!vs.\ tool calling} both extend the model's knowledge beyond its training data, but they serve fundamentally different purposes and exhibit different operational characteristics. Choosing the right mechanism---or combining both---depends on the nature of the data, the required action, and the operational constraints.

\begin{itemize}
  \item \textbf{Structured data and actions} $\rightarrow$ \textbf{tool calling.} When the system needs to query a database with specific parameters, invoke an API, perform a calculation, or trigger a side effect (e.g., sending an email, creating a record), tool calling is the appropriate mechanism. The model formulates structured arguments that map directly to an API contract.

  \item \textbf{Unstructured knowledge} $\rightarrow$ \textbf{RAG.} When the system needs to ground its response in large corpora of documents, articles, policies, or manuals, retrieval pipelines (embedding-based search, reranking, chunk assembly) are more appropriate. RAG excels at surfacing relevant passages from heterogeneous text sources.

  \item \textbf{Hybrid patterns:} Many production systems combine both mechanisms. For example, \ishtar{} uses RAG to retrieve relevant conflict reports and background context, then uses tool calling to perform entity resolution, timeline extraction, and geocoding on the retrieved information. The key operational principle is that retrieval provides \emph{knowledge} while tool calling provides \emph{capability}---the ability to act on or transform that knowledge.
\end{itemize}

Table~\ref{tab:ch02_tool_vs_rag} provides decision criteria for choosing between tool calling and RAG, comparing their operational characteristics across data structure, freshness, failure modes, and security considerations.

\begin{table}[t]
\centering
\small
\caption{Decision criteria for choosing between tool calling and RAG. In practice, many production LLM systems combine both: RAG for knowledge grounding and tool calling for structured actions and computations.}
\label{tab:ch02_tool_vs_rag}
\setlength{\tabcolsep}{5pt}
\renewcommand{\arraystretch}{1.3}
\rowcolors{2}{gray!8}{white}
...
\toprule
\rowcolor{gray!15}
\textbf{Criterion} & \textbf{Tool Calling} & \textbf{RAG} \\
\midrule
Data structure & Structured (databases, APIs, schemas) & Unstructured (documents, articles, policies) \\
\addlinespace[2pt]
Action vs.\ knowledge & Performs actions or computations & Provides contextual knowledge \\
\addlinespace[2pt]
Freshness & Real-time (queries live systems) & Depends on index refresh cadence \\
\addlinespace[2pt]
Failure mode & Schema mismatch, argument injection, timeouts & Retrieval miss, irrelevant chunks, stale index \\
\addlinespace[2pt]
Latency profile & Varies by backend API (ms to seconds) & Embedding search + reranking (typically 50--200\,ms) \\
\addlinespace[2pt]
Security surface & API credentials, sandboxing, rate limits & Index access control, PII in chunks \\
\addlinespace[2pt]
Versioning unit & Tool schema version & Index version + embedding model version \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{codebox}{Comprehensive evaluation metrics configuration defining weighted thresholds across factual consistency, coherence, style adherence, safety, latency, and cost dimensions with slice-level analysis and LLM-as-judge scoring. This multi-dimensional evaluation framework enforces critical-metric gates for safety and factuality while aggregating non-critical scores into an overall quality threshold for automated promotion decisions.}{lst:ch02_evaluation_config}
\begin{minted}{yaml}
...
using trained judges and prediction-powered inference to reduce annotation costs \cite{ragas_eacl2024,ares}.

\subsection{Human Feedback and Alignment}
As language models become more powerful, ensuring they align with human intentions and values is paramount. Two key approaches in this domain are Reinforcement Learning from Human Feedback (RLHF)~\cite{ouyang2022instructgpt} and Constitutional AI~\cite{bai2022constitutional}.
...
in CI/CD and incident response \cite{OWASP2024LLMTop10}.
...
Let us revisit parameter memory with a quick table that aligns with Eq.~\ref{eq:param-mem} for some representative model scales (all assuming FP16 weights). Table~\ref{tab:ch02_param_memory} provides concrete memory requirements for common model sizes, helping teams assess deployment feasibility and infrastructure needs:
...
\renewcommand{\arraystretch}{1.3}
\rowcolors{2}{gray!8}{white}
\begin{tabularx}{\linewidth}{@{}X S[table-format=1.2e2] S[table-format=3.0]@{}}
\toprule
\rowcolor{gray!15}
\textbf{Model} & {\textbf{Parameters $P$}} & {\textbf{$M_{\text{params}}$ (GB, approx.)}} \\
\midrule
7B   & 7e9   & {$\approx 14$} \\
13B  & 1.3e10 & {$\approx 26$} \\
70B  & 7.0e10 & {$\approx 140$} \\
175B & 1.75e11 & {$\approx 350$} \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{itemize}
    \item \textbf{LangChain~\cite{LangChain2023Framework}, LlamaIndex~\cite{llamaindex_rag_docs}:} Frameworks for orchestrating prompts, retrieval, and external tool usage in LLM applications.  
    ...
    \item \textbf{MLflow~\cite{mlflow}, Weights \& Biases (W\&B)~\cite{wandb}:} Tools for experiment tracking and model management.  
    ...
    Examples include Pinecone~\cite{pinecone_hybrid_docs} and Weaviate~\cite{weaviate_hybrid_docs} (managed services) and FAISS~\cite{Johnson2019FAISS} (an open-source library from Facebook). Vector databases are optimized for storing and retrieving high-dimensional embeddings (e.g., 768-D from BERT or 4096-D from GPT-family models). They often support:
    \begin{itemize}
    ...
    Examples include \texttt{vLLM}~\cite{Kwon2023vLLM}, Hugging Face TGI (Text Generation Inference)~\cite{tgi-whitepaper}, and NVIDIA TensorRT-LLM~\cite{tensorrt-llm}:
    \begin{itemize}
    ...
    \end{itemize}
    Specialized LLM observability tools are emerging, such as \texttt{LangFuse}~\cite{langfuse_docs}, which traces LLM calls and tool usage, providing insights into prompt chains, response lengths, and anomalies. Other options include OpenAI \emph{evals}~\cite{openai_evals} and LangSmith by LangChain.  

...
\begin{lstlisting}[
  float=htbp,
  style=springerfloat,
  caption={Complete LLMOps pipeline configuration orchestrating seven stages from data acquisition through monitoring, including model selection with fallback, LoRA fine-tuning, prompt versioning with A/B testing, evaluation gating, Kubernetes deployment with autoscaling, and feedback-loop alerting. This single declarative artifact codifies the entire operational lifecycle and enables reproducible, auditable deployments.},
...
\begin{lstlisting}[
  float=htbp,
  style=springerfloat,
  caption={Chain-of-thought prompt template with few-shot examples demonstrating structured multi-step reasoning for conflict analysis. By decomposing complex queries into fact extraction, perspective comparison, implication assessment, and conclusion synthesis, this template steers the model toward more transparent and verifiable outputs in high-stakes domains.},
...
\begin{lstlisting}[
  float=htbp,
  style=springerfloat,
  caption={Tool calling schema with semantic versioning, access-control validation, rate limiting, tenant isolation, and built-in monitoring thresholds. This schema-driven approach treats each tool as a governed API contract, enabling safe model-initiated invocations with auditability, backward compatibility guarantees, and operational alerting on error rates and latency.},
...
\begin{lstlisting}[
  float=htbp,
  style=springerfloat,
  caption={Comprehensive evaluation metrics configuration defining weighted thresholds across factual consistency, coherence, style adherence, safety, latency, and cost dimensions with slice-level analysis and LLM-as-judge scoring. This multi-dimensional evaluation framework enforces critical-metric gates for safety and factuality while aggregating non-critical scores into an overall quality threshold for automated promotion decisions.},

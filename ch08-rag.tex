...
\midrule
Frequently changing knowledge & RAG & Corpus updates propagate without retraining; freshness is a first-class property. \\
Stable domain knowledge + style adaptation & Fine-tuning (+ optional RAG) & Model internalizes domain reasoning; RAG supplements with specific facts. \\
Precise structured queries (counts, joins, lookups) & Knowledge graph / SQL & Exact answers from structured data; no retrieval ambiguity. \\
Mixed query types & Hybrid: RAG for open-ended + structured retrieval for precise queries & Router classifies query type and selects the appropriate backend. \\
\bottomrule
\end{tabularx}
\end{table}

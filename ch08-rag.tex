...
\begin{lstlisting}[
  float=t,
  style=springerfloat,
  caption={Production vector index configuration defining the embedding dimension, similarity metric, metadata schema for filtering, and role-based access control. This YAML specification ensures that retrieval respects both relevance and security clearance constraints.},
...
\begin{lstlisting}[
  float=htbp,
  style=springerfloat,
  caption={End-to-end RAG pipeline configuration integrating document ingestion, chunking, embedding generation, vector storage, hybrid retrieval, LLM generation with citation support, and post-processing safety filters. This unified YAML specification connects all pipeline stages and cross-references component-level configurations.},
...
\begin{lstlisting}[
  float=htbp,
  style=springerfloat,
  caption={Hybrid retrieval configuration combining dense vector search and sparse keyword search with query expansion, cross-encoder reranking, deduplication, and access control enforcement. This configuration demonstrates how multiple retrieval signals are fused and filtered to maximize both precision and recall.},

...
% Pipeline stages
\node[block, fill=llmblue!15] (q) {\textbf{User query}};
\node[block, fill=llmgreen!15, right=of q] (qp) {\textbf{Query processing}\\[2pt]\footnotesize rewrite, filters};
\node[block, fill=llmorange!15, right=of qp] (ret) {\textbf{Retrieval}\\[2pt]\footnotesize dense/sparse/hybrid};
\node[block, fill=llmpurple!15, right=of ret] (rr) {\textbf{Rerank \& filter}\\[2pt]\footnotesize cross-encoder, RRF};
\node[block, fill=llmpurple!15, right=of rr] (pack) {\textbf{Context packer}\\[2pt]\footnotesize dedupe, trim, cite};
\node[block, fill=llmorange!15, right=of pack] (gen) {\textbf{LLM generation}};
\node[block, fill=llmgreen!15, below=of gen, node distance=18mm] (ans) {\textbf{Answer + citations}};
...
\textbf{\large Production vector-index configuration surface}\\[4mm]
\begin{minipage}[t]{0.95\linewidth}
\colorbox{llmblue!10}{\parbox{0.98\linewidth}{\vspace{2mm}\textbf{\textcolor{llmblue!90!black}{Embedding space:}} \texttt{dimension}\,=\,384 (matches the embedding model, e.g.\ MiniLM-L6-v2); \texttt{metric}\,=\,\texttt{cosine} for normalized embeddings (alternatives: \texttt{euclidean}, \texttt{dotproduct}); changes here force a full reindex \cite{Thakur2021BEIR}.\vspace{2mm}}}\\[2mm]
\colorbox{llmpurple!10}{\parbox{0.98\linewidth}{\vspace{2mm}\textbf{\textcolor{llmpurple!90!black}{Provider \& topology:}} \texttt{pinecone} (or \texttt{weaviate}, \texttt{qdrant}, \texttt{faiss\_on\_disk}); 2 replicas, 1 shard, region-pinned to the inference fleet; \texttt{p1.x1} pod for the article corpus.\vspace{2mm}}}\\[2mm]
\colorbox{llmgreen!10}{\parbox{0.98\linewidth}{\vspace{2mm}\textbf{\textcolor{llmgreen!90!black}{Indexing strategy:}} hourly upserts in batches of 100; rebuild the index when the metadata schema changes; treat backfills and news surges as a separate operational regime from steady-state writes.\vspace{2mm}}}\\[2mm]
\colorbox{llmorange!10}{\parbox{0.98\linewidth}{\vspace{2mm}\textbf{\textcolor{llmorange!90!black}{Filterable metadata schema:}} indexed columns for \texttt{document\_id}, \texttt{published\_date}, \texttt{source\_outlet}, \texttt{category}, and \texttt{security\_clearance} (integer, default 0\,=\,public); filters are part of the query plan, not a post-processing step.\vspace{2mm}}}\\[2mm]
\colorbox{llmgray!10}{\parbox{0.98\linewidth}{\vspace{2mm}\textbf{\textcolor{llmgray!90!black}{Access control on retrieval:}} default clearance 0; map the calling user's \texttt{user.security\_clearance} attribute onto the metadata predicate so that ANN search only returns documents the requester is permitted to see.\vspace{2mm}}}
\end{minipage}
};
\end{tikzpicture}
\end{llmfigbox}
...
\textbf{\large End-to-end RAG pipeline contracts}\\[4mm]
\begin{minipage}[t]{0.95\linewidth}
\colorbox{llmblue!10}{\parbox{0.98\linewidth}{\vspace{2mm}\textbf{\textcolor{llmblue!90!black}{Ingestion \& preprocessing:}} two connectors (news API every 15\,min, internal-reports filesystem nightly); preprocess with BeautifulSoup HTML-to-text, Presidio PII redaction (\texttt{PERSON}, \texttt{LOCATION}, \texttt{PHONE\_NUMBER}), and FastText language detection.\vspace{2mm}}}\\[2mm]
\colorbox{llmgreen!10}{\parbox{0.98\linewidth}{\vspace{2mm}\textbf{\textcolor{llmgreen!90!black}{Indexing path (chunk $\rightarrow$ embed $\rightarrow$ store):}} chunking per Figure~\ref{fig:ch08_chunking_callout}; embeddings from \texttt{all-MiniLM-L6-v2} on CUDA in batches of 64 with the embedding cache enabled; vector store per Figure~\ref{fig:ch08_vector_index_callout}.\vspace{2mm}}}\\[2mm]
\colorbox{llmorange!10}{\parbox{0.98\linewidth}{\vspace{2mm}\textbf{\textcolor{llmorange!90!black}{Retrieval \& generation:}} retrieval per Figure~\ref{fig:ch08_retrieval_callout}; \texttt{llama-3.1-8b-instruct} via internal HTTP endpoint; \texttt{temperature\,=\,0.7}, \texttt{top\_p\,=\,0.9}, \texttt{max\_new\_tokens\,=\,512}; \texttt{news\_summary\_with\_citations\_v1} prompt; truncate-oldest context strategy; markdown-link citations.\vspace{2mm}}}\\[2mm]
\colorbox{llmpurple!10}{\parbox{0.98\linewidth}{\vspace{2mm}\textbf{\textcolor{llmpurple!90!black}{Safety \& post-processing:}} hallucination detector flags low-faithfulness outputs for human review; Perspective-API toxicity filter refuses to ship responses above threshold (a hard gate, not an advisory).\vspace{2mm}}}\\[2mm]
\colorbox{llmgray!10}{\parbox{0.98\linewidth}{\vspace{2mm}\textbf{\textcolor{llmgray!90!black}{Monitoring \& evaluation hooks:}} emit telemetry against the Chapter~\ref{ch:monitoring} schema; trigger the Chapter~\ref{ch:cicd} evaluation harness on every release; route alerts through the rules in Section~\ref{sec:alerts-incident-response-and-resilience}.\vspace{2mm}}}
\end{minipage}
};
\end{tikzpicture}
\end{llmfigbox}
...
% Nodes
\node[block, fill=llmblue!15] (sources) {\textbf{Data Sources}\\[2pt]\footnotesize Databases, APIs, Websites,\\RSS, Social Media};
\node[block, fill=llmgreen!15, below=of sources] (preprocess) {\textbf{Preprocessing \& Cleaning}\\[2pt]\footnotesize Format conversion, noise removal,\\translation};
\node[block, fill=llmorange!15, below=of preprocess] (chunk) {\textbf{Chunking Strategy}\\[2pt]\footnotesize Paragraphs, sections,\\token windows};
\node[block, fill=llmpurple!15, below=of chunk] (dedup) {\textbf{Deduplication \& Filtering}\\[2pt]\footnotesize Remove duplicates,\\drop irrelevant data};
\node[block, fill=llmpurple!15, below=of dedup] (metadata) {\textbf{Metadata Enrichment}\\[2pt]\footnotesize Timestamps, geotags,\\source reliability};
\node[block, fill=llmorange!15, below=of metadata] (vector) {\textbf{Vector Store}\\[2pt]\footnotesize FAISS, Pinecone,\\Weaviate, Milvus};
...
\textbf{\large Type-aware chunking strategy}\\[4mm]
\begin{minipage}[t]{0.95\linewidth}
\colorbox{llmblue!10}{\parbox{0.98\linewidth}{\vspace{2mm}\textbf{\textcolor{llmblue!90!black}{Default (recursive text splitter):}} 512-token chunks with 128-token overlap; prioritized separators (paragraph $\rightarrow$ line $\rightarrow$ sentence $\rightarrow$ word) so splits land on natural boundaries before falling back to fixed size.\vspace{2mm}}}\\[2mm]
\colorbox{llmgreen!10}{\parbox{0.98\linewidth}{\vspace{2mm}\textbf{\textcolor{llmgreen!90!black}{Reports (structural splitter):}} split on Markdown headings (\texttt{\#}, \texttt{\#\#}, \texttt{\#\#\#}); 100--1024 token bounds; preserves section semantics for navigational queries.\vspace{2mm}}}\\[2mm]
\colorbox{llmpurple!10}{\parbox{0.98\linewidth}{\vspace{2mm}\textbf{\textcolor{llmpurple!90!black}{Legal documents (sentence splitter):}} 5 sentences per chunk with 1-sentence overlap; carry \texttt{case\_id} and \texttt{paragraph\_id} metadata so citations resolve to the exact paragraph.\vspace{2mm}}}\\[2mm]
\colorbox{llmorange!10}{\parbox{0.98\linewidth}{\vspace{2mm}\textbf{\textcolor{llmorange!90!black}{Social posts (fixed-size):}} 128-token chunks with no overlap; tag \texttt{source\,=\,social\_media}, \texttt{platform\,=\,twitter} so retrieval can downweight or filter at query time.\vspace{2mm}}}\\[2mm]
\colorbox{llmgray!10}{\parbox{0.98\linewidth}{\vspace{2mm}\textbf{\textcolor{llmgray!90!black}{Global metadata \& post-filters:}} stamp every chunk with \texttt{source\_system}, \texttt{processing\_date}, and \texttt{embedding\_model\_version}; drop chunks under 50 tokens; strip boilerplate (advertisements, copyright lines) via regex.\vspace{2mm}}}
\end{minipage}
};
\end{tikzpicture}
\end{llmfigbox}
...
\textbf{\large Hybrid retrieval pipeline stages}\\[4mm]
\begin{minipage}[t]{0.95\linewidth}
\colorbox{llmblue!10}{\parbox{0.98\linewidth}{\vspace{2mm}\textbf{\textcolor{llmblue!90!black}{Query processing:}} LLM-rewrite query expansion produces 3 variants via \texttt{llama-3.1-8b-query-rewriter}; HyDE (hypothetical document embedding) compression available but off by default to keep latency low on initial retrieval.\vspace{2mm}}}\\[2mm]
\colorbox{llmgreen!10}{\parbox{0.98\linewidth}{\vspace{2mm}\textbf{\textcolor{llmgreen!90!black}{Dense retriever (weight 0.7):}} top-10 from \texttt{ishtar-news-vector-index} with metadata filters (\texttt{published\_date $\geq$ now-30d}, \texttt{category $\in$ \{politics, conflict\}}); pushes the filter into the query plan, not after the fact \cite{Lin2021SPLADE}.\vspace{2mm}}}\\[2mm]
\colorbox{llmpurple!10}{\parbox{0.98\linewidth}{\vspace{2mm}\textbf{\textcolor{llmpurple!90!black}{Sparse retriever (weight 0.3):}} top-20 from Elasticsearch on \texttt{ishtar-news-keywords}; complements the dense path on rare named entities and numeric tokens that embeddings tend to blur \cite{Thakur2021BEIR}.\vspace{2mm}}}\\[2mm]
\colorbox{llmorange!10}{\parbox{0.98\linewidth}{\vspace{2mm}\textbf{\textcolor{llmorange!90!black}{Cross-encoder reranker:}} \texttt{cross-encoder/ms-marco-MiniLM-L-6-v2} reranks the merged candidate pool to a final top-5 in batches of 32; the precision step that converts a recall-heavy candidate list into a context-window-ready set \cite{khattab2020colbert,Zhan2021ColBERTv2,cormack2009rrf}.\vspace{2mm}}}\\[2mm]
\colorbox{llmgray!10}{\parbox{0.98\linewidth}{\vspace{2mm}\textbf{\textcolor{llmgray!90!black}{Post-retrieval filters \& observability:}} embedding-similarity dedup at threshold 0.95; access control through OPA (\texttt{document\_access\_policy\_v1}); log full retrieved-document content (post-redaction) for offline evaluation and incident triage.\vspace{2mm}}}
\end{minipage}
};
\end{tikzpicture}
\end{llmfigbox}

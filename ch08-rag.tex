...
\begin{codebox}{Production vector index configuration defining the embedding dimension, similarity metric, metadata schema for filtering, and role-based access control. This YAML specification ensures that retrieval respects both relevance and security clearance constraints.}{lst:ch08_vector_index_config}
\begin{minted}{yaml}
...
\begin{codebox}{End-to-end RAG pipeline configuration integrating document ingestion, chunking, embedding generation, vector storage, hybrid retrieval, LLM generation with citation support, and post-processing safety filters. This unified YAML specification connects all pipeline stages and cross-references component-level configurations.}{lst:ch08_rag_pipeline_config}
\begin{minted}{yaml}
...
\begin{codebox}{Chunking strategy configuration supporting multiple document types with type-specific splitting strategies, overlap settings, and post-chunking filters. Proper chunking directly impacts retrieval quality by ensuring semantically coherent units that match the embedding model's effective context window.}{lst:ch08_chunking_config}
\begin{minted}{yaml}
...
\begin{codebox}{Hybrid retrieval configuration combining dense vector search and sparse keyword search with query expansion, cross-encoder reranking, deduplication, and access control enforcement. This configuration demonstrates how multiple retrieval signals are fused and filtered to maximize both precision and recall.}{lst:ch08_retrieval_config}
\begin{minted}{yaml}

...
Hallucinations are often a byproduct of the model optimizing for plausibility rather than truth when evidence is missing. RAG shifts the task from open-book recall to \emph{grounded synthesis}: the model is given retrieved passages and instructed to answer using that evidence \cite{Lewis2020RAG,Izacard2020Fusion}. This does not eliminate hallucinations outright, but it makes them easier to detect (via attribution) and easier to reduce (via retrieval and prompt improvements). In practice, RAG evaluation frameworks explicitly measure \emph{faithfulness}—the degree to which an answer is supported by the retrieved context—alongside answer relevance \cite{ragas_eacl2024}.
...
\paragraph{Fine-tuning as an alternative.}\index{fine-tuning!vs RAG}
When domain knowledge is stable, deeply specialized, and needs to be integrated into the model's reasoning style (not just recalled as facts), fine-tuning may outperform RAG. Fine-tuned models internalize domain patterns---terminology, reasoning heuristics, output formatting---without requiring retrieval at inference time. This eliminates retrieval latency and the risk of retrieval misses. However, fine-tuning requires curated training data, is expensive to update, and cannot easily incorporate knowledge that changes daily. For \ishtar{}, fine-tuning is appropriate for learning journalistic writing style and domain-specific reasoning patterns, while RAG handles the fast-changing factual layer.

\paragraph{Structured knowledge bases.}\index{knowledge graph}
When queries demand precise, structured answers---exact counts, relationships between entities, or data that lives naturally in tables and graphs---RAG over unstructured text may be suboptimal. Knowledge graphs and SQL databases can answer ``How many incidents occurred in Region X between March 1 and March 15?'' with exact precision, while RAG would need to retrieve and parse multiple passages hoping the answer is stated explicitly. Hybrid architectures that route structured queries to databases and open-ended queries to RAG often outperform either approach alone.

\paragraph{Decision framework: choosing the right approach.}
The choice between RAG, fine-tuning, structured retrieval, and hybrid approaches depends on the nature of the knowledge, the query patterns, and operational constraints. Table~\ref{tab:ch08_rag_alternatives} provides a decision framework to guide this choice:

\begin{table}[t]
\centering
\small
\caption{Decision framework for choosing between RAG, fine-tuning, and structured retrieval. The right approach depends on knowledge volatility, query precision requirements, and operational constraints. Most production systems benefit from a hybrid that combines multiple approaches.}
\label{tab:ch08_rag_alternatives}
\setlength{\tabcolsep}{6pt}
\renewcommand{\arraystretch}{1.3}
\rowcolors{2}{gray!8}{white}
\begin{tabularx}{0.95\linewidth}{@{}>{\raggedright\arraybackslash}p{3.2cm}>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X@{}}
\toprule
\rowcolor{gray!15}
\textbf{Scenario} & \textbf{Recommended Approach} & \textbf{Rationale} \\
\midrule
\rowcolor{blue!8}
Frequently changing knowledge & \textbf{RAG} & Corpus updates propagate without retraining; freshness is a first-class property. \\
\rowcolor{green!10}
Stable domain knowledge + style adaptation & \textbf{Fine-tuning} (+ optional RAG) & Model internalizes domain reasoning; RAG supplements with specific facts. \\
\rowcolor{orange!10}
Precise structured queries (counts, joins, lookups) & \textbf{Knowledge graph / SQL} & Exact answers from structured data; no retrieval ambiguity. \\
\rowcolor{cyan!10}
Mixed query types & \textbf{Hybrid}: RAG for open-ended + structured retrieval for precise queries & Router classifies query type and selects the appropriate backend. \\
\bottomrule
\end{tabularx}
\end{table}

\noindent For \ishtar{}, the production system uses a hybrid approach: RAG handles open-ended questions grounded in recent reporting, a fine-tuned model provides the journalistic reasoning style, and structured queries against incident databases answer precise quantitative questions. This layered design ensures each knowledge access pattern is served by the most appropriate mechanism.
...
A production RAG system\index{RAG!system} consists of \emph{offline} components (ingestion\index{ingestion}, cleaning\index{cleaning}, chunking\index{chunking}, embedding\index{embedding}, indexing\index{indexing}) and \emph{online} components (query processing\index{query processing}, retrieval\index{retrieval}, reranking\index{reranking}, context construction\index{context!construction}, and generation\index{generation})~\cite{Pinecone2023RAG}. As shown in Figure~\ref{fig:ch08_rag_querytime}, the online query-time loop orchestrates these components to produce grounded, verifiable answers.

\begin{figure}[tb]
\centering
\begin{llmfigbox}
% Local styles built on the book-wide llm/* contract (macros.tex).
% Color carries the role: blue = user/data, green = controller/answer,
% orange = retrieval/generation steps, purple = ranking/packing.
\tikzset{
  stage/.style ={llm/box, minimum width=26mm, font=\footnotesize\bfseries},
  data/.style  ={llm/box/blue,   minimum width=26mm, font=\footnotesize\bfseries},
  ctrl/.style  ={llm/box/green,  minimum width=26mm, font=\footnotesize\bfseries},
  proc/.style  ={llm/box/orange, minimum width=26mm, font=\footnotesize\bfseries},
  pack/.style  ={llm/box/purple, minimum width=26mm, font=\footnotesize\bfseries},
  flow/.style  ={llm/arrow},
}
\begin{tikzpicture}[node distance=6mm and 5mm]
  \node[data]                         (q)    {User query};
  \node[ctrl, right=of q]             (qp)   {Query processing\\[1pt]\footnotesize rewrite, filters};
  \node[proc, right=of qp]            (ret)  {Retrieval\\[1pt]\footnotesize dense/sparse/hybrid};
  \node[pack, right=of ret]           (rr)   {Rerank \& filter\\[1pt]\footnotesize cross-encoder, RRF};
  \node[pack, right=of rr]            (cp)   {Context packer\\[1pt]\footnotesize dedupe, trim, cite};
  \node[proc, right=of cp]            (gen)  {LLM generation};
  \node[ctrl, below=8mm of gen]       (ans)  {Answer + citations};
...
\rowcolors{2}{gray!8}{white}
\begin{xltabular}{\linewidth}{@{}>{\raggedright\arraybackslash}p{2.8cm}>{\raggedright\arraybackslash}p{1.6cm}>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X@{}}
...
Figure~\ref{fig:ch08_vector_index_callout} summarizes a production vector index as four coupled decisions: which embedding space to use, how to shard and replicate it, what metadata to make filterable, and how to enforce access control on retrieval.

\begin{figure}[t]
\centering
\begin{llmfigbox}
% Local styles built on the book-wide llm/* contract (macros.tex).
% Color carries the role: blue = embedding, purple = topology, green = ops,
% orange = filter schema, gray = neutral access control.
\tikzset{
  card/.style={text width=0.90\linewidth, align=left, font=\footnotesize},
}
\begin{tikzpicture}[node distance=2mm]
  \node[font=\bfseries\large] (title) {Production vector-index configuration surface};
  \node[llm/box/blue,   card, below=4mm of title.south, anchor=north] (r1)
       {\textbf{Embedding space:} \texttt{dimension}\,=\,384 (matches the embedding model, e.g.\ MiniLM-L6-v2); \texttt{metric}\,=\,\texttt{cosine} for normalized embeddings (alternatives: \texttt{euclidean}, \texttt{dotproduct}); changes here force a full reindex \cite{Thakur2021BEIR}.};
  \node[llm/box/purple, card, below=of r1] (r2)
       {\textbf{Provider \& topology:} \texttt{pinecone} (or \texttt{weaviate}, \texttt{qdrant}, \texttt{faiss\_on\_disk}); 2 replicas, 1 shard, region-pinned to the inference fleet; \texttt{p1.x1} pod for the article corpus.};
  \node[llm/box/green,  card, below=of r2] (r3)
       {\textbf{Indexing strategy:} hourly upserts in batches of 100; rebuild the index when the metadata schema changes; treat backfills and news surges as a separate operational regime from steady-state writes.};
  \node[llm/box/orange, card, below=of r3] (r4)
       {\textbf{Filterable metadata schema:} indexed columns for \texttt{document\_id}, \texttt{published\_date}, \texttt{source\_outlet}, \texttt{category}, and \texttt{security\_clearance} (integer, default 0\,=\,public); filters are part of the query plan, not a post-processing step.};
  \node[llm/box/gray,   card, below=of r4] (r5)
       {\textbf{Access control on retrieval:} default clearance 0; map the calling user's \texttt{user.security\_clearance} attribute onto the metadata predicate so that ANN search only returns documents the requester is permitted to see.};
\end{tikzpicture}
\end{llmfigbox}
\caption{Production vector-index configuration as five coupled surfaces. Embedding-space choices (dimension and metric) bind the index to the embedding model; topology (provider, replicas, shards) governs availability and tail latency; indexing strategy controls update behavior; metadata schema determines which filters are cheap; access control turns the metadata schema into a security boundary by pushing clearance into the query predicate rather than filtering after the fact.}
\label{fig:ch08_vector_index_callout}
\end{figure}
...
Figure~\ref{fig:ch08_rag_pipeline_callout} summarizes the end-to-end RAG pipeline for Ishtar AI as five contracts. Each contract connects to a more detailed figure or chapter where the corresponding component is fully specified, so the pipeline acts as a table of contents for the RAG stack.

\begin{figure}[t]
\centering
\begin{llmfigbox}
% Local styles built on the book-wide llm/* contract (macros.tex).
% Each row is an llm/box/<color> tile so the contract palette and stroke
% weights match the rest of the book; color carries the role
% (blue = ingestion, green = indexing, orange = retrieval/generation,
% purple = safety, gray = neutral observability).
\tikzset{
  card/.style={text width=0.90\linewidth, align=left, font=\footnotesize},
}
\begin{tikzpicture}[node distance=2mm]
  \node[font=\bfseries\large] (title) {End-to-end RAG pipeline contracts};
  \node[llm/box/blue,   card, below=4mm of title.south, anchor=north] (r1)
       {\textbf{Ingestion \& preprocessing:} two connectors (news API every 15\,min, internal-reports filesystem nightly); preprocess with BeautifulSoup HTML-to-text, Presidio PII redaction (\texttt{PERSON}, \texttt{LOCATION}, \texttt{PHONE\_NUMBER}), and FastText language detection.};
  \node[llm/box/green,  card, below=of r1] (r2)
       {\textbf{Indexing path (chunk $\rightarrow$ embed $\rightarrow$ store):} chunking per Figure~\ref{fig:ch08_chunking_callout}; embeddings from \texttt{all-MiniLM-L6-v2} on CUDA in batches of 64 with the embedding cache enabled; vector store per Figure~\ref{fig:ch08_vector_index_callout}.};
  \node[llm/box/orange, card, below=of r2] (r3)
       {\textbf{Retrieval \& generation:} retrieval per Figure~\ref{fig:ch08_retrieval_callout}; \texttt{llama-3.1-8b-instruct} via internal HTTP endpoint; \texttt{temperature\,=\,0.7}, \texttt{top\_p\,=\,0.9}, \texttt{max\_new\_tokens\,=\,512}; \texttt{news\_summary\_with\_citations\_v1} prompt; truncate-oldest context strategy; markdown-link citations.};
  \node[llm/box/purple, card, below=of r3] (r4)
       {\textbf{Safety \& post-processing:} hallucination detector flags low-faithfulness outputs for human review; Perspective-API toxicity filter refuses to ship responses above threshold (a hard gate, not an advisory).};
  \node[llm/box/gray,   card, below=of r4] (r5)
       {\textbf{Monitoring \& evaluation hooks:} emit telemetry against the Chapter~\ref{ch:monitoring} schema; trigger the Chapter~\ref{ch:cicd} evaluation harness on every release; route alerts through the rules in Section~\ref{sec:alerts-incident-response-and-resilience}.};
\end{tikzpicture}
\end{llmfigbox}
\caption{End-to-end RAG pipeline expressed as five contracts: ingestion, indexing path, retrieval and generation, safety, and observability. Each contract is the smallest interface that the next stage downstream needs in order to do its job; the indexing path and the retrieval-and-generation contract delegate to component-level figures (Figures~\ref{fig:ch08_chunking_callout}, \ref{fig:ch08_vector_index_callout}, and \ref{fig:ch08_retrieval_callout}) so the pipeline view stays focused on flow and policy rather than implementation detail.}
\label{fig:ch08_rag_pipeline_callout}
\end{figure}
...
\subsubsection{When to Use Multi-Stage vs.\ Single-Stage Retrieval}\index{RAG!multi-hop}
The choice between single-stage and multi-stage retrieval is an engineering decision driven by query complexity, knowledge distribution, and latency constraints.

\paragraph{Decision criteria.}
Single-stage retrieval suffices when: (i) answers are localized in one or two passages, (ii) the query is self-contained (no implicit sub-questions), and (iii) latency budgets are tight (single-stage adds no retrieval overhead). Multi-stage retrieval is justified when: (i) answering requires synthesizing evidence from multiple, non-adjacent documents, (ii) the initial query is underspecified and needs decomposition, or (iii) first-pass retrieval has low precision and iterative refinement improves the evidence set.

\paragraph{Multi-hop patterns.}
Three common patterns for multi-hop retrieval have emerged in practice:
\begin{itemize}[leftmargin=1.5em, itemsep=2pt]
  \item \textbf{Iterative retrieval:} Retrieve, generate a partial answer, identify knowledge gaps, retrieve again to fill gaps, and repeat. This pattern works well when the model can self-assess its own uncertainty. The system converges when no new relevant evidence is found or a maximum iteration count is reached.
  \item \textbf{Query decomposition:} Break a complex question into atomic sub-questions (e.g., ``What aid reached Region X?'' $\rightarrow$ ``What organizations are active in Region X?'' + ``How much aid was delivered this week?''), retrieve evidence for each sub-question independently, then synthesize. This pattern is especially effective for comparison and aggregation queries.
  \item \textbf{Chain-of-retrieval:} Use the output of one retrieval step as input to the next. For example, retrieve documents about an event, extract entity names, then retrieve background information about those entities. This pattern supports deep investigative queries where context builds incrementally.
\end{itemize}

\paragraph{Trade-offs: latency vs.\ recall.}
Each retrieval hop adds 50--200\,ms of latency (embedding + ANN search + reranking) plus the cost of any intermediate LLM calls for query reformulation. For a two-hop system, this typically adds 200--500\,ms to end-to-end latency. Three or more hops can push latency beyond interactive thresholds (>2\,s) unless aggressive caching and parallelization are applied. However, multi-hop retrieval can improve recall by 15--40\% on complex queries compared to single-stage, because each hop surfaces evidence that the initial query alone would miss. The optimal number of hops depends on the workload: for \ishtar{}, a two-stage approach (initial retrieval + targeted follow-up for entity-specific details) provides the best balance of quality and responsiveness.

\PitfallBox{Multi-hop retrieval can amplify errors: if the first retrieval step returns irrelevant context, subsequent hops may ``drift'' further from the correct evidence. Always validate intermediate retrieval quality and implement early termination when retrieval confidence drops below a threshold.}

\subsection{Agent-Enhanced RAG}
Agent-enhanced RAG treats retrieval as one tool among many. An agent decides when to retrieve, how to reformulate queries, and whether additional tools (web search, calculators, structured DB queries) are needed~\cite{Yao2022ReAct,Schick2023Toolformer}. This can improve flexibility, but increases the risk surface (prompt injection, tool misuse) and requires stronger guardrails and observability.
...
\begin{llmfigbox}
% Local styles built on the book-wide llm/* contract (macros.tex).
% Compact vertical pipeline: each stage uses llm/box defaults (9mm minimum
% height, small font), with 5mm node distance to keep the figure within the
% Springer textblock without scaling.
% Color carries the role: blue = inbound data, green = preprocessing,
% orange = chunking/storage, purple = quality/governance.
\tikzset{
  data/.style ={llm/box/blue,   minimum width=42mm, font=\footnotesize\bfseries},
  prep/.style ={llm/box/green,  minimum width=42mm, font=\footnotesize\bfseries},
  proc/.style ={llm/box/orange, minimum width=42mm, font=\footnotesize\bfseries},
  qual/.style ={llm/box/purple, minimum width=42mm, font=\footnotesize\bfseries},
  flow/.style ={llm/arrow},
}
\begin{tikzpicture}[node distance=5mm]
  \node[data]                          (sources)    {Data sources\\[1pt]\footnotesize databases, APIs, RSS, social};
  \node[prep, below=of sources]        (preprocess) {Preprocessing \& cleaning\\[1pt]\footnotesize format, noise removal, translate};
  \node[proc, below=of preprocess]     (chunk)      {Chunking strategy\\[1pt]\footnotesize paragraphs, sections, token windows};
  \node[qual, below=of chunk]          (dedup)      {Deduplication \& filtering\\[1pt]\footnotesize drop duplicates and noise};
  \node[qual, below=of dedup]          (metadata)   {Metadata enrichment\\[1pt]\footnotesize timestamps, geotags, reliability};
  \node[proc, below=of metadata]       (vector)     {Vector store\\[1pt]\footnotesize FAISS, Pinecone, Weaviate, Milvus};

  \draw[flow] (sources)    -- (preprocess);
  \draw[flow] (preprocess) -- (chunk);
  \draw[flow] (chunk)      -- (dedup);
  \draw[flow] (dedup)      -- (metadata);
  \draw[flow] (metadata)   -- (vector);
\end{tikzpicture}
\end{llmfigbox}
...
Figure~\ref{fig:ch08_chunking_callout} summarizes a production chunking configuration that picks a strategy per document type, attaches identifying metadata, and filters out the kind of low-value chunks that erode retrieval quality.

\begin{figure}[t]
\centering
\begin{llmfigbox}
% Local styles built on the book-wide llm/* contract (macros.tex).
% Color carries the role: blue = default policy, green = structural,
% purple = sentence-aware, orange = social/fixed, gray = neutral global.
\tikzset{
  card/.style={text width=0.90\linewidth, align=left, font=\footnotesize},
}
\begin{tikzpicture}[node distance=2mm]
  \node[font=\bfseries\large] (title) {Type-aware chunking strategy};
  \node[llm/box/blue,   card, below=4mm of title.south, anchor=north] (r1)
       {\textbf{Default (recursive text splitter):} 512-token chunks with 128-token overlap; prioritized separators (paragraph $\rightarrow$ line $\rightarrow$ sentence $\rightarrow$ word) so splits land on natural boundaries before falling back to fixed size.};
  \node[llm/box/green,  card, below=of r1] (r2)
       {\textbf{Reports (structural splitter):} split on Markdown headings (\texttt{\#}, \texttt{\#\#}, \texttt{\#\#\#}); 100--1024 token bounds; preserves section semantics for navigational queries.};
  \node[llm/box/purple, card, below=of r2] (r3)
       {\textbf{Legal documents (sentence splitter):} 5 sentences per chunk with 1-sentence overlap; carry \texttt{case\_id} and \texttt{paragraph\_id} metadata so citations resolve to the exact paragraph.};
  \node[llm/box/orange, card, below=of r3] (r4)
       {\textbf{Social posts (fixed-size):} 128-token chunks with no overlap; tag \texttt{source\,=\,social\_media}, \texttt{platform\,=\,twitter} so retrieval can downweight or filter at query time.};
  \node[llm/box/gray,   card, below=of r4] (r5)
       {\textbf{Global metadata \& post-filters:} stamp every chunk with \texttt{source\_system}, \texttt{processing\_date}, and \texttt{embedding\_model\_version}; drop chunks under 50 tokens; strip boilerplate (advertisements, copyright lines) via regex.};
\end{tikzpicture}
\end{llmfigbox}
\caption{Type-aware chunking strategy: a recursive default for unstructured prose, structural splitting for reports, sentence splitting for legal text, and fixed-size chunks for short social posts. Global metadata and post-filters apply uniformly across types so that downstream retrieval has a consistent surface to filter on (model version, source system, length floor) regardless of which strategy produced the chunk.}
\label{fig:ch08_chunking_callout}
\end{figure}
...
\noindent\textbf{Guidance.} For $\leq$\,a few million vectors, HNSW or Flat+GPU often suffice; at tens of millions, IVF (optionally with reranking) is a strong choice; for 100M–1B+, consider IVF+PQ or hybrid schemes and/or distributed shards. Always validate on your data distribution and latency SLOs.
...
Neural sparse methods (e.g., SPLADE) blend lexical interpretability with learned term expansion, often improving recall while retaining sparse indexing \cite{Lin2021SPLADE,AWS2023NeuralSparse}.
...
Figure~\ref{fig:ch08_retrieval_callout} summarizes a production hybrid retrieval pipeline. Each card is a stage that turns the user's raw query into a small, ranked, policy-clean candidate set ready for the prompt.

\begin{figure}[t]
\centering
\begin{llmfigbox}
% Local styles built on the book-wide llm/* contract (macros.tex).
% Color carries the role: blue = query processing, green = dense retriever,
% purple = sparse retriever, orange = reranker, gray = neutral filters/obs.
\tikzset{
  card/.style={text width=0.90\linewidth, align=left, font=\footnotesize},
}
\begin{tikzpicture}[node distance=2mm]
  \node[font=\bfseries\large] (title) {Hybrid retrieval pipeline stages};
  \node[llm/box/blue,   card, below=4mm of title.south, anchor=north] (r1)
       {\textbf{Query processing:} LLM-rewrite query expansion produces 3 variants via \texttt{llama-3.1-8b-query-rewriter}; HyDE (hypothetical document embedding) compression available but off by default to keep latency low on initial retrieval.};
  \node[llm/box/green,  card, below=of r1] (r2)
       {\textbf{Dense retriever (weight 0.7):} top-10 from \texttt{ishtar-news-vector-index} with metadata filters (\texttt{published\_date $\geq$ now-30d}, \texttt{category $\in$ \{politics, conflict\}}); pushes the filter into the query plan, not after the fact \cite{Lin2021SPLADE}.};
  \node[llm/box/purple, card, below=of r2] (r3)
       {\textbf{Sparse retriever (weight 0.3):} top-20 from Elasticsearch on \texttt{ishtar-news-keywords}; complements the dense path on rare named entities and numeric tokens that embeddings tend to blur \cite{Thakur2021BEIR}.};
  \node[llm/box/orange, card, below=of r3] (r4)
       {\textbf{Cross-encoder reranker:} \texttt{cross-encoder/ms-marco-MiniLM-L-6-v2} reranks the merged candidate pool to a final top-5 in batches of 32; the precision step that converts a recall-heavy candidate list into a context-window-ready set \cite{khattab2020colbert,Zhan2021ColBERTv2,cormack2009rrf}.};
  \node[llm/box/gray,   card, below=of r4] (r5)
       {\textbf{Post-retrieval filters \& observability:} embedding-similarity dedup at threshold 0.95; access control through OPA (\texttt{document\_access\_policy\_v1}); log full retrieved-document content (post-redaction) for offline evaluation and incident triage.};
\end{tikzpicture}
\end{llmfigbox}
\caption{Hybrid retrieval pipeline expressed as five stages. Query processing widens the search; dense and sparse retrievers run in parallel with a 0.7/0.3 weighted fusion so each path keeps its strengths (semantic match for paraphrase, exact match for rare entities); the cross-encoder reranker turns a recall-heavy candidate pool into a precision-oriented top-5; post-retrieval filters enforce deduplication and access policy before any chunk reaches the prompt builder.}
\label{fig:ch08_retrieval_callout}
\end{figure}
...
\subsection{Adaptive Retrieval}\index{RAG!adaptive retrieval}
\label{sec:rag-adaptive-retrieval}
Not every query benefits from retrieval. Adaptive retrieval systems decide \emph{when} to retrieve rather than retrieving on every request, optimizing both latency and cost.

\paragraph{Self-RAG: model-driven retrieval decisions.}
Self-RAG trains the model to generate special tokens indicating whether retrieval is needed for a given query. When the model is confident in its parametric knowledge (e.g., well-known facts, common definitions), it generates directly; when uncertainty is detected, it triggers retrieval. This approach can reduce unnecessary retrieval calls substantially while maintaining answer quality on knowledge-intensive queries.

\paragraph{Confidence-based retrieval.}
A simpler production pattern uses the model's output probability distribution as a confidence signal. When the maximum token probability falls below a threshold during generation, the system pauses, retrieves relevant context, and restarts generation with the augmented prompt. This approach requires no special training---it works with any autoregressive model---but adds engineering complexity for mid-generation interruption and restart.

\paragraph{Cost optimization.}
Skipping retrieval for simple queries provides direct cost and latency savings: no embedding computation, no vector search, no reranking. For workloads where 40--60\% of queries are answerable from parametric knowledge (e.g., general knowledge questions, style guidance, or previously discussed topics), adaptive retrieval can reduce average latency by 30\% and retrieval infrastructure costs proportionally.

\paragraph{When to apply adaptive retrieval.}
Adaptive retrieval is most beneficial when: (i) the query distribution is heterogeneous (mixing simple and complex questions), (ii) retrieval latency is a significant fraction of total response time, and (iii) the base model has strong parametric coverage for a subset of expected queries. For \ishtar{}, adaptive retrieval could skip retrieval for common definitional questions (``What is an IDP?'') while always retrieving for event-specific queries (``What happened in Region X yesterday?''). The key risk is false confidence: the model may skip retrieval when it should not, producing a fluent but unsupported answer. Monitoring abstention rates and faithfulness metrics is essential when deploying adaptive retrieval.
...
\paragraph{The ``lost in the middle'' bias.}\index{lost in the middle}
Research on long-context language models has consistently shown that models attend more strongly to information at the \emph{beginning} and \emph{end} of the context window, while evidence placed in the middle receives less attention \cite{Liu2024LostInTheMiddle}. This positional bias means that a naively ordered context---simply concatenating passages in retrieval rank order---may cause the model to underweight critical evidence that falls in the middle positions. The effect becomes more pronounced as context length grows: at 8K+ tokens, middle-positioned facts can be effectively invisible to the generator.

\paragraph{Mitigation strategies.}
Several practical strategies address positional bias in prompt construction:
\begin{itemize}[leftmargin=1.5em, itemsep=2pt]
  \item \textbf{Strategic placement:} Place the single most answer-bearing passage at position 1 (beginning) and the second-most relevant at the final position (end). Secondary evidence fills the middle.
  \item \textbf{Key fact repetition:} For critical facts (dates, quantities, named entities), repeat them in both the passage context and a summary preamble before the passages. This redundancy ensures the model encounters the fact in a high-attention position regardless of where the source passage falls.
  \item \textbf{Summary-first pattern:} Prepend a brief (2--3 sentence) extractive summary of the key facts from all passages before listing the full passages. The model uses the summary for primary reasoning and the full passages for citation and verification. This pattern adds a small number of tokens but significantly improves answer accuracy on long contexts.
  \item \textbf{Diversity-based ordering:} Instead of pure relevance ranking, interleave passages from different sources or covering different aspects of the query. This prevents the model from fixating on a single narrative thread and improves coverage of multi-faceted questions.
\end{itemize}
For \ishtar{}, the summary-first pattern proved especially effective: a 50-token extractive summary prepended to 4--6 source passages improved answer completeness by 12\% on internal evaluations without meaningful latency increase.
...
\end{enumerate}
Prompted citations are easy to implement but can be misattributed; post-hoc approaches are more reliable but require additional engineering. Regardless of strategy, evaluate citation precision and answer faithfulness (Section~\ref{sec:rag-metrics}) \cite{ragas_eacl2024}.
...
RAG corpora frequently include untrusted text (web pages, emails, social media). Retrieved passages may contain instructions that conflict with your system prompt~\cite{OWASP2024LLMTop10}. Mitigations include:
\begin{itemize}
...
\begin{llmfigbox}
% Local styles built on the book-wide llm/* contract (macros.tex).
% Color carries the role: blue = sources/data, green = ingestion stage,
% orange = retrieval/generation, purple = indexing.
\tikzset{
  data/.style ={llm/box/blue,   minimum width=42mm, font=\footnotesize\bfseries},
  ing/.style  ={llm/box/green,  minimum width=42mm, font=\footnotesize\bfseries},
  idx/.style  ={llm/box/purple, minimum width=42mm, font=\footnotesize\bfseries},
  proc/.style ={llm/box/orange, minimum width=42mm, font=\footnotesize\bfseries},
  flow/.style ={llm/arrow},
}
\begin{tikzpicture}[node distance=5mm]
  \node[data]                    (src) {Trusted sources\\[1pt]\footnotesize news, NGOs, bulletins};
  \node[ing,  below=of src]      (ing) {Ingestion \& chunking\\[1pt]\footnotesize normalize + metadata};
  \node[idx,  below=of ing]      (idx) {Embed + index\\[1pt]\footnotesize vector store};
  \node[proc, below=of idx]      (ret) {Retrieve + rerank\\[1pt]\footnotesize hybrid + filtering};
  \node[proc, below=of ret]      (gen) {Generate answer\\[1pt]\footnotesize with citations};

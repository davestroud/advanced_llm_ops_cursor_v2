...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[
    ...
    stage/.style={llm/box, minimum width=26mm, minimum height=14mm, font=\footnotesize, align=center},
    lever/.style={llm/box, minimum width=46mm, minimum height=26mm, font=\footnotesize, align=left, text width=42mm},
    bullet/.style={font=\scriptsize, align=left, text width=42mm},
  ]
    % Stage banner
    ...
    \node[stage, llm/box/blue,   below=4mm of banner]                (s1) {\textbf{Client /}\\\textbf{Gateway}\\[1pt]\scriptsize Network + auth\\$\sim$20--80\,ms};
    \node[stage, llm/box/green,  right=3mm of s1]                     (s2) {\textbf{Tokenize +}\\\textbf{Policy checks}\\[1pt]\scriptsize Parse + validate\\$\sim$5--25\,ms};
    \node[stage, llm/box/orange, right=3mm of s2]                     (s3) {\textbf{Retrieval (RAG)}\\\textbf{+ rerank}\\[1pt]\scriptsize Search + filters\\$\sim$20--150+\,ms};
    \node[stage, llm/box/purple, right=3mm of s3]                     (s4) {\textbf{LLM inference}\\\textbf{(prefill + decode)}\\[1pt]\scriptsize TTFT + tokens\\$\sim$0.3--3.0\,s};
    \node[stage, llm/box/gray,   right=3mm of s4]                     (s5) {\textbf{Post-process}\\\textbf{+ logging}\\[1pt]\scriptsize Citations + redact\\$\sim$5--60\,ms};
    ...
    \node[lever, llm/box/green,  below=3mm of lbanner, xshift=-50mm] (l1) {\textbf{Throughput levers}\\[1pt]\scriptsize $\bullet$ Dynamic batching\\$\bullet$ Concurrency limits\\$\bullet$ Length-aware scheduling\\$\bullet$ KV-cache policy\\$\bullet$ Faster runtimes (vLLM/TGI)};
    \node[lever, llm/box/orange, right=3mm of l1]                     (l2) {\textbf{Cost levers}\\[1pt]\scriptsize $\bullet$ Prompt/context budgets\\$\bullet$ Caching (responses, embeddings)\\$\bullet$ Quantization / distillation\\$\bullet$ Model routing (small$\to$large)\\$\bullet$ Admission control (shed load)};
    \node[lever, llm/box/blue,   right=3mm of l2]                     (l3) {\textbf{Capacity planning}\\[1pt]\scriptsize $\bullet$ Peak QPS $\times$ tokens/request\\$\bullet$ Tail latency (p95/p99)\\$\bullet$ Little's Law: $L = \lambda W$\\$\bullet$ Autoscale on queue depth\\$\bullet$ Keep GPU util high};
    ...
    \node[llm/label, font=\scriptsize, below=2mm of l2, align=center]
...
\begin{tikzpicture}[
  ...
  user/.style={ellipse, draw=none, fill=llmblue!15, minimum width=20mm, minimum height=10mm, align=center, font=\small\bfseries},
  orchestrator/.style={rectangle, draw=none, fill=llmgreen!15, rounded corners=5pt, minimum width=34mm, minimum height=12mm, align=center, font=\small\bfseries},
  retriever/.style={rectangle, draw=none, fill=llmorange!15, rounded corners=5pt, minimum width=27mm, minimum height=10mm, align=center, font=\small},
  knowledge/.style={rectangle, draw=none, fill=llmpurple!15, rounded corners=5pt, minimum width=26mm, minimum height=11mm, align=center, font=\small},
  llm/.style={rectangle, draw=none, fill=llmorange!15, rounded corners=5pt, minimum width=26mm, minimum height=12mm, align=center, font=\small\bfseries},
  tool/.style={rectangle, draw=none, fill=llmgreen!15, rounded corners=5pt, minimum width=24mm, minimum height=10mm, align=center, font=\small},
  response/.style={ellipse, draw=none, fill=llmpurple!15, minimum width=20mm, minimum height=10mm, align=center, font=\small\bfseries},
  ...
  toolflow/.style={-{Latex}, line width=1.2pt, color=llmgreen!70!black},
  retrieval/.style={-{Latex}, line width=1.2pt, color=llmorange!70!black},
  queryflow/.style={-{Latex}, line width=1.2pt, color=llmblue!70!black},
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[
    ...
    phase/.style={llm/box, minimum width=52mm, minimum height=22mm, align=center, font=\footnotesize, text width=48mm},
    goal/.style ={llm/box, minimum width=36mm, minimum height=18mm, align=center, font=\footnotesize, text width=32mm},
  ]
    % Header
    ...
    \node[phase, llm/box/blue,   below=4mm of title, xshift=-32mm]    (detect)   {\textbf{1) Detect drift}\\[2pt]\scriptsize Monitor: hit-rate@k / MRR\\Faithfulness / groundedness\\Citation coverage\\Latency \& cost anomalies};
    \node[phase, llm/box/green,  right=36mm of detect]                 (diagnose) {\textbf{2) Diagnose cause}\\[2pt]\scriptsize Classify: index staleness vs.\\query shift vs.\ governance\\Localize by slice (topic, source,\\region, recency)};
    \node[phase, llm/box/orange, below=20mm of diagnose]               (remediate){\textbf{3) Remediate}\\[2pt]\scriptsize Re-embed / re-index\\Tune retriever \& filters\\Adjust chunking / reranking\\Update permissions \& trust rules};
    \node[phase, llm/box/purple, left=36mm of remediate]               (validate) {\textbf{4) Validate + gate release}\\[2pt]\scriptsize Canary query suite (locked evidence)\\A/B or shadow evaluation\\Snapshot index + configs\\Rollback on regression};

    % Goal centered inside the cycle
    \node[goal, llm/box/gray, fill=llmorange!12, draw=llmorange!70]
      at ($(detect)!0.5!(remediate)$) (goal) {\textbf{Goal}\\[2pt]\scriptsize Stable retrieval +\\trustworthy answers\\as corpora evolve};
    ...
    \node[llm/box/gray, below=10mm of validate, xshift=34mm,
          ...
          align=left, font=\scriptsize, text width=146mm]
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[
    ...
    % Box styles - enhanced
    constraint/.style={rectangle, draw=none, fill=llmorange!15, rounded corners=5pt, minimum width=2.3cm, minimum height=0.95cm, align=center, font=\footnotesize\bfseries},
    slo/.style={rectangle, draw=none, fill=llmblue!15, rounded corners=5pt, minimum width=2.1cm, minimum height=0.85cm, align=center, font=\footnotesize\bfseries},
    lever/.style={rectangle, draw=none, fill=llmgreen!15, rounded corners=5pt, minimum width=2.1cm, minimum height=0.8cm, align=center, font=\footnotesize\bfseries},
  ...
  \draw[arrow, color=llmorange!70!black] (param.east) -- (ttft.west);
  \draw[arrow, color=llmorange!70!black] (param.east) -- (parallel.west);
  
  \draw[arrow, color=llmorange!60!black] (context.east) -- (ttft.west);
  \draw[arrow, color=llmorange!60!black] (context.east) -- (tput.west);
  
  \draw[arrow, color=llmorange!50!black] (kv.east) -- (tput.west);
  
  \draw[arrow, color=llmblue!70!black] (ttft.east) -- (parallel.west);
  \draw[arrow, color=llmblue!70!black] (tput.east) -- (quant.west);
  \draw[arrow, color=llmblue!70!black] (tput.east) -- (batch.west);
  \draw[arrow, color=llmblue!70!black] (cost.east) -- (quant.west);
  
  \draw[arrow, color=llmgreen!70!black] (parallel.west) -- (cost.east);
  \draw[arrow, color=llmgreen!70!black] (quant.west) -- (cost.east);
  \draw[arrow, color=llmgreen!70!black] (batch.west) -- (tput.east);

  % Column labels (simpler, no braces)
  \node[font=\small\bfseries, color=llmorange!80!black, above=0.5cm of param] {Constraints};
  \node[font=\small\bfseries, color=llmblue!80!black, above=0.5cm of ttft] {SLOs};
  \node[font=\small\bfseries, color=llmgreen!80!black, above=0.5cm of parallel] {Levers};
...
\matrix (top) [row sep=0mm, column sep=6mm] {
  \node[phase, fill=llmblue!12, draw=llmblue!40] (fund) {LLMOps\\Fundamentals\\(Ch.~\ref{ch:llmops-fundamentals})}; &
  \node[phase, fill=llmgreen!12, draw=llmgreen!40] (infra) {Infrastructure\\\& Environment\\(Ch.~\ref{ch:infra})}; &
  \node[phase, fill=llmorange!12, draw=llmorange!40] (cicd)  {CI/CD\\(Ch.~\ref{ch:cicd})}; &
  \node[phase, fill=llmpurple!12, draw=llmpurple!40] (mon)   {Monitoring\\(Ch.~\ref{ch:monitoring})}; &
  \node[phase, fill=llmpurple!12, draw=llmpurple!40] (scale) {Scaling\\(Ch.~\ref{ch:scaling})}; &
  \node[phase, fill=llmorange!12, draw=llmorange!40] (perf)  {Performance\\(Ch.~\ref{ch:performance})}; \\
};
...
\node[phase, fill=llmgreen!12, draw=llmgreen!40, below=12mm of perf] (rag) {RAG\\(Ch.~\ref{ch:rag})};
\node[phase, fill=llmgreen!12, draw=llmgreen!40, right=6mm of rag] (orch) {Multi‑Agent\\(Ch.~\ref{ch:multiagent})};

% ---- Bottom row placed relative to second row ----
\node[phase, fill=llmorange!12, draw=llmorange!40, below=12mm of rag]   (test)   {Testing \&\\Robustness\\(Ch.~\ref{ch:testing})};
\node[phase, fill=llmpurple!12, draw=llmpurple!40, right=6mm of test]   (ethics) {Ethics \&\\Governance\\(Ch.~\ref{ch:ethics})};
\node[phase, fill=llmorange!12, draw=llmorange!40, right=6mm of ethics] (case)   {Case Study\\(Ch.~\ref{ch:case-study})};
...
\begin{tikzpicture}[
  ...
  source/.style={llm/box/purple, ellipse, minimum width=22mm, minimum height=10mm, font=\scriptsize, align=center, text width=20mm},
  ingest/.style={llm/box/blue,  minimum width=22mm, minimum height=10mm, font=\scriptsize, align=center, text width=20mm},
  query/.style ={llm/box/purple, ellipse, minimum width=22mm, minimum height=10mm, font=\scriptsize\bfseries, align=center},
  rag/.style   ={llm/box/orange, minimum width=22mm, minimum height=10mm, font=\footnotesize\bfseries, align=center},
  orch/.style  ={llm/box/orange, minimum width=24mm, minimum height=10mm, font=\footnotesize\bfseries, align=center},
  agent/.style ={llm/box/green,  minimum width=22mm, minimum height=9mm, font=\scriptsize, align=center},
  gpu/.style   ={llm/box/orange, fill=llmorange!25, minimum width=22mm, minimum height=10mm, font=\scriptsize\bfseries, align=center, text width=20mm},
  store/.style ={llm/box/green,  cylinder, shape border rotate=90, aspect=0.3, minimum width=20mm, minimum height=14mm, font=\scriptsize, align=center},
  respond/.style={llm/box/gray,  ellipse, minimum width=22mm, minimum height=10mm, font=\scriptsize\bfseries, align=center, text width=20mm},

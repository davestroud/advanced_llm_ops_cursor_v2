...
This chapter introduces LLMOps as the engineering practice for running LLM-powered systems reliably at scale. We begin by motivating LLMOps through production failure modes and operational challenges (compute economics, serving infrastructure, data drift, evaluation, observability, and security). We then introduce the Ishtar AI case study as a running example throughout the book, examine early deployment case studies to extract operational lessons, and conclude with a roadmap mapping each chapter to the end-to-end LLMOps lifecycle.
...
These trade-offs across latency, throughput, and cost are illustrated in Figure~\ref{fig:ch01_cost_latency_throughput}, which shows how each stage of the request path contributes to end-to-end response time and how different optimization strategies target specific bottlenecks.
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[
    ...
    stage/.style={llm/box, minimum width=26mm, minimum height=15mm, font=\footnotesize, align=center},
    lever/.style={llm/box, minimum width=46mm, minimum height=30mm, font=\footnotesize, align=left, text width=42mm},
  ]
    % Stage banner
    \node[llm/label, font=\footnotesize\itshape, align=center]
      (banner) {End-to-end latency $= \sum$(stage latencies)\quad $|$\quad Track p50/p95/p99 and TTFT separately};

    % Top row: pipeline stages
    \node[stage, llm/box/blue,   below=4mm of banner]                (s1) {\textbf{Client /}\\\textbf{Gateway}\\[1pt]\footnotesize Network + auth\\$\sim$20--80\,ms};
    \node[stage, llm/box/green,  right=3mm of s1]                     (s2) {\textbf{Tokenize +}\\\textbf{Policy checks}\\[1pt]\footnotesize Parse + validate\\$\sim$5--25\,ms};
    \node[stage, llm/box/orange, right=3mm of s2]                     (s3) {\textbf{Retrieval (RAG)}\\\textbf{+ rerank}\\[1pt]\footnotesize Search + filters\\$\sim$20--150+\,ms};
    \node[stage, llm/box/purple, right=3mm of s3]                     (s4) {\textbf{LLM inference}\\\textbf{(prefill + decode)}\\[1pt]\footnotesize TTFT + tokens\\$\sim$0.3--3.0\,s};
    \node[stage, llm/box/gray,   right=3mm of s4]                     (s5) {\textbf{Post-process}\\\textbf{+ logging}\\[1pt]\footnotesize Citations + redact\\$\sim$5--60\,ms};

    % Lever-group banner
    \node[llm/label, font=\footnotesize\itshape, below=9mm of s3]    (lbanner) {Primary scale levers};

    % Bottom row: lever groups (aligned under stages)
    \node[lever, llm/box/green,  below=3mm of lbanner, xshift=-50mm] (l1) {\textbf{Throughput levers}\\[1pt]\footnotesize $\bullet$ Dynamic batching\\$\bullet$ Concurrency limits\\$\bullet$ Length-aware scheduling\\$\bullet$ KV-cache policy\\$\bullet$ Faster runtimes (vLLM/TGI)};
    \node[lever, llm/box/orange, right=3mm of l1]                     (l2) {\textbf{Cost levers}\\[1pt]\footnotesize $\bullet$ Prompt/context budgets\\$\bullet$ Caching (responses, embeddings)\\$\bullet$ Quantization / distillation\\$\bullet$ Model routing (small$\to$large)\\$\bullet$ Admission control (shed load)};
    \node[lever, llm/box/blue,   right=3mm of l2]                     (l3) {\textbf{Capacity planning}\\[1pt]\footnotesize $\bullet$ Peak QPS $\times$ tokens/request\\$\bullet$ Tail latency (p95/p99)\\$\bullet$ Little's Law: $L = \lambda W$\\$\bullet$ Autoscale on queue depth\\$\bullet$ Keep GPU util high};

    % Linkages: which levers bear on which stages
    \draw[llm/arrow/dashed] (l1.north) to[out=90,in=270] (s2.south);
    \draw[llm/arrow/dashed] (l1.north) to[out=90,in=270] (s4.south);
    \draw[llm/arrow/dashed] (l2.north) to[out=90,in=270] (s3.south);
    \draw[llm/arrow/dashed] (l2.north) to[out=90,in=270] (s4.south);
    \draw[llm/arrow/dashed] (l3.north) to[out=90,in=270] (s1.south);
    \draw[llm/arrow/dashed] (l3.north) to[out=90,in=270] (s4.south);
    \draw[llm/arrow/dashed] (l3.north) to[out=90,in=270] (s5.south);

    % Footnote-style SLO reminder
    \node[llm/label, font=\footnotesize, below=2mm of l2, align=center]
      {Practical SLO framing: TTFT (time-to-first-token), tokens/s, cost per 1K tokens, and correctness/groundedness --- measured by slice.};
  \end{tikzpicture}
  \end{llmfigbox}
...
Reliability\index{reliability} is also a concern: when a subcomponent fails (retriever times out, a tool API is down, or the model hits a rate limit), the overall system should gracefully degrade (e.g., use cached or partial results, fall back to a simpler response) rather than fail catastrophically. Figure~\ref{fig:ch01_architecture} illustrates how these components interact and where fallback mechanisms can be inserted to maintain system availability under partial failures.

% Preamble requirements: \usepackage{tikz} and \usetikzlibrary{arrows.meta,positioning,shapes.geometric}
\begin{figure}[t]
\centering
\begin{llmfigbox}
% Local styles built on the book-wide llm/* contract (macros.tex).
% Shape vocabulary: ellipse = actor / endpoint, rectangle = component,
% diamond = failure indicator. Solid arrows = primary flow,
% dashed arrows = fallback / degraded path.
\begin{tikzpicture}[
  node distance=12mm and 22mm,
  actor/.style    ={llm/box/purple, ellipse, minimum width=22mm, minimum height=10mm, font=\footnotesize\bfseries},
  result/.style   ={llm/box/gray,   ellipse, minimum width=22mm, minimum height=10mm, font=\footnotesize\bfseries},
  ctrl/.style     ={llm/box/green,  minimum width=36mm, minimum height=12mm, font=\footnotesize\bfseries, align=center},
  worker/.style   ={llm/box/blue,   minimum width=28mm, minimum height=11mm, font=\footnotesize, align=center},
  store/.style    ={llm/box/blue,   minimum width=28mm, minimum height=11mm, font=\footnotesize, align=center},
  model/.style    ={llm/box/orange, minimum width=28mm, minimum height=12mm, font=\footnotesize\bfseries, align=center},
  ext/.style      ={llm/box/purple, minimum width=26mm, minimum height=11mm, font=\footnotesize, align=center},
  failure/.style  ={diamond, aspect=1.4, draw=black!70, line width=0.6pt,
                    fill=black!8, font=\footnotesize\bfseries, inner sep=1pt},
  flow/.style     ={llm/arrow},
  fallback/.style ={llm/arrow/dashed},
  edgelbl/.style  ={font=\scriptsize, fill=white, inner sep=1pt, text=black!70},
]

% Top level: actor and result endpoints
\node[actor]                              (user)         {User};
\node[result, right=36mm of user]         (response)     {Response};

% Orchestrator (controller, centered below user)
\node[ctrl, below=14mm of user]           (orchestrator) {Orchestrator\\\footnotesize LLM Application};

% Retrieval lane (left of orchestrator)
\node[worker, below left=12mm and 16mm of orchestrator] (retriever)  {Retriever\\\footnotesize (Vector DB)};
\node[store,  below=10mm of retriever]                   (knowledge) {Knowledge Base\\\footnotesize (Documents)};

% Inference lane (right of orchestrator)
\node[model, below right=12mm and 16mm of orchestrator] (llm)        {LLM Model};
\node[ext,   below=10mm of llm]                          (tool)      {External Tool\\\footnotesize / API};

% Failure indicator on the retrieval path (shape carries semantics; the
% accompanying text label keeps the figure legible in grayscale).
\node[failure, left=6mm of retriever] (failure) {\textbf{!}};
\node[font=\scriptsize\bfseries, text=black!75, left=2mm of failure] {Failure};

% Primary control flow: User -> Orchestrator -> Response
\draw[flow] (user)         -- node[edgelbl, above]{User query}   (orchestrator);
\draw[flow] (orchestrator) -- node[edgelbl, above]{Return answer}(response);

% Retrieval round-trip (outbound from south-west, return to south-west
% so the inference lane keeps the south-east region uncluttered).
\draw[flow] ([xshift=-6mm]orchestrator.south) -- node[edgelbl, left]{Retrieve docs} (retriever.north);
\draw[flow] (retriever.south)                  -- node[edgelbl, right]{Query index}  (knowledge.north);
\draw[flow] (knowledge.east) to[out=0,in=-30, looseness=1.4]
            node[edgelbl, right, pos=0.55]{Docs} (retriever.east);
\draw[flow] (retriever.north east) to[out=60,in=-110, looseness=1.0]
            node[edgelbl, right, pos=0.55]{Evidence} ([xshift=-2mm]orchestrator.south);

% Failure tap on the retrieval path (dashed = degraded / non-primary)
\draw[fallback] (retriever.west) -- (failure.east);

% LLM call (primary): outbound from south-east, return to south-east so
% inbound and outbound LLM edges do not collide with retrieval edges.
\draw[flow] ([xshift=6mm]orchestrator.south) -- node[edgelbl, right]{Prompt + context} (llm.north);
\draw[flow] (llm.north west) to[out=120,in=-60, looseness=1.0]
            node[edgelbl, left, pos=0.45]{LLM answer} ([xshift=2mm]orchestrator.south);

% Fallback path: skip retrieval and call LLM directly (dashed). Routed
% as an elbow east of the orchestrator and down to the LLM model, so it
% does not cross the prompt/answer edges that share orchestrator.south.
\draw[fallback] (orchestrator.east) -| node[edgelbl, above, pos=0.25]
                {Fallback: skip retrieval} (llm.north east);

% Tool round-trip
\draw[flow] (llm.south)  -- node[edgelbl, right]{Tool call} (tool.north);
\draw[flow] (tool.east) to[out=0,in=-30, looseness=1.2]
            node[edgelbl, right, pos=0.55]{Result} (llm.east);

\end{tikzpicture}
\end{llmfigbox}
\caption{LLM application architecture\index{architecture!LLM application} determines reliability and capability. The orchestrator (green controller) coordinates a retrieval lane (blue retriever and knowledge base) and an inference lane (orange model with optional external tools, purple); a diamond marks the retrieval failure mode and dashed arrows show the fallback path that skips retrieval when evidence is unavailable. This RAG\index{RAG|see{Retrieval-Augmented Generation}}\index{Retrieval-Augmented Generation|(}-based design balances accuracy (via grounded retrieval), reliability (via explicit fallbacks), and functionality (via tool integration) for production deployments.}
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[
    ...
    phase/.style={llm/box, minimum width=54mm, minimum height=24mm, align=center, font=\footnotesize, text width=50mm},
    goal/.style ={llm/box, minimum width=38mm, minimum height=20mm, align=center, font=\footnotesize, text width=34mm},
  ]
    % Header
    \node[llm/label, font=\small\bfseries, align=center]
      (title) {RAG Drift Management: Detect $\to$ Diagnose $\to$ Remediate $\to$ Validate};

    % Four phases arranged in a cycle (clockwise from top-left)
    \node[phase, llm/box/blue,   below=4mm of title, xshift=-32mm]    (detect)   {\textbf{1) Detect drift}\\[2pt]\footnotesize Monitor: hit-rate@k / MRR\\Faithfulness / groundedness\\Citation coverage\\Latency \& cost anomalies};
    \node[phase, llm/box/green,  right=36mm of detect]                 (diagnose) {\textbf{2) Diagnose cause}\\[2pt]\footnotesize Classify: index staleness vs.\\query shift vs.\ governance\\Localize by slice (topic, source,\\region, recency)};
    \node[phase, llm/box/orange, below=20mm of diagnose]               (remediate){\textbf{3) Remediate}\\[2pt]\footnotesize Re-embed / re-index\\Tune retriever \& filters\\Adjust chunking / reranking\\Update permissions \& trust rules};
    \node[phase, llm/box/purple, left=36mm of remediate]               (validate) {\textbf{4) Validate + gate release}\\[2pt]\footnotesize Canary query suite (locked evidence)\\A/B or shadow evaluation\\Snapshot index + configs\\Rollback on regression};

    % Goal centered inside the cycle (orange-accented to highlight the
    % central objective; built on the gray neutral base for consistency).
    \node[goal, llm/box/gray, fill=llmorange!12, draw=llmorange!70]
      at ($(detect)!0.5!(remediate)$) (goal) {\textbf{Goal}\\[2pt]\footnotesize Stable retrieval +\\trustworthy answers\\as corpora evolve};

    % Cycle arrows
    \draw[llm/arrow] (detect)    -- (diagnose);
    \draw[llm/arrow] (diagnose)  -- (remediate);
    \draw[llm/arrow] (remediate) -- (validate);
    \draw[llm/arrow] (validate)  -- (detect);

    % Drift sources callout
    \node[llm/box/gray, below=10mm of validate, xshift=34mm,
          minimum width=152mm, minimum height=15mm,
          align=left, font=\footnotesize, text width=146mm]
         (sources) {\textbf{Typical drift sources:}\\
          $\bullet$ Corpus updates (policies, docs)\quad
          $\bullet$ Embedding model change\quad
          $\bullet$ Index/config change (chunking, filters)\\
          $\bullet$ Query distribution shift (events, new intents)\quad
          $\bullet$ Permission/provenance drift (ACLs, lineage)};
  \end{tikzpicture}
  \end{llmfigbox}
...
\begin{codebox}{Standardized observability telemetry schema capturing the full semantic path of an LLM request, from prompt and retrieval metadata through tool invocations to output quality signals and cost estimates. This structured trace format enables end-to-end debugging, governance auditing, and cost attribution across production LLM pipelines.}{lst:ch01_observability_schema}
\begin{minted}{json}
{
...
\begin{codebox}{Versioned tool schema with security controls for a source-verification tool, including authentication requirements, rate limiting, sandboxed execution, and contract test references. This defense-in-depth approach ensures that LLM-invoked tools operate within well-defined security boundaries while maintaining backward compatibility across schema versions.}{lst:ch01_tool_schema}
\begin{minted}{json}
{
...
% Preamble requirements: \usepackage{tikz} and \usetikzlibrary{arrows.meta,positioning,shapes.geometric,calc,fit}
\begin{landscape}
\begin{figure}[t]
  \centering
  \begin{llmfigbox}
  \begin{tikzpicture}[
    ...
    % Three layer-specific box variants, all built on llm/box for uniform
    % stroke weight, corner radius, and typography.
    surface/.style ={llm/box/blue,
                     minimum width=30mm, minimum height=14mm,
                     align=center, font=\footnotesize, text width=26mm},
    threat/.style  ={llm/box/orange,
                     minimum width=30mm, minimum height=12mm,
                     align=center, font=\footnotesize, text width=26mm},
    control/.style ={llm/box/green,
                     minimum width=30mm, minimum height=14mm,
                     align=center, font=\footnotesize, text width=26mm},
    crosscut/.style={llm/box/purple, fill=llmpurple!8,
                     minimum height=11mm,
                     align=left, font=\footnotesize, inner sep=4pt},
    lanelabel/.style={font=\small\bfseries, color=black!75,
                      align=left, text width=28mm, anchor=east},
    % Arrow semantics:
    %   flow  : data pipeline (left to right along the surface lane)
    %   maps  : threat attacks a surface  (surface down to threat)
    %   mits  : control mitigates a threat (threat down to control)
    %   trace : audit lane taps every threat
    flow/.style ={llm/arrow, color=llmblue!75!black,  line width=0.9pt},
    maps/.style ={llm/arrow/dashed, color=llmorange!80!black},
    mits/.style ={llm/arrow, color=llmgreen!70!black, line width=0.9pt},
    % Audit tap: thin solid connector (no arrowhead) + small "port" dot on the
    % audit band. Reads as a bus-tap rather than directional flow.
    tap/.style  ={line width=0.55pt, color=llmpurple!70!black},
    port/.style ={circle, fill=llmpurple!70!black, inner sep=0pt, minimum size=1.3mm},
  ]
    % --- Layout grid (5 columns, 3 functional rows + 1 cross-cutting band) ---
    \def\colA{0}    \def\colB{3.6}  \def\colC{7.2}  \def\colD{10.8} \def\colE{14.4}
    \def\rowS{7.8}   % surface row
    \def\rowT{5.0}   % threat row
    \def\rowX{3.1}   % cross-cut row
    \def\rowC{0.7}   % control row

    % --- Row 1: threat surfaces (data-pipeline stages) ---
    \node[surface] (input)     at (\colA,\rowS) {\textbf{Untrusted input}\\User query\\Uploads\\Links};
    \node[surface] (prompt)    at (\colB,\rowS) {\textbf{Prompt / policy}\\System rules\\Templates\\Routing};
    \node[surface] (retrieval) at (\colC,\rowS) {\textbf{Retrieval (RAG)}\\Search + filters\\Evidence bundle\\Citations};
    \node[surface] (tools)     at (\colD,\rowS) {\textbf{Tools / actions}\\APIs\\DB writes\\Workflows};
    \node[surface] (output)    at (\colE,\rowS) {\textbf{Model output}\\Answer text\\Citations\\Logs};

    \foreach \a/\b in {input/prompt, prompt/retrieval, retrieval/tools, tools/output}
      \draw[flow] (\a.east) -- (\b.west);

    % --- Row 2: threats (what can go wrong at each surface) ---
    \node[threat]  (inj)        at (\colA,\rowT) {\textbf{Prompt injection}\\User text overrides\\instructions};
    \node[threat]  (bypass)     at (\colB,\rowT) {\textbf{Policy bypass}\\Hidden rules exposed\\or weakened};
    \node[threat]  (exfil)      at (\colC,\rowT) {\textbf{Data exfiltration}\\Unauthorized docs\\retrieved};
    \node[threat]  (misuse)     at (\colD,\rowT) {\textbf{Tool misuse}\\(confused deputy)\\Unsafe side effects};
    \node[threat]  (leak)       at (\colE,\rowT) {\textbf{Output leakage}\\(PII / secrets)\\Sensitive content};

    % Surface -> Threat mappings (dashed orange)
    \foreach \a/\b in {input/inj, prompt/bypass, retrieval/exfil, tools/misuse, output/leak}
      \draw[maps] (\a.south) -- (\b.north);

    % --- Row 3: cross-cutting audit / tracing band ---
    \node[crosscut, minimum width=173mm] (tracing) at (7.2,\rowX)
      {\textbf{Cross-cutting requirement:}\quad structured tracing + audit logs
       (prompt IDs, retrieved doc IDs, tool calls, policy decisions)
       enable incident response and rollback across every layer.};

    % Tracing taps every threat column: short solid connector down to the
    % audit band + small filled "port" dot at the tap point (bus-tap motif).
    \foreach \t in {inj, bypass, exfil, misuse, leak}{
      \draw[tap] (\t.south) -- (\t.south |- tracing.north);
      \node[port] at (\t.south |- tracing.north) {};
    }

    % --- Row 4: controls (what you implement) ---
    \node[control] (sanitize)   at (\colA,\rowC) {\textbf{Input sanitization}\\Separate user vs\\system content\\Block known patterns};
    \node[control] (hierarchy)  at (\colB,\rowC) {\textbf{Instruction hierarchy}\\System $>$ developer $>$ user\\Constrained formats};
    \node[control] (governance) at (\colC,\rowC) {\textbf{Retrieval governance}\\ACL checks\\Provenance + recency\\Trust allow-list};
    \node[control] (toolsec)    at (\colD,\rowC) {\textbf{Tool security}\\Allow-list tools\\Argument validation\\Sandbox side effects};
    \node[control] (outctrl)    at (\colE,\rowC) {\textbf{Output controls}\\PII / secret redaction\\Citation requirements\\Rate limits \& HITL};

    % Threat -> Control mitigations (solid green)
    \foreach \a/\b in {inj/sanitize, bypass/hierarchy, exfil/governance, misuse/toolsec, leak/outctrl}
      \draw[mits] (\a.south) -- (\b.north);

    % --- Swim-lane labels (grayscale-safe) on the left margin ---
    \node[lanelabel, text=llmblue!70!black]   at (-2.8, \rowS) {Threat\\surfaces};
    \node[lanelabel, text=llmorange!80!black] at (-2.8, \rowT) {Threats\\\textnormal{\scriptsize what can go wrong}};
    \node[lanelabel, text=llmpurple!70!black] at (-2.8, \rowX) {Audit lane};
    \node[lanelabel, text=llmgreen!60!black]  at (-2.8, \rowC) {Controls\\\textnormal{\scriptsize what you implement}};

    % --- Legend: horizontal strip centered below the controls row ---
    \node[llm/box/gray, align=center, font=\scriptsize,
          inner sep=4pt, minimum width=170mm,
          anchor=north] (legend) at (7.2,-0.4)
      {\textbf{Legend:}\;\;
       \tikz[baseline=-0.6ex]{\draw[flow]  (0,0) -- (7mm,0);}\;data flow\quad
       \tikz[baseline=-0.6ex]{\draw[maps]  (0,0) -- (7mm,0);}\;surface $\rightarrow$ threat\quad
       \tikz[baseline=-0.6ex]{\draw[mits]  (0,0) -- (7mm,0);}\;threat $\rightarrow$ control\quad
       \tikz[baseline=-0.6ex]{\draw[tap]   (0,0) -- (7mm,0);
                               \node[port] at (3.5mm,0) {};}\;audit tap};
  \end{tikzpicture}
  \end{llmfigbox}
...
The preceding discussion of operational challenges is not merely theoretical---real-world deployments have already produced costly failures that underscore the need for disciplined LLMOps practices. Table~\ref{tab:ch01_economic_impact} catalogs several high-profile incidents\index{LLM!failures}\index{hallucination!economic impact} where inadequate operational controls led to financial, legal, or reputational consequences. Each case illustrates a specific failure mode---hallucination\index{hallucination}, data leakage\index{data leakage}, or unsafe generation---and the corresponding LLMOps lesson that could have prevented or mitigated the harm.

\begin{table}[t]
\centering
\small
\caption{Real-world economic impact of LLM operational failures. These incidents demonstrate that hallucination, data leakage, and unsafe generation carry concrete financial and reputational costs. Each row maps a public incident to its consequence and the LLMOps practice that addresses the underlying failure mode.}
\label{tab:ch01_economic_impact}
\setlength{\tabcolsep}{5pt}
\renewcommand{\arraystretch}{1.3}
\rowcolors{2}{gray!8}{white}
\begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{26mm}>{\raggedright\arraybackslash}p{14mm}>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X@{}}
\toprule
\rowcolor{gray!15}
\textbf{Incident} & \textbf{Year} & \textbf{Consequence} & \textbf{LLMOps Lesson} \\
\midrule
Air Canada chatbot promised unauthorized refund\index{Air Canada chatbot} &
2024 &
Court ordered airline to honor the hallucinated refund policy; estimated low five-figure payout plus legal costs &
Output grounding\index{grounding} and policy-constrained generation: responses touching contractual or financial commitments must be validated against authoritative sources before delivery \\
\addlinespace[2pt]
Lawyer submitted ChatGPT-fabricated case citations\index{fabricated citations} &
2023 &
\$5{,}000 court-imposed fine; sanctioned attorneys; reputational damage to the firm &
Citation verification\index{citation!verification} pipelines and human-in-the-loop review gates for high-stakes outputs; never treat LLM output as factual without independent validation \\
\addlinespace[2pt]
Samsung engineers leaked proprietary code via ChatGPT\index{data leakage!Samsung} &
2023 &
Intellectual property exposure; company-wide ban on external LLM tools; remediation and policy overhaul costs &
Data loss prevention\index{data loss prevention} (DLP) controls, input sanitization, and organizational policies governing which data may be sent to external model endpoints \\
\addlinespace[2pt]
Healthcare chatbot hallucinated medical advice\index{hallucination!healthcare} &
2023--24 &
Patient safety risk; regulatory scrutiny; potential malpractice liability &
Domain-specific evaluation suites\index{evaluation!domain-specific}, retrieval grounding against vetted medical sources, and mandatory human review for clinical content \\
\addlinespace[2pt]
DPD chatbot generated offensive content\index{DPD chatbot} &
2024 &
Viral brand damage; system taken offline; public relations crisis &
Adversarial testing\index{adversarial testing} (red-teaming), output safety filters, and real-time behavioral monitoring\index{monitoring!behavioral} with automated circuit breakers \\
\bottomrule
\end{tabularx}
\end{table}

These incidents share a common thread: the failure was not in the model's raw capability, but in the absence of operational safeguards---grounding checks, output filters, access controls, and evaluation gates---that LLMOps practices are designed to provide. As LLM-powered systems expand into higher-stakes domains (legal, medical, financial), the cost of operational gaps will only increase, making the investment in systematic LLMOps practices not merely prudent but economically essential. Table~\ref{tab:ch01_llmops_extends_mlops} articulates how LLMOps extends traditional MLOps across four critical dimensions: scale, complexity, variability, and risk.
...
\centering
\begin{llmfigbox}
% Local styles built on the book-wide palette (macros.tex):
%   capability milestones use the four core hues (blue / green / orange / purple),
%   operational inflection bands reuse the same hues at lighter saturations.
%   Tick marks reuse llm/arrow heads; year text uses footnotesize.
\begin{tikzpicture}[
  font=\footnotesize,
  milestoneA/.style ={fill=llmblue!18,   draw=llmblue!60,   line width=0.5pt},
  milestoneB/.style ={fill=llmgreen!18,  draw=llmgreen!60,  line width=0.5pt},
  milestoneC/.style ={fill=llmorange!18, draw=llmorange!60, line width=0.5pt},
  milestoneD/.style ={fill=llmpurple!18, draw=llmpurple!60, line width=0.5pt},
  ribbonA/.style    ={fill=llmblue!14,   draw=llmblue!50,   line width=0.4pt},
  ribbonB/.style    ={fill=llmgreen!14,  draw=llmgreen!50,  line width=0.4pt},
  ribbonC/.style    ={fill=llmorange!14, draw=llmorange!50, line width=0.4pt},
  axis/.style       ={draw=black!75, line width=0.6pt},
  tick/.style       ={draw=black!60, line width=0.4pt},
  label/.style      ={inner sep=2.5pt, rounded corners=2pt, align=center,
                      font=\footnotesize, text=black!85},
  yearlabel/.style  ={font=\footnotesize\bfseries, inner sep=1pt, text=black!75},
  opstext/.style    ={font=\footnotesize\itshape, text=black!75},
  drop/.style       ={llm/arrow},
]
...
\draw[drop] (1,\yaxis+0.5) -- (1,\yaxis+0.08);
...
\draw[drop] (3,\yaxis+1.5) -- (3,\yaxis+0.08);
...
\draw[drop] (8.6,\yaxis+2.1) -- (8.6,\yaxis+0.08);
...
\draw[drop] (11.1,\yaxis+3.1) -- (11.1,\yaxis+0.08);
...
\draw[drop] (13.6,\yaxis+2.4) -- (13.6,\yaxis+0.08);

% --- Ribbon underlay: Ops consequences bands (below axis) ---
% Serving at scale (2020+)
\node[anchor=west, opstext] at (3,\yops+0.35) {Serving at scale (batching, KV cache, sharding)};
...
\fill[llmblue!5, rounded corners=6pt]
...
% --- Legend (compact) ---
\node[llm/box/gray, anchor=north west, align=left, font=\footnotesize,
      inner sep=4pt, minimum height=0pt]
  at (0.15,\ytop-0.2)
...
\begin{figure}[t]
  ...
  \begin{llmfigbox}
  % Local styles built on the book-wide llm/* contract (macros.tex):
  % constraints (orange) -> SLOs (blue) -> levers (green). The lever feedback
  % into cost/throughput is dashed because it is the closed-loop response.
  \begin{tikzpicture}[
    node distance=0.8cm and 1.8cm,
    constraint/.style={llm/box/orange, minimum width=26mm, minimum height=11mm, font=\footnotesize\bfseries, align=center},
    slo/.style       ={llm/box/blue,   minimum width=22mm, minimum height=10mm, font=\footnotesize\bfseries, align=center},
    lever/.style     ={llm/box/green,  minimum width=24mm, minimum height=10mm, font=\footnotesize\bfseries, align=center},
    flow/.style      ={llm/arrow},
    feedback/.style  ={llm/arrow/dashed},
    collabel/.style  ={font=\small\bfseries, text=black!75},
  ]

  % Left column: Constraints (with formulas integrated)
  \node[constraint] (param)   at (-4.0, 1.6) {Parameter Scale\\\footnotesize $M_{\text{params}} \approx P \cdot b$};
  \node[constraint] (context) at (-4.0, 0.0) {Context Length\\\footnotesize Cost $\propto T^2$};
  \node[constraint] (kv)      at (-4.0,-1.6) {KV-Cache Pressure\\\footnotesize $M_{\text{KV}} \propto T \cdot B$};

  % Center column: SLOs
  \node[slo] (ttft) at (0, 1.6) {TTFT};
  \node[slo] (tput) at (0, 0.0) {Tokens/s};
  \node[slo] (cost) at (0,-1.6) {\$/1K tokens};

  % Right column: Engineering Levers
  \node[lever] (parallel) at (4.0, 1.6) {Parallelism};
  \node[lever] (quant)    at (4.0, 0.0) {Quantization};
  \node[lever] (batch)    at (4.0,-1.6) {Batching \&\\Scheduling};

  % Constraints -> SLOs (which constraint drives which SLO)
  \draw[flow] (param.east)   -- (ttft.west);
  \draw[flow] (context.east) -- (ttft.west);
  \draw[flow] (context.east) -- (tput.west);
  \draw[flow] (kv.east)      -- (tput.west);

  % SLOs -> Levers (which lever you pull to meet which SLO)
  \draw[flow] (ttft.east) -- (parallel.west);
  \draw[flow] (tput.east) -- (quant.west);
  \draw[flow] (tput.east) -- (batch.west);
  \draw[flow] (cost.east) -- (quant.west);

  % Closed-loop feedback from levers back to cost / throughput
  % (dashed = optimization response, not a primary causal arrow).
  \draw[feedback] (parallel.south west) to[bend right=12] (cost.north east);
  \draw[feedback] (quant.west)         to[bend right=10] (cost.east);
  \draw[feedback] (batch.north west)   to[bend left=10]  (tput.south east);

  % Column labels
  \node[collabel, above=8mm of param]    {Constraints};
  \node[collabel, above=8mm of ttft]     {SLOs};
  \node[collabel, above=8mm of parallel] {Levers};
...
Frameworks such as LangChain and LangGraph have emerged to coordinate these workflows and to orchestrate multi-agent systems. For instance, separate ``researcher,'' ``writer,'' and ``validator'' agents may collaborate on a task, introducing orchestration dependencies and new failure modes. Integration with external tools (APIs, calculators, search, internal services) further expands the attack surface and the observability requirements. As a result, LLMOps places strong emphasis on end-to-end tracing and structured logging of prompts, retrieved context, tool calls, and model outputs.
...
LLM systems introduce heightened ethical, safety, and security risks that differ from classical ML systems. Models may generate biased or toxic content \cite{vice}, hallucinate plausible but incorrect facts \cite{vice}, or expose sensitive information under certain conditions. Prompt injection illustrates a distinct vulnerability class, where adversarial inputs attempt to override system instructions or induce unsafe tool use. The key difference from traditional software security is that LLMs function as natural-language interpreters, making them susceptible to semantic attacks that bypass conventional input validation. Mitigations often combine alignment (e.g., RLHF), output filtering, sandboxed tool execution, retrieval governance, and human review for high-stakes tasks. For detailed coverage of security controls and threat models, see the earlier discussion in Section~\ref{sec:operational-challenges} and Chapter~\ref{ch:ethics}.
...
\medskip
\noindent By progressing through these parts, readers will develop both a conceptual framework for LLMOps and a practical toolkit for production deployment. Throughout, checklists, best-practice summaries, and the running \ishtar{} case study provide reusable guidance that can be adapted across domains and operational contexts. Table~\ref{tab:ch01_llmops_legend} maps each chapter to its operational purpose, helping readers navigate the book according to their specific interests and needs.
...
% Local styles built on the book-wide llm/* contract (macros.tex):
% the phase boxes are the four palette colors used semantically
% (blue = foundations, green = infrastructure / build, orange = delivery,
% purple = governance/quality). Solid arrows = lifecycle progression,
% dashed arrows = feedback loop into earlier phases.
\begin{tikzpicture}[
  phase/.style    ={llm/box, text width=30mm, minimum height=14mm, font=\footnotesize\bfseries, align=center},
  flow/.style     ={llm/arrow},
  feedback/.style ={llm/arrow/dashed},
  note/.style     ={llm/label, font=\footnotesize, align=center},
  partlabel/.style={font=\small\bfseries, text=black!85, anchor=west},
]

% ---- Top row in a matrix (clean alignment) ----
\matrix (top) [row sep=0mm, column sep=6mm] {
  \node[phase, llm/box/blue]   (fund)  {LLMOps\\Fundamentals\\(Ch.~\ref{ch:llmops-fundamentals})}; &
  \node[phase, llm/box/green]  (infra) {Infrastructure\\\& Environment\\(Ch.~\ref{ch:infra})};       &
  \node[phase, llm/box/orange] (cicd)  {CI/CD\\(Ch.~\ref{ch:cicd})};                                 &
  \node[phase, llm/box/purple] (mon)   {Monitoring\\(Ch.~\ref{ch:monitoring})};                      &
  \node[phase, llm/box/purple] (scale) {Scaling\\(Ch.~\ref{ch:scaling})};                            &
  \node[phase, llm/box/orange] (perf)  {Performance\\(Ch.~\ref{ch:performance})};                    \\
};

% ---- Part labels above top row ----
\node[partlabel] at ([yshift=10mm]fund.north) {Part I: Foundations};
\node[partlabel] at ([yshift=10mm]cicd.north) {Part II: Delivery Operations};
\node[partlabel] at ([yshift=10mm]perf.north) {Part III: Optimization};

% ---- Second row: RAG and Agents ----
\node[phase, llm/box/green, below=12mm of perf] (rag)  {RAG\\(Ch.~\ref{ch:rag})};
\node[phase, llm/box/green, right=6mm of rag]   (orch) {Multi-Agent\\(Ch.~\ref{ch:multiagent})};

% ---- Bottom row placed relative to second row ----
\node[phase, llm/box/orange, below=12mm of rag]   (test)   {Testing \&\\Robustness\\(Ch.~\ref{ch:testing})};
\node[phase, llm/box/purple, right=6mm of test]   (ethics) {Ethics \&\\Governance\\(Ch.~\ref{ch:ethics})};
\node[phase, llm/box/orange, right=6mm of ethics] (case)   {Case Study\\(Ch.~\ref{ch:case-study})};

% ---- Part label for bottom row ----
\node[partlabel] at ([yshift=10mm]test.north) {Part IV: Governance};

% ---- Top row flow ----
\draw[flow] (fund)  -- (infra);
\draw[flow] (infra) -- (cicd);
\draw[flow] (cicd)  -- (mon);
\draw[flow] (mon)   -- (scale);
\draw[flow] (scale) -- (perf);
...
% ---- Downward transition ----
\draw[flow] (rag.south)  -- (test.north);
\draw[flow] (orch.south) |- (ethics.north);

% ---- Bottom row flow ----
\draw[flow] (test)   -- (ethics);
\draw[flow] (ethics) -- (case);
...
\node[note, below=10mm of test.south, text width=60mm, anchor=north]
...
\midrule
% Swatches use the book-wide llm/* palette (macros.tex); colors are
% assigned to match Fig.~\ref{fig:ch01_llmops_lifecycle} so the legend
% reinforces the visual grouping rather than introducing a new key.
\swatch{llmblue} &
...
\begin{llmfigbox}
% Local styles built on the book-wide llm/* contract (macros.tex).
% Color groups segments by lifecycle role:
%   blue  = data path (ingestion, retrieval),
%   green = build / orchestration (agents, serving),
%   purple = governance (observability, ethics),
%   orange = quality (testing).
\begin{tikzpicture}[
  font=\footnotesize,
  node distance=8mm and 6mm,
  seg/.style  ={llm/box, minimum width=32mm, minimum height=13mm, font=\footnotesize, align=center},
  band/.style ={llm/box/gray, fill=black!2, draw=black!15, rounded corners=5pt},
  arrow/.style={llm/arrow},
  note/.style ={llm/label, font=\footnotesize, align=center},
  e2e/.style  ={llm/box/gray, fill=black!4, inner sep=4pt, align=left,
                font=\footnotesize, minimum height=0pt},
]

% Band container sized to hold both rows with the chapter labels.
\node[band, minimum width=0.95\linewidth, minimum height=60mm] (bandbox) {};

% Top row: data path + runtime (4 boxes)
\node[seg, llm/box/blue]                     (ing)   at ([xshift=-60mm,yshift=12mm]bandbox.center) {Ingestion\\\footnotesize (feeds, ETL)};
\node[seg, llm/box/blue,   right=of ing]     (rag)   {Retrieval\\\footnotesize (RAG)};
\node[seg, llm/box/green,  right=of rag]     (agent) {Agents\\\footnotesize (orchestration)};
\node[seg, llm/box/orange, right=of agent]   (serve) {Serving\\\footnotesize (GPU/TPU)};

% Bottom row: cross-cutting concerns (3 boxes, centered with spacing)
\node[seg, llm/box/purple]                   (obs)   at ([xshift=-40mm,yshift=-20mm]bandbox.center) {Observability\\\footnotesize (traces, SLOs)};
\node[seg, llm/box/orange, right=of obs]     (test)  {Testing\\\footnotesize (eval/robust)};
\node[seg, llm/box/purple, right=of test]    (eth)   {Ethics\\\footnotesize (governance)};

% Chapter labels above top row segments
...
% Chapter labels above bottom row segments
...
% Arrows from segments to chapter labels (solid book-wide arrow style).
\foreach \a/\b in {ing/ingC, rag/ragC, agent/agtC, serve/srvC, obs/obsC, test/tstC, eth/ethC}{
  \draw[arrow] (\a.north) -- (\b.south);
}

% End-to-End label tucked into the upper-right of the band.
\node[e2e, anchor=north east] at ([xshift=-4mm,yshift=-2mm]bandbox.north east)
  {\textbf{End-to-End:} \ishtar{} $\rightarrow$ Ch.~\ref{ch:case-study}};

% Mapping note anchored at the band's bottom-left to avoid clipping.
\node[note, anchor=north west, align=left] at ([xshift=4mm,yshift=-2mm]bandbox.south west)
...
\begin{figure}[tb]
\centering
\renewcommand{\LLMFigMaxWidth}{1.05\linewidth}
\begin{llmfigbox}
\begin{tikzpicture}[
  node distance=6mm and 10mm,
  source/.style ={llm/box/purple, ellipse, minimum width=24mm, minimum height=11mm, font=\footnotesize, align=center, text width=22mm},
  ingest/.style ={llm/box/blue,            minimum width=24mm, minimum height=11mm, font=\footnotesize, align=center, text width=22mm},
  query/.style  ={llm/box/purple, ellipse, minimum width=24mm, minimum height=11mm, font=\footnotesize\bfseries, align=center},
  rag/.style    ={llm/box/orange,          minimum width=24mm, minimum height=11mm, font=\footnotesize\bfseries, align=center},
  orch/.style   ={llm/box/orange,          minimum width=26mm, minimum height=11mm, font=\footnotesize\bfseries, align=center},
  agent/.style  ={llm/box/green,           minimum width=24mm, minimum height=10mm, font=\footnotesize, align=center},
  gpu/.style    ={llm/box/orange, fill=llmorange!22, minimum width=24mm, minimum height=11mm, font=\footnotesize\bfseries, align=center, text width=22mm},
  store/.style  ={llm/box/green,  cylinder, shape border rotate=90, aspect=0.3, minimum width=22mm, minimum height=15mm, font=\footnotesize, align=center},
  respond/.style={llm/box/gray,   ellipse, minimum width=24mm, minimum height=11mm, font=\footnotesize\bfseries, align=center, text width=22mm},
  band/.style   ={llm/lane, minimum height=8mm, inner sep=4pt, font=\footnotesize\bfseries, text=black!75},
]
  % Top telemetry band
  \node[band, minimum width=160mm] (topband) {Observability \& Telemetry};

  % Left column: data sources (feed into ingestion)
  \node[source, below=10mm of topband.south west, xshift=10mm]  (src1) {News wires\\\& reports};
  \node[source, below=4mm of src1]                               (src2) {NGO\\bulletins};
  \node[source, below=4mm of src2]                               (src3) {Social media\\(vetted)};

  % Ingestion agents column
  \node[ingest, right=12mm of src1] (in1) {Ingestion\\Agent 1};
  \node[ingest, right=12mm of src2] (in2) {Ingestion\\Agent 2};
  \node[ingest, right=12mm of src3] (in3) {Ingestion\\Agent 3};

  % Vector database (downstream of ingestion)
  \node[store, below right=18mm and 6mm of in3] (vdb) {Vector\\Database};

  % Center: journalist query -> RAG -> Orchestrator
  \node[query, right=30mm of in1, yshift=6mm] (jq)  {Journalist\\Query};
  \node[rag,   below=6mm of jq]                (rag) {RAG\\Retrieval};
  \node[orch,  below=10mm of rag]              (orch){Orchestrator};

  % Agent row (under orchestrator)
  \node[agent, below=8mm of orch, xshift=-18mm] (sum) {Summarizer};
  \node[agent, right=6mm of sum]                 (fc)  {Fact-Checker};
  \node[agent, right=6mm of fc]                  (ref) {Refiner};

  % GPU cluster (right of orchestrator)
  \node[gpu, right=18mm of orch, yshift=4mm]  (gpu1) {GPU\\Node 1};
  \node[gpu, below=4mm of gpu1]                (gpu2) {GPU\\Node 2};

  % Response
  \node[respond, right=10mm of gpu1, yshift=-4mm] (resp) {Response\\+ Citations};

  % Bottom telemetry band
  \node[band, minimum width=160mm, below=28mm of orch] (bottomband) {Traces, Metrics, Logs};

  % -------- Arrows --------
  % Sources to ingestion agents
  \draw[llm/arrow] (src1) -- (in1);
  \draw[llm/arrow] (src2) -- (in2);
  \draw[llm/arrow] (src3) -- (in3);

  % Ingestion agents to vector DB
  \draw[llm/arrow] (in1.south) to[out=-90,in=90] (vdb.north);
  \draw[llm/arrow] (in2.south) to[out=-90,in=100] (vdb.north);
  \draw[llm/arrow] (in3.south) to[out=-90,in=110] (vdb.north);

  % Query flow
  \draw[llm/arrow] (jq)   -- (rag);
  \draw[llm/arrow] (rag)  -- (orch);

  % Vector DB feeds RAG retrieval
  \draw[llm/arrow] (vdb.east) to[out=0,in=200] (rag.west);

  % Orchestrator fan-out to agents
  \draw[llm/arrow] (orch) -- (sum);
  \draw[llm/arrow] (orch) -- (fc);
  \draw[llm/arrow] (orch) -- (ref);

  % Agents loop back to orchestrator (dashed = feedback)
  \draw[llm/arrow/dashed] (sum.north east) to[bend right=20] (orch.south west);
  \draw[llm/arrow/dashed] (fc.north)       -- (orch.south);
  \draw[llm/arrow/dashed] (ref.north west) to[bend left=20]  (orch.south east);

  % Orchestrator to GPU cluster
  \draw[llm/arrow] (orch) -- (gpu1);
  \draw[llm/arrow] (orch) -- (gpu2);

  % GPU cluster to response
  \draw[llm/arrow] (gpu1) -- (resp);
  \draw[llm/arrow] (gpu2.east) to[bend right=10] (resp.west);

  % Observability taps (dashed) from top band to major stages
  \foreach \t in {rag, orch, gpu1}
    \draw[llm/arrow/dashed] (topband.south) to[out=-90,in=90] (\t.north);

  % Telemetry taps (dashed) from bottom band up to major stages
  \foreach \t in {vdb, orch, sum, fc, ref}
    \draw[llm/arrow/dashed, <-] (\t.south) to[out=-90,in=90] (bottomband.north);
\end{tikzpicture}
\end{llmfigbox}
\renewcommand{\LLMFigMaxWidth}{1.15\linewidth}
...
Serving is supported by a GPU-backed cluster running optimized inference engines such as vLLM~\cite{Kwon2023vLLM} or Hugging Face Text Generation Inference (TGI)~\cite{tgi-whitepaper}. Request batching, caching, and (where needed) model parallelism reduce latency and support concurrency. LLMOps practices govern utilization, scaling, reliability, and cost-efficiency.
...
\begin{llmfigbox}
% Local styles built on the book-wide llm/* contract (macros.tex).
% The four input categories use distinct palette hues so the figure
% remains legible in grayscale. Solid arrows show ingestion flow.
\begin{tikzpicture}[
  font=\footnotesize,
  node distance=10mm and 15mm,
  hub/.style       ={llm/box/gray, fill=black!2, draw=black!15, rounded corners=5pt},
  bubble/.style    ={llm/box, circle, minimum size=18mm, inner sep=2pt, font=\footnotesize, align=center},
  bubbleBlue/.style  ={bubble, fill=llmblue!12,   draw=llmblue!70},
  bubbleGreen/.style ={bubble, fill=llmgreen!12,  draw=llmgreen!70},
  bubblePurple/.style={bubble, fill=llmpurple!12, draw=llmpurple!70},
  bubbleOrange/.style={bubble, fill=llmorange!12, draw=llmorange!70},
  arrow/.style     ={llm/arrow},
  note/.style      ={llm/label, font=\footnotesize, align=left},
  funnel/.style    ={llm/box/orange, minimum width=24mm, minimum height=10mm,
                     font=\footnotesize\bfseries, align=center},
  journalist/.style={llm/box/gray, minimum height=12mm, inner sep=4pt,
                     align=center, font=\footnotesize},
]

% Hub container positioned first (gray neutral wrapper).
\node[hub, minimum width=75mm, minimum height=52mm] (group) {};
\node[font=\footnotesize\itshape, text=black!60] at ([yshift=-6pt]group.north) {Curated external evidence (ingested and vetted)};

% Position bubbles relative to hub center in a 2x2 grid.
\node[bubbleBlue]   (news)   at ([xshift=-18mm,yshift=8mm]group.center)  {News wires\\\& reports};
\node[bubbleGreen]  (ngo)    at ([xshift=18mm,yshift=8mm]group.center)   {NGO bulletins};
\node[bubblePurple] (social) at ([xshift=-18mm,yshift=-8mm]group.center) {Social media\\(vetted)};
\node[bubbleOrange] (health) at ([xshift=18mm,yshift=-8mm]group.center)  {Public health\\\& infrastructure};

% Connection lines between bubbles (subtle, indicate kinship not flow).
\draw[black!15, line width=1.5pt] (news)   -- (ngo);
\draw[black!15, line width=1.5pt] (news)   -- (social);
\draw[black!15, line width=1.5pt] (social) -- (health);
\draw[black!15, line width=1.5pt] (ngo)    -- (health);
...
\node[journalist, right=of funnel] (journalist)
      {\textbf{Journalist queries}\\\footnotesize (questions and tasks)};

% Arrows from bubbles to funnel
\draw[arrow] (news.east)   -- (funnel.west);
\draw[arrow] (ngo.east)    -- (funnel.west);
\draw[arrow] (social.east) -- (funnel.west);
\draw[arrow] (health.east) -- (funnel.west);

% Arrow from funnel to journalist
\draw[arrow] (funnel.east) -- (journalist.west);
...
\begin{itemize}
    ...
    \item \textbf{A/B testing and canary releases:} Test new prompts on a small percentage of traffic or internal users, comparing metrics against control prompts.
...
\begin{codebox}{Versioned prompt template structure with semantic versioning, few-shot examples, and regression test references. Treating prompts as versioned artifacts with changelogs and test suites enables systematic A/B testing, rollback on quality regressions, and auditable prompt evolution across production deployments.}{lst:ch01_prompt_template}
\begin{minted}{yaml}
...
Retrieval-Augmented Generation (RAG) grounds outputs in external evidence. It addresses the limited knowledge cutoff of trained models and mitigates hallucinations by injecting relevant documents into prompts at query time.
...
\begin{codebox}{RAG pipeline configuration covering embedding model selection, chunking strategy, HNSW vector store tuning, hybrid dense-sparse retrieval with reranking, and context assembly with SLO thresholds. This end-to-end configuration enables reproducible retrieval quality across environments and provides operational guardrails for latency, recall, and faithfulness in production.}{lst:ch01_rag_configuration}
\begin{minted}{yaml}
...
\begin{itemize}
    ...
    \item \textbf{Telemetry:} Track TTFT, throughput (tokens/s), and cost per 1k tokens. Right-size clusters to balance performance and cost.
...
\begin{itemize}
    \item \textbf{Automated metrics:} ROUGE/BLEU for summarization, EM/F1 for QA. LLM-as-a-judge scoring for relevance, coherence, and factuality.
...
\subsubsection{Example}
In \ishtar{}, every answer is logged with source provenance and factuality scores. Prompt injection attempts (e.g., “Ignore previous instructions”) are detected in logs and flagged. Dips in faithfulness or satisfaction trigger investigation and rollback.  
...
Meta’s chief AI scientist even remarked that the model was being ``misused'' and shut it down, quipping that it was no longer possible to ``have fun by casually misusing it'' \parencite{Heaven2022Galactica}. The PR fallout highlighted that releasing an LLM without robust hallucination mitigation and staged rollout (e.g., limited beta with experts, retrieval integration, or clear user education) is irresponsible. Galactica showed that even very capable models can be worse than useless if operated without alignment, guardrails, and monitoring.  

\subsection*{Failure Case -- Bing Chat ``Sydney'' (Microsoft, 2023)}
In early 2023, Microsoft integrated a GPT-4-powered chat mode into Bing search, codenamed \emph{Sydney}. Users rapidly discovered that Sydney could be prompted to reveal its hidden system instructions and internal developer notes---a classic prompt injection vulnerability \parencite{ibm2023,theverge2023}. By simply asking it to ``ignore previous instructions'' and then to display its initial system prompt, users obtained the rules governing Sydney’s behavior, including its code name and formatting policies.  
...
The key lesson is that operational excellence can substitute for sheer model size. By focusing on its domain (conversational personas) and iterating rapidly, Character.AI delivered a popular service with modest models but exceptional infrastructure and feedback loops. Table~\ref{tab:ch01_early_llmops_cases} summarizes these cases alongside other early LLM deployments, extracting common lessons about operational discipline and risk management.  
...
Figure~\ref{fig:ch01_llmops_quick_checklist} provides a concise checklist of key monitoring areas that span these chapters, offering practitioners a quick reference for maintaining production reliability.

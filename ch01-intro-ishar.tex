...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[
    ...
    trace/.style={llm/arrow/dashed, color=llmpurple!70!black, dash pattern=on 1pt off 1.5pt},
  ]
    ...
    % Tracing taps every threat (dotted purple, no arrowhead on top end)
    \foreach \t in {inj, bypass, exfil, misuse, leak}
      \draw[trace] (\t.south) -- (\t.south |- tracing.north);
    ...
    \node[llm/box/gray, align=left, font=\scriptsize,
          anchor=north east, inner sep=3pt, text width=44mm]
          (legend) at (17.3,8.8)
      {\textbf{Legend}\\
       \tikz[baseline=-0.6ex]{\draw[flow]  (0,0) -- (6mm,0);} data flow\\
       \tikz[baseline=-0.6ex]{\draw[maps]  (0,0) -- (6mm,0);} surface $\rightarrow$ threat\\
       \tikz[baseline=-0.6ex]{\draw[mits]  (0,0) -- (6mm,0);} threat $\rightarrow$ control\\
       \tikz[baseline=-0.6ex]{\draw[trace] (0,0) -- (6mm,0);} audit tap};
  \end{tikzpicture}
  \end{llmfigbox}

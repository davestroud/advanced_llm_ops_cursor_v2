...
\begin{lstlisting}[
  float=htbp,
  style=springerfloat,
  caption={Standardized observability telemetry schema capturing the full semantic path of an LLM request, from prompt and retrieval metadata through tool invocations to output quality signals and cost estimates. This structured trace format enables end-to-end debugging, governance auditing, and cost attribution across production LLM pipelines.},
...
\begin{lstlisting}[
  float=htbp,
  style=springerfloat,
  caption={Versioned tool schema with security controls for a source-verification tool, including authentication requirements, rate limiting, sandboxed execution, and contract test references. This defense-in-depth approach ensures that LLM-invoked tools operate within well-defined security boundaries while maintaining backward compatibility across schema versions.},
...
\begin{lstlisting}[
  float=htbp,
  style=springerfloat,
  caption={RAG pipeline configuration covering embedding model selection, chunking strategy, HNSW vector store tuning, hybrid dense-sparse retrieval with reranking, and context assembly with SLO thresholds. This end-to-end configuration enables reproducible retrieval quality across environments and provides operational guardrails for latency, recall, and faithfulness in production.},

...
\node[box, fill=llmblue!15, minimum width=30mm, minimum height=11mm] (ui) {User /\\Client};

\node[box, fill=llmgreen!15, minimum width=34mm, minimum height=13mm, below=of ui] (orch) {Orchestrator\\(routing, budgets, policy)};

\node[box, fill=llmorange!15, minimum width=30mm, minimum height=11mm, left=of orch] (retr) {Retrieval\\Agent};
\node[box, fill=llmpurple!15, minimum width=30mm, minimum height=11mm, right=of orch] (synth) {Synthesis\\Agent};

\node[box, fill=llmpurple!15, minimum width=30mm, minimum height=11mm, below=of retr] (ver) {Verification\\Agent};
\node[box, fill=llmorange!15, minimum width=30mm, minimum height=11mm, below=of synth] (safe) {Safety\\Agent};

\node[box, fill=llmgreen!15, minimum width=34mm, minimum height=11mm, below=of orch, yshift=-14mm] (mem) {Shared Memory\\(episodic + semantic)};

\node[box, fill=llmgreen!15, minimum width=34mm, minimum height=11mm, below=of mem] (tools) {External Tools\\(search, DBs, APIs)};
...
\begin{tikzpicture}[
  ...
  agent/.style={box, fill=llmblue!15},
  ctrl/.style={box, fill=llmgreen!15},
  decision/.style={diamond, draw=none, fill=llmorange!15, aspect=2.8, inner sep=4pt, align=center, font=\small\bfseries},
  arrow/.style={-{Latex}, line width=1.2pt, color=black!70},
...
\node[box, fill=llmblue!15, minimum width=32mm, minimum height=12mm] (ingest) {Ingestion};
\node[box, fill=llmorange!15, minimum width=32mm, minimum height=12mm, right=of ingest] (retr) {Retrieval};
\node[box, fill=llmpurple!15, minimum width=32mm, minimum height=12mm, right=of retr] (synth) {Synthesis};

% Bottom row - increased spacing
\node[box, fill=llmpurple!15, minimum width=32mm, minimum height=12mm, below=of synth] (verify) {Verification};
\node[box, fill=llmorange!15, minimum width=32mm, minimum height=12mm, left=of verify] (safety) {Safety};

% Orchestrator - positioned bottom-left, well clear of other nodes
\node[box, fill=llmgreen!15, minimum width=36mm, minimum height=14mm, below=of ingest, yshift=-12mm] (orch) {Orchestrator\\(router + budgets)};

% Delivery node - positioned to receive from Safety, aligned with bottom row
\node[box, fill=llmgreen!15, minimum width=32mm, minimum height=12mm, below=of safety, yshift=-8mm] (deliver) {Delivery};
...
\node[state, fill=llmgreen!15, draw=llmgreen!60] (closed) {Closed\\{\footnotesize (normal)}};
\node[state, fill=llmorange!15, draw=llmorange!60, right=of closed] (open) {Open\\{\footnotesize (rejecting)}};
\node[state, fill=llmorange!15, draw=llmorange!60, below right=25mm and 20mm of closed] (half) {Half-Open\\{\footnotesize (probing)}};

\draw[arrow, llmgreen!70!black] (closed) -- node[above, font=\footnotesize]{failure threshold exceeded} (open);
\draw[arrow, llmorange!70!black] (open) -- node[right, font=\footnotesize, text width=2.5cm, align=center]{cooldown\\expires} (half);
\draw[arrow, llmorange!70!black] (half) -- node[below left, font=\footnotesize]{probe succeeds} (closed);
\draw[arrow, llmorange!70!black] (half) to[bend right=30] node[below right, font=\footnotesize]{probe fails} (open);

% Self-loops
\draw[arrow, llmgreen!50!black] (closed) to[loop above, looseness=5] node[above, font=\footnotesize]{success / minor failure} (closed);
\draw[arrow, llmorange!50!black] (open) to[loop above, looseness=5] node[above, font=\footnotesize]{reject / fallback} (open);
\end{tikzpicture}
\end{llmfigbox}

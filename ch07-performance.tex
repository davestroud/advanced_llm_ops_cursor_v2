...
\begin{lstlisting}[
  float=htbp,
  style=springerfloat,
  caption={Mixed-precision quantization configuration with calibration data, quality validation gates, and per-layer fallback policies. This configuration balances memory reduction with accuracy preservation, enabling deployment of large models on production hardware.},
...
\begin{lstlisting}[
  float=htbp,
  style=springerfloat,
  caption={Comprehensive caching configuration for response and embedding caching with Redis and pgvector backends, TTL policies, eviction strategies, and cache warming schedules. This multi-layer caching approach reduces both latency and cost by avoiding redundant LLM calls and embedding computations.},
...
\begin{lstlisting}[
  ...
  style=springerfloat,
  language=Python,
  caption={GPU profiling script using NVIDIA Nsight Systems for identifying inference bottlenecks, measuring kernel execution times, and comparing baseline versus optimized performance. This script automates the profiling workflow and generates structured JSON reports suitable for CI gate integration.},
...
\begin{lstlisting}[
  float=htbp,
  style=springerfloat,
  caption={Production vLLM inference engine configuration covering continuous batching, PagedAttention, KV-cache management, tensor parallelism, and Prometheus monitoring. This configuration optimizes throughput and latency for serving large language models at scale.},

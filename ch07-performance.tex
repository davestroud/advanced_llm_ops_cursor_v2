...
Fig.~\ref{fig:ch07_opt_pyramid} illustrates the four-layer optimization framework as a stacked pyramid, where broader, lower-effort techniques form the foundation and progressively specialized techniques build on top. Figure~\ref{fig:ch07_opt_gains_callout} summarizes practical gains achievable from common optimization techniques, while Figure~\ref{fig:ch07_latency_before_after} demonstrates the cumulative impact on latency percentiles.
...
\fill[llmblue!20, draw=llmblue!60, line width=0.8pt, rounded corners=2pt]
  (-5.0, 0) -- (5.0, 0) -- (4.0, 1.6) -- (-4.0, 1.6) -- cycle;
\fill[llmgreen!20, draw=llmgreen!60, line width=0.8pt, rounded corners=2pt]
  (-4.0, 1.8) -- (4.0, 1.8) -- (3.0, 3.4) -- (-3.0, 3.4) -- cycle;
\fill[llmorange!20, draw=llmorange!60, line width=0.8pt, rounded corners=2pt]
  (-3.0, 3.6) -- (3.0, 3.6) -- (2.0, 5.2) -- (-2.0, 5.2) -- cycle;
\fill[llmpurple!20, draw=llmpurple!60, line width=0.8pt, rounded corners=2pt]
  (-2.0, 5.4) -- (2.0, 5.4) -- (1.2, 6.8) -- (-1.2, 6.8) -- cycle;

% Layer labels (centered in each trapezoid)
\node[font=\small\bfseries, text=llmblue!80!black] at (0, 0.8)
  {Layer 1: Infrastructure \& Hardware Selection};
\node[font=\small\bfseries, text=llmgreen!80!black] at (0, 2.6)
  {Layer 2: Model Compression \& Quantization};
\node[font=\small\bfseries, text=llmorange!80!black] at (0, 4.4)
  {Layer 3: Inference Engine Optimization};
\node[font=\small\bfseries, text=llmpurple!80!black] at (0, 6.1)
  {Layer 4: Application-Level};

% Annotations (right side)
\node[anchor=west, text width=4.5cm, font=\footnotesize, text=llmblue!70!black]
  at (5.3, 0.8) {GPU/TPU selection, cloud instance sizing, multi-GPU topology};
\node[anchor=west, text width=4.5cm, font=\footnotesize, text=llmgreen!70!black]
  at (4.3, 2.6) {INT8/FP8 quantization, pruning, distillation, LoRA adapters};
\node[anchor=west, text width=4.5cm, font=\footnotesize, text=llmorange!70!black]
  at (3.3, 4.4) {Kernel fusion, FlashAttention, KV-cache paging, continuous batching};
\node[anchor=west, text width=4.5cm, font=\footnotesize, text=llmpurple!70!black]
  at (2.3, 6.1) {Prompt optimization, RAG compression, caching, speculative decoding};

% Arrow indicating effort/impact
\draw[llm/arrow, color=llmgray] (-6.0, 0) -- (-6.0, 7.0);
\node[llm/label, rotate=90, font=\footnotesize\bfseries, text=llmgray] at (-6.5, 3.5) {Increasing specialization \& effort};
\end{tikzpicture}
\end{llmfigbox}
\caption{Four-layer optimization pyramid organizes techniques by scope and effort. The base layer (infrastructure) provides the broadest leverage with the least model-specific effort; each successive layer adds more targeted optimizations. Teams should build upward: ensure the lower layers are well-tuned before investing in higher-layer techniques, as gains compound multiplicatively across layers.}
\label{fig:ch07_opt_pyramid}
\end{figure}
...
\renewcommand{\labelitemi}{\textcolor{llmblue!80!black}{$\bullet$}}
\begin{minipage}[t]{0.95\linewidth}
\colorbox{llmblue!10}{\parbox{0.98\linewidth}{\vspace{2mm}\textbf{\textcolor{llmblue!90!black}{Quantization (INT8/FP8/4-bit):}} $\sim$2$\times$ reduction in parameter memory; 1.3--2.0$\times$ higher tokens/s on supported hardware (model-dependent) \cite{Xiao2023SmoothQuant,Frantar2023GPTQ,Lin2023AWQ,NVIDIA2023Hopper}.\vspace{2mm}}}\\[2mm]
\colorbox{llmgreen!10}{\parbox{0.98\linewidth}{\vspace{2mm}\textbf{\textcolor{llmgreen!90!black}{Operator fusion \& kernel tuning:}} 10--30\% end-to-end speedups by reducing memory traffic and kernel-launch overhead \cite{Dao2022FlashAttention,Dao2023FlashAttention2}.\vspace{2mm}}}\\[2mm]
\colorbox{llmorange!10}{\parbox{0.98\linewidth}{\vspace{2mm}\textbf{\textcolor{llmorange!90!black}{Dynamic/continuous batching:}} 3--8$\times$ more tokens/s at steady state with a small ($<\!20$\,ms) batching window, especially under bursty traffic \cite{Kwon2023vLLM}.\vspace{2mm}}}\\[2mm]
\colorbox{llmpurple!10}{\parbox{0.98\linewidth}{\vspace{2mm}\textbf{\textcolor{llmpurple!90!black}{KV-cache engineering (paged KV, prefix reuse):}} 1.5--3.0$\times$ larger effective batches and reduced fragmentation under long contexts \cite{Kwon2023vLLM}.\vspace{2mm}}}\\[2mm]
\colorbox{llmpurple!10}{\parbox{0.98\linewidth}{\vspace{2mm}\textbf{\textcolor{llmpurple!90!black}{Speculative decoding:}} 1.4--2.0$\times$ lower latency on long generations via draft+verify \cite{Chen2023SpeculativeSampling}.\vspace{2mm}}}\\[2mm]
\colorbox{llmgreen!10}{\parbox{0.98\linewidth}{\vspace{2mm}\textbf{\textcolor{llmgreen!90!black}{Response \& embedding caches:}} can cut GPU load roughly in proportion to cache hit rate and reduce tail latency during bursts; validate correctness with TTLs, versioning, and guardrails \cite{langchain-cache}.\vspace{2mm}}}
\end{minipage}
};
\end{tikzpicture}
\end{llmfigbox}
...
\textbf{\large Mixed-precision quantization recipe}\\[4mm]
\begin{minipage}[t]{0.95\linewidth}
\colorbox{llmblue!10}{\parbox{0.98\linewidth}{\vspace{2mm}\textbf{\textcolor{llmblue!90!black}{Precision map (per layer family):}} attention in FP8 weights/activations with INT8 KV-cache; FFN in INT8 weights and FP16 activations; embeddings and output projection in FP16 to preserve final accuracy \cite{Xiao2023SmoothQuant,Frantar2023GPTQ,Lin2023AWQ,NVIDIA2023Hopper}.\vspace{2mm}}}\\[2mm]
\colorbox{llmorange!10}{\parbox{0.98\linewidth}{\vspace{2mm}\textbf{\textcolor{llmorange!90!black}{Calibration:}} representative dataset of $\sim$512 samples; choose SmoothQuant, GPTQ, or AWQ; preserve outliers above 3$\sigma$; per-channel scaling with $\alpha\!=\!0.5$ for SmoothQuant.\vspace{2mm}}}\\[2mm]
\colorbox{llmgreen!10}{\parbox{0.98\linewidth}{\vspace{2mm}\textbf{\textcolor{llmgreen!90!black}{Quality gates:}} validate against a golden set on perplexity, task accuracy, and faithfulness; reject if perplexity rises $>\!5$\%, task accuracy drops $>\!2$\%, or faithfulness drops $>\!1$\%; fall problem layers back to FP16.\vspace{2mm}}}\\[2mm]
\colorbox{llmpurple!10}{\parbox{0.98\linewidth}{\vspace{2mm}\textbf{\textcolor{llmpurple!90!black}{Runtime tuning:}} enable Tensor Cores; turn on FP8 Tensor Cores on H100; INT8 KV-cache with per-channel quantization; static activation quantization for predictable latency.\vspace{2mm}}}\\[2mm]
\colorbox{llmgray!10}{\parbox{0.98\linewidth}{\vspace{2mm}\textbf{\textcolor{llmgray!90!black}{Targets \& shipping format:}} aim for $\sim$50\% memory reduction, 1.5$\times$ speedup, and $\geq\!98$\% accuracy retention; export to ONNX, TensorRT, or TorchScript with a versioned tag (\texttt{quantized\_v\textit{x.y.z}}).\vspace{2mm}}}
\end{minipage}
};
\end{tikzpicture}
\end{llmfigbox}
\caption{Mixed-precision quantization recipe organized as five decisions: precision map, calibration, quality gates, runtime tuning, and shipping format. Concrete thresholds (perplexity $\leq\!1.05\times$, task accuracy $\geq\!0.98\times$, faithfulness $\geq\!0.99\times$) act as the merge gate; layers that fail individually fall back to FP16, preserving accuracy on the few sensitive sublayers without sacrificing the throughput win on the rest of the model.}
\label{fig:ch07_quantization_callout}
\end{figure}
...
\addlinespace[2pt]
\rowcolor{orange!10}
4-bit & \textcolor{orange!70!black}{\textbf{$\sim$75\% vs.\ FP16}} & High & Low--Moderate, task dependent & Supported via software toolkits (e.g., GPTQ, bitsandbytes) on NVIDIA GPUs; performance varies by architecture. \\
\addlinespace[2pt]
\rowcolor{blue!8}
...
\subsection{Efficient Fine-Tuning}\index{fine-tuning!parameter-efficient}
...
\rowcolors{2}{gray!8}{white}
\begin{xltabular}{\linewidth}{@{}>{\raggedright\arraybackslash\bfseries}p{2.3cm}
...
% A100
\addplot+[
    fill=llmblue!70, draw=llmblue, line width=0.6pt,
    every node near coord/.append style={
        fill=white, inner sep=1.5pt, rounded corners=2pt,
        draw=llmblue, line width=0.3pt
    }
...
% H100
\addplot+[
    fill=llmorange!70, draw=llmorange, line width=0.6pt,
    every node near coord/.append style={
        fill=white, inner sep=1.5pt, rounded corners=2pt,
        draw=llmorange, line width=0.3pt
    }
...
\end{tikzpicture}
\end{llmfigbox}
\caption{Quantization can roughly double throughput on A100/H100 relative to FP16, turning an accuracy-preserving compression into a direct cost lever. FP8 is native on H100; 4-bit results vary by toolkit and model architecture, so always benchmark before committing to a format.}
\label{fig:ch07_quant_throughput_bars}
\end{figure}

Table~\ref{tab:ch07_mem_vs_precision} shows how precision selection affects memory requirements and deployment feasibility for different model sizes, while Figure~\ref{fig:ch07_memory_cheatsheet} provides a practical sizing cheat-sheet for inference memory planning.
...
\begin{llmfigbox}
\begin{tikzpicture}[font=\small]
\node[llm/box/gray, align=left, inner sep=4mm, text width=0.92\linewidth, font=\footnotesize]{%
...
\addlinespace[2pt]
TGI (FP16, dyn.\ batch)  & 180  & 520 & 28  & Dynamic batching; ONNX Runtime integration \\
\addlinespace[2pt]
vLLM (FP16, PagedAttn)   & 320  & 540 & 21  & Continuous batching; page-based KV cache; strong on long contexts \\
\addlinespace[2pt]
TensorRT-LLM (INT8/FP8)  & 420  & 410 & 18  & Fused GPU kernels; Tensor Core utilization; requires engine compilation step \\
\bottomrule
\end{tabularx}
\begin{tablenotes}[flushleft]\footnotesize
\item \emph{Setup:} 30B-parameter model, context length 4k, batch window 10\,ms. A100-80GB and H100-80GB GPUs showed $\sim$1.3--1.8$\times$ speedup for TensorRT-LLM vs.\ FP16 baseline; vLLM excelled on long contexts due to PagedAttention \cite{Kwon2023vLLM,NVIDIA2023Hopper}.
\end{tablenotes}
\end{threeparttable}
\end{table}

\FloatBarrier
\section{System-Level Optimization}\index{optimization!system-level}
...
\begin{axis}[
  ...
  ymin=0, ymax=1.05,
  xtick={1,2,4,8,16,32},
  legend style={at={(0.5,1.02)},anchor=south,draw=none,fill=none,font=\footnotesize},
]
\addplot+ coordinates {
...
From a production standpoint, dynamic batching is a low-hanging fruit for optimization: frameworks like TGI~\cite{tgi-whitepaper} and DeepSpeed-Inference provide it out-of-the-box. Continuous batching (token-level interleaving) is more complex but very powerful for large models under heavy multi-user load. \ishtar{} initially implemented simple request grouping, then moved to vLLM’s dynamic token batching to handle surges of simultaneous queries without exploding latency.
...
Figure~\ref{fig:ch07_caching_callout} summarizes the three-layer caching architecture used in production: response cache for completed answers, embedding cache for vectorization output, and prefix cache for shared prompt prefixes, plus the warming and observability that make hit rates real.
...
\textbf{\large Three-layer LLM caching architecture}\\[4mm]
\begin{minipage}[t]{0.95\linewidth}
\colorbox{llmblue!10}{\parbox{0.98\linewidth}{\vspace{2mm}\textbf{\textcolor{llmblue!90!black}{Response cache (Redis):}} key on \texttt{(prompt\_hash, model\_version, temperature, max\_tokens, template\_id)} with SHA-256; LFU eviction at 10\,GB / 1\,M entries; LZ4 compression above 1\,KB; per-query-type TTL (15\,min for breaking news, 30\,min fact-checking, 24\,h historical) \cite{langchain-cache}.\vspace{2mm}}}\\[2mm]
\colorbox{llmgreen!10}{\parbox{0.98\linewidth}{\vspace{2mm}\textbf{\textcolor{llmgreen!90!black}{Embedding cache (pgvector):}} key on \texttt{(content\_hash, embedding\_model\_id, version)}; HNSW index ($m\!=\!16$, $ef\!=\!200$) at 384-d; per-content-type TTL (7\,d news, 90\,d reference); invalidate on model or content update.\vspace{2mm}}}\\[2mm]
\colorbox{llmpurple!10}{\parbox{0.98\linewidth}{\vspace{2mm}\textbf{\textcolor{llmpurple!90!black}{Prefix cache (Redis):}} cache the first 512 tokens of shared prompts (system prompt + boilerplate) with a 2\,h TTL so the inference engine can reuse PagedAttention pages across requests \cite{Kwon2023vLLM}.\vspace{2mm}}}\\[2mm]
\colorbox{llmorange!10}{\parbox{0.98\linewidth}{\vspace{2mm}\textbf{\textcolor{llmorange!90!black}{Cache warming:}} replay a popular-queries dataset on a nightly cron (\texttt{0 2 * * *}, batches of 100) so cold-start traffic at peak hours hits a warm cache rather than the model.\vspace{2mm}}}\\[2mm]
\colorbox{llmgray!10}{\parbox{0.98\linewidth}{\vspace{2mm}\textbf{\textcolor{llmgray!90!black}{Observability \& alerts:}} track hit/miss rate, latency reduction, and cost savings as first-class metrics; alert when hit rate falls below 10\% (warming or keying broken) or the cache itself adds $>\!5$\,ms (backend hot-spot or network issue).\vspace{2mm}}}
\end{minipage}
};
\end{tikzpicture}
\end{llmfigbox}
\caption{Three-layer LLM caching architecture: a response cache amortizes whole-answer cost, an embedding cache eliminates redundant vectorization, and a prefix cache lets the inference engine reuse KV pages for shared system prompts. Cache warming converts cold-start traffic into cache hits at peak hours; observability turns the cache from an opaque accelerator into a measurable cost and latency lever, with alert thresholds chosen so a low hit rate or a slow backend immediately surfaces.}
\label{fig:ch07_caching_callout}
\end{figure}
...
\FloatBarrier
\section{Prompt Optimization}\index{prompt optimization}
...
\paragraph{Cost analysis of context compression.}\index{context compression!cost}
The cost impact of context compression is substantial and often underappreciated. Consider a typical RAG query that retrieves four passages totaling 4{,}000 tokens. Compressing these to 1{,}000 tokens yields a 75\% reduction in input tokens per request. At typical API pricing of \$0.01 per 1{,}000 input tokens, this saves \$0.03 per request---which at 100{,}000 daily requests translates to roughly \$3{,}000/month in input token costs alone. Self-hosted deployments benefit analogously through reduced prefill compute and the ability to serve larger batches within fixed GPU memory.

\paragraph{When to compress vs.\ retrieve fewer chunks.}
Compression is most valuable when (i) retrieved passages are long but only partially relevant, (ii) the query requires evidence from many sources, or (iii) the model struggles with ``lost in the middle'' effects in long contexts. In contrast, simply retrieving fewer chunks is preferable when (i) retrieval precision is already high, (ii) each chunk is short and tightly focused, or (iii) the compression step itself introduces unacceptable latency. A practical heuristic: if your average passage length exceeds 500 tokens and you retrieve more than four passages, compression is likely beneficial.

\paragraph{Trade-offs: compression quality.}
Context compression is not lossless. Abstractive summarization may lose nuanced qualifiers, specific numbers, or temporal details that matter for answer correctness. Empirical studies with tools such as LLMLingua\index{LLMLingua} and RECOMP\index{RECOMP} show that aggressive compression (above 4$\times$) can degrade faithfulness by 5--15\% on fact-intensive benchmarks, while moderate compression (2--3$\times$) typically preserves answer quality within 1--3\% of the uncompressed baseline. Extractive approaches (selecting key sentences rather than rewriting) tend to be safer for domains like journalism where factual precision is paramount.

\paragraph{Practical tools.}
Several open-source tools support context compression in production RAG pipelines:
\begin{itemize}[leftmargin=1.5em, itemsep=2pt]
  \item \textbf{LLMLingua / LongLLMLingua}: prompt compression via token pruning guided by perplexity, achieving 2--10$\times$ compression with minimal quality loss.
  \item \textbf{RECOMP}: trains specialized compressors (extractive or abstractive) that produce query-focused summaries of retrieved passages.
  \item \textbf{LangChain ContextualCompressionRetriever}: integrates compression into the retrieval pipeline, supporting pluggable compressor backends.
\end{itemize}
For \ishtar{}, we found that a lightweight extractive compressor (selecting the top 3--5 sentences per passage by query relevance) provided the best balance of compression ratio, latency, and faithfulness.
...
\begin{codebox}{GPU profiling script using NVIDIA Nsight Systems for identifying inference bottlenecks, measuring kernel execution times, and comparing baseline versus optimized performance. This script automates the profiling workflow and generates structured JSON reports suitable for CI gate integration.}{lst:ch07_profiling_script}
\begin{minted}{python}
...
def run_nsight_systems(command, output_file, duration_seconds=30):
    nsys_cmd = [
        "nsys", "profile",
    ...
    subprocess.run(nsys_cmd, capture_output=True, text=True, check=True)
    return output_file
...
def analyze_nsys_report(report_file: str) -> ProfilingResult:
    ...
    kernel_times, bottleneck_kernels = {}, []
    for kernel in stats.get("GPU Trace", {}).get("Kernels", []):
        name = kernel.get("Name", "unknown")
        duration_ms = kernel.get("Duration", 0) / 1e6
        kernel_times[name] = duration_ms
        if duration_ms > 100:  # >10% of total time threshold
            bottleneck_kernels.append(name)
...
def profile_inference_workload(model_path, test_prompts, output_dir="./prof"):
    # ... (writes a small driver that loads model_path with transformers
    #      and generates each test_prompt, then invokes run_nsight_systems
    #      and returns analyze_nsys_report on the .nsys-rep file. Full
    #      source in companion repo.) ...
    ...

def compare_profiles(baseline_report: str, optimized_report: str):
    base = analyze_nsys_report(baseline_report)
    opt  = analyze_nsys_report(optimized_report)
    base_total = sum(base.kernel_time_ms.values())
    opt_total  = sum(opt.kernel_time_ms.values())
    return {
        "speedup": base_total / opt_total if opt_total > 0 else 1.0,
        "baseline_total_ms": base_total,
        "optimized_total_ms": opt_total,
        "gpu_utilization_improvement":
            opt.gpu_utilization_percent - base.gpu_utilization_percent,
        "memory_bandwidth_improvement":
            opt.memory_bandwidth_gb_s - base.memory_bandwidth_gb_s,
    }
\end{minted}
\end{codebox}
...
\begin{itemize}
\item \textbf{End-to-end latency distribution}: track p50/p95/p99 (and max) for full request latency, including queueing and networking. For interactive systems, tail latency (p95/p99) is often the dominant UX driver \cite{Dean2013Tail}. Figure~\ref{fig:ch07_latency_cdf} illustrates how CDF plots reveal tail latency behavior critical for user experience.
...
\item \textbf{Cost per 1,000 tokens}: track dollars per 1k tokens under your deployment model (on-demand vs.\ reserved vs.\ spot/preemptible). This metric links performance to business cost; see Eq.~\eqref{eq:cost-k} and Figure~\ref{fig:ch07_cost_per_1k_by_engine} for engine-specific cost comparisons.
\end{itemize}
...
\begin{axis}[
    ...
    yticklabel style={fill=white, inner sep=1pt, text opacity=1.0, font=\footnotesize},
    xticklabel style={fill=white, inner sep=1pt, text opacity=1.0, font=\footnotesize}
]
% CDF curve
\addplot+[
    color=llmblue, solid,
    line width=1.4pt,
    mark=*, mark size=2pt,
    mark options={fill=llmblue, draw=white, line width=0.4pt},
    smooth
] table{
x   y
50   0.02
...
% p50 marker
\addplot+[domain=0:1, samples=2, color=llmgreen, dashed,
          line width=1pt, dash pattern=on 4pt off 2pt, mark=none]
    coordinates {(420,0) (420,0.5)};
\addplot+[only marks, mark=square*, mark size=3pt,
          color=llmgreen,
          mark options={fill=llmgreen, draw=white, line width=0.4pt}]
    coordinates {(420,0) (420,0.5)};
\node[llm/label, fill=white, inner sep=3pt, rounded corners=3pt,
      draw=llmgreen, line width=0.5pt, text=llmgreen,
      anchor=west, font=\footnotesize\bfseries]
    at (axis cs:550,0.55) {p50: 420\,ms};

% p95 marker
\addplot+[domain=0:1, samples=2, color=llmorange, dashed,
          line width=1pt, dash pattern=on 4pt off 2pt, mark=none]
    coordinates {(2000,0) (2000,0.95)};
\addplot+[only marks, mark=*, mark size=3pt, color=llmorange,
          mark options={fill=llmorange, draw=white, line width=0.4pt}]
    coordinates {(2000,0) (2000,0.95)};
\node[llm/label, fill=white, inner sep=3pt, rounded corners=3pt,
      draw=llmorange, line width=0.5pt, text=llmorange,
      anchor=west, font=\footnotesize\bfseries]
    at (axis cs:2200,0.98) {p95: 2000\,ms};

% p99 marker
\addplot+[domain=0:1, samples=2, color=llmgray, dashed,
          line width=1pt, dash pattern=on 4pt off 2pt, mark=none]
    coordinates {(3200,0) (3200,0.99)};
\addplot+[only marks, mark=triangle*, mark size=3pt,
          color=llmgray,
          mark options={fill=llmgray, draw=white, line width=0.4pt}]
    coordinates {(3200,0) (3200,0.99)};
\node[llm/label, fill=white, inner sep=3pt, rounded corners=3pt,
      draw=llmgray, line width=0.5pt, text=llmgray,
      anchor=west, font=\footnotesize\bfseries]
...
Table~\ref{tab:ch07_ishtar_phase_metrics} shows the cumulative impact of each optimization phase on \ishtar{}'s key performance indicators, illustrating how gains compound when optimizations are applied in sequence.

\begin{table}[t]
\centering
\small
\caption{\ishtar{} performance metrics across optimization phases demonstrate how gains compound. Each row adds one optimization to the previous baseline. TTFT = time to first token; Tokens/s = sustained generation throughput per GPU; Cost/Req = estimated cost per average request; P95 Latency = 95th percentile end-to-end latency. Values are representative of a 13B model on A100-80GB with typical news-query workloads.}
\label{tab:ch07_ishtar_phase_metrics}
\setlength{\tabcolsep}{5pt}
\renewcommand{\arraystretch}{1.3}
\rowcolors{2}{gray!8}{white}
\begin{tabularx}{0.95\linewidth}{@{}>{\raggedright\arraybackslash}p{3.8cm}
...
\toprule
\rowcolor{gray!15}
\textbf{Phase} & \textbf{TTFT (P50\,/\,P99)} & {\textbf{Tokens/s}} & {\textbf{Cost/Req (\$)}} & {\textbf{P95 Lat.\,(ms)}} \\
\midrule
Baseline (no optimization) & 1200\,ms\,/\,3000\,ms & 45 & 0.032 & 4500 \\
+ Continuous batching       & 800\,ms\,/\,1800\,ms  & 120 & 0.018 & 2800 \\
+ INT8 quantization         & 600\,ms\,/\,1400\,ms  & 180 & 0.012 & 2100 \\
+ FlashAttention-2          & 500\,ms\,/\,1100\,ms  & 220 & 0.010 & 1600 \\
+ Speculative decoding      & 400\,ms\,/\,900\,ms   & 310 & 0.008 & 1200 \\
\bottomrule
\end{tabularx}
\end{table}
...
\subsection{Optimization Priority Framework}\index{optimization!priority framework}
\label{sec:perf-priority-framework}
When time and engineering resources are limited, practitioners need a principled way to decide which optimization to tackle first.
The following priority order reflects a combination of \emph{effort required}, \emph{risk of quality regression}, and \emph{expected impact on throughput and cost}.
Table~\ref{tab:ch07_opt_priority} summarizes the framework.

\begin{table}[t]
\centering
\small
\caption{Optimization priority framework guides practitioners to the highest-impact changes first. Lower-numbered priorities require less effort and carry lower regression risk, while higher-numbered priorities deliver substantial gains but demand more engineering investment. Teams should work top-to-bottom, validating each level before proceeding.}
\label{tab:ch07_opt_priority}
\setlength{\tabcolsep}{6pt}
\renewcommand{\arraystretch}{1.3}
\rowcolors{2}{gray!8}{white}
\begin{tabularx}{0.95\linewidth}{@{}c>{\raggedright\arraybackslash}p{3.8cm}>{\raggedright\arraybackslash}p{2.2cm}>{\raggedright\arraybackslash}p{1.6cm}>{\raggedright\arraybackslash}X@{}}
\toprule
\rowcolor{gray!15}
\textbf{Priority} & \textbf{Optimization} & \textbf{Expected Impact} & \textbf{Effort} & \textbf{Notes} \\
\midrule
\rowcolor{green!10}
\textcolor{green!70!black}{\textbf{1}} & Batch size tuning + continuous batching & 2--8$\times$ throughput & \textcolor{green!70!black}{\textbf{Low}} & Near-free performance; enable in vLLM/TGI. Validate latency SLOs. \\
\rowcolor{blue!8}
\textcolor{blue!70!black}{\textbf{2}} & Quantization (INT8 / FP8) & 1.3--2$\times$ tokens/s; 50\% memory reduction & Moderate & Calibrate on representative data; validate with golden set. \\
\rowcolor{blue!8}
\textcolor{blue!70!black}{\textbf{3}} & KV-cache optimization (paged attention, quantized KV) & 1.5--3$\times$ effective batch capacity & Moderate & Critical for long contexts and high concurrency. \\
\rowcolor{orange!10}
\textcolor{orange!70!black}{\textbf{4}} & Model distillation & 2--10$\times$ smaller model; proportional cost reduction & \textcolor{orange!70!black}{\textbf{High}} & Requires teacher--student training pipeline; best for stable workloads. \\
\rowcolor{red!8}
\textcolor{red!70!black}{\textbf{5}} & Custom kernels / FlashAttention & 10--30\% end-to-end speedup & \textcolor{red!70!black}{\textbf{High}} & Hardware-specific; requires kernel expertise or framework support. \\
\bottomrule
\end{tabularx}
\end{table}

\noindent The rationale is straightforward: batching and continuous batching are configuration changes that require no model modification and carry essentially zero quality risk; quantization is a one-time calibration step with well-understood trade-offs; KV-cache optimization enables higher concurrency without additional hardware; distillation and custom kernels deliver the largest gains but require significant engineering investment and ongoing maintenance.

\BestPracticeBox{%
\textbf{Optimization priority order (when time is limited):}
\begin{enumerate}[leftmargin=1.5em, itemsep=2pt]
  \item \textbf{Batch size + continuous batching} --- free throughput gains.
  \item \textbf{INT8/FP8 quantization} --- high impact, moderate effort.
  \item \textbf{KV-cache optimization} --- unlock longer contexts and larger batches.
  \item \textbf{Model distillation} --- high effort, high reward for stable production workloads.
  \item \textbf{Custom kernels / FlashAttention} --- hardware-specific, requires specialized expertise.
\end{enumerate}
Always validate each level with regression tests before proceeding to the next. The first two levels alone can deliver 3--5$\times$ improvement for most deployments.
}
...
\midrule
Median latency (p50)     & \textcolor{orange!70!black}{\textbf{2.3 s}}  & \textcolor{green!70!black}{\textbf{1.1 s}}  & INT8/4-bit + vLLM batching + RAG compression. \\
Tail latency (p95)       & \textcolor{orange!70!black}{\textbf{5.0 s}}  & \textcolor{green!70!black}{\textbf{2.0 s}}  & Fewer outliers; async streaming improved TTFT. \\
Throughput (tokens/s/GPU)& \textcolor{orange!70!black}{\textbf{130}}    & \textcolor{green!70!black}{\textbf{210}}    & $\sim$60\% gain from batching + engine swap. \\
Cost per 1k tokens (USD) & \textcolor{orange!70!black}{\textbf{1.00}}   & \textcolor{green!70!black}{\textbf{0.70}}   & $\sim$30\% GPU-hour reduction at same volume. \\
\bottomrule
\end{tabularx}
\end{table}
...
\textbf{Latency/Throughput:} Time-to-first-token (TTFT) depends on prompt length $T_0$ since the model must encode the entire prompt before producing output. Subsequent tokens are generated step by step, each attending to all prior tokens. Naive complexity per step is $O(T^2)$, but optimizations like caching and FlashAttention significantly reduce runtime \cite{Dao2023FlashAttention2}. In steady state, throughput is measured in tokens/s:
\begin{equation}
  \Theta \;\approx\; \frac{B \cdot \Delta}{\tau_{\text{step}}(B, T_0, \Delta)} \cdot \eta
\end{equation}

where $B$ is batch size, $\Delta$ the generated length, $\tau_{\text{step}}$ the per-step time, and $\eta$ an efficiency factor accounting for utilization \cite{Kwon2023vLLM,Dao2022FlashAttention}. Larger $B$ boosts throughput, but queueing delays increase latency.
...
\]
Thus long contexts and larger batches quickly dominate GPU memory, often equaling or exceeding model weights. Paging and quantizing KV (\S\ref{sec:inference-engines}) expand feasible deployment ranges \cite{Kwon2023vLLM}.
...
MoE models activate a sparse subset of experts per token, reducing effective FLOPs while maintaining high capacity.
...
Self-attention time complexity per layer with naive kernels is $O(B\,T^2\,H\,d_h)$. For generation, time-to-first-token (TTFT) scales with $T_0^2$; steady-state token latency scales linearly in $\Delta$ with optimized kernels such as FlashAttention \cite{Dao2023FlashAttention2}.
...
In terms of compute complexity, a naive self-attention layer is $O(N^2 d_{\text{model}})$ where $d_{\text{model}} = H \cdot d_h$. For generation, TTFT scales as $O(N_0^2)$, while subsequent tokens scale closer to $O(N_0^2 + N_0 \Delta)$ with optimized kernels. FlashAttention~\cite{Dao2022FlashAttention} and similar implementations reduce memory traffic and constants, so generating $\Delta$ tokens is much faster than a naive quadratic analysis suggests.
...
\renewcommand{\arraystretch}{1.3}
\rowcolors{2}{gray!8}{white}
\begin{tabularx}{\linewidth}{@{}p{3.2cm}XXX@{}}
\toprule
\rowcolor{gray!15}
...
A number of specialized inference engines have been developed to serve LLMs efficiently. Here we compare three widely used runtimes for decoder-based LLM inference in the cloud: vLLM~\cite{Kwon2023vLLM}, Hugging Face Text Generation Inference (TGI)~\cite{tgi-whitepaper}, and NVIDIA TensorRT-LLM~\cite{tensorrt-llm}. Each runtime embodies a distinct approach to batching, memory management, and kernel optimization. Table~\ref{tab:ch07_runtimes} provides a qualitative comparison drawn from public documentation and reported benchmarks, focused on decoder-only use cases since they remain the most challenging to serve at scale; validate with your own workloads.

Figure~\ref{fig:ch07_inference_engine_callout} summarizes a production vLLM deployment as five tunable surfaces. Each surface owns a small number of knobs whose defaults you should understand before promoting the engine into the request path.
...
\textbf{\large Production vLLM tunable surfaces}\\[4mm]
\begin{minipage}[t]{0.95\linewidth}
\colorbox{llmblue!10}{\parbox{0.98\linewidth}{\vspace{2mm}\textbf{\textcolor{llmblue!90!black}{Batching \& scheduling:}} \texttt{max\_model\_len}\,16k, \texttt{max\_num\_batched\_tokens}\,8k, \texttt{max\_num\_seqs}\,256; chunked prefill on; \texttt{preemption\_mode\,=\,recompute} (favor latency over swap I/O) \cite{Kwon2023vLLM}.\vspace{2mm}}}\\[2mm]
\colorbox{llmpurple!10}{\parbox{0.98\linewidth}{\vspace{2mm}\textbf{\textcolor{llmpurple!90!black}{KV cache \& PagedAttention:}} prefix caching on, \texttt{block\_size}\,16, \texttt{gpu\_memory\_utilization}\,0.90; PagedAttention v2 enabled so long contexts don't fragment the cache \cite{Kwon2023vLLM}.\vspace{2mm}}}\\[2mm]
\colorbox{llmgreen!10}{\parbox{0.98\linewidth}{\vspace{2mm}\textbf{\textcolor{llmgreen!90!black}{Parallelism \& kernels:}} \texttt{tensor\_parallel\_size} sized to the GPU count; FLASHINFER attention backend (fall back to FLASH\_ATTN/XFORMERS by hardware); custom all-reduce kept on; LoRA off unless a multi-tenant adapter pool is needed.\vspace{2mm}}}\\[2mm]
\colorbox{llmorange!10}{\parbox{0.98\linewidth}{\vspace{2mm}\textbf{\textcolor{llmorange!90!black}{Serving surface:}} OpenAI-compatible HTTP on :8000; streaming on with per-token interval; request and batch capped at \texttt{max\_model\_len}; CORS restricted to first-party origins; API keys enforced at the gateway.\vspace{2mm}}}\\[2mm]
\colorbox{llmgray!10}{\parbox{0.98\linewidth}{\vspace{2mm}\textbf{\textcolor{llmgray!90!black}{Observability:}} Prometheus on :8001 exposing \texttt{num\_requests\_running/waiting}, \texttt{gpu\_cache\_usage\_percent}, \texttt{time\_to\_first\_token\_seconds}, and \texttt{time\_per\_output\_token\_seconds}---the four signals that drive HPA and SLO alerting.\vspace{2mm}}}
\end{minipage}
};
\end{tikzpicture}
\end{llmfigbox}
\caption{Production vLLM deployment expressed as five tunable surfaces. Batching and KV-cache settings together determine peak throughput and the maximum supported context; parallelism and kernel choice determine how the model is sharded across hardware; the serving surface and observability layer determine how the engine plugs into the rest of the platform. Defaults shown are appropriate for a 13B chat model on a single H100/A100; resize batching and \texttt{tensor\_parallel\_size} for larger models.}
\label{fig:ch07_inference_engine_callout}
\end{figure}
...
{\footnotesize
  ...
  \renewcommand{\arraystretch}{1.2}
  \rowcolors{2}{gray!8}{white}
  \begin{xltabular}{\linewidth}{@{}>{\raggedright\arraybackslash}p{3.0cm}>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X@{}}
    ...
    \toprule
    \rowcolor{gray!15}
    & \textbf{vLLM} & \textbf{Hugging Face TGI} & \textbf{TensorRT-LLM} \\
    \midrule
    \endhead
    \midrule
...
LangChain~\cite{LangChain2023Framework} provides orchestration that, if instrumented carefully, improves both performance and reliability for complex LLM workflows.

\subsection{Tracing, Telemetry, and Token Accounting}
Use LangSmith (or OpenTelemetry export) to trace chains/graphs: per-node latency, prompt token counts, output lengths, and error rates. Maintain per-route KPIs (e.g., RAG retrieval latency, generation TTFT, tokens/s) and alert on drifts.

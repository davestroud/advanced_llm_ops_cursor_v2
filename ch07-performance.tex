...
if __name__ == "__main__":
    ...
    # Compare with a previous baseline if available
    baseline_path = Path("./profiling_results/baseline.nsys-rep")
    if baseline_path.exists():
        comparison = compare_profiles(
            str(baseline_path),
            str(Path("./profiling_results")
                / f"profile_{int(time.time())}.nsys-rep")
        )
        print(f"\n{'='*50}")
        print(f"Comparison vs. Baseline:")
        print(f"  Speedup: {comparison['speedup']:.2f}x")
        print(f"  Baseline total: "
              f"{comparison['baseline_total_ms']:.1f}ms")
        print(f"  Optimized total: "
              f"{comparison['optimized_total_ms']:.1f}ms")
        print(f"  GPU util. delta: "
              f"{comparison['gpu_utilization_improvement']:+.1f}%")
        print(f"  Mem BW delta: "
              f"{comparison['memory_bandwidth_improvement']:+.2f} GB/s")

    # Write structured JSON report for CI integration
    report = {
        "timestamp": int(time.time()),
        "gpu_utilization_pct": result.gpu_utilization_percent,
        "memory_bandwidth_gb_s": result.memory_bandwidth_gb_s,
        "top_kernels": dict(sorted_kernels),
        "bottlenecks": result.bottleneck_kernels,
        "suggestions": result.optimization_suggestions,
        "pass": result.gpu_utilization_percent > 70
                and len(result.bottleneck_kernels) < 3
    }
    report_path = Path("./profiling_results/report.json")
    report_path.write_text(json.dumps(report, indent=2))
    print(f"\nReport written to {report_path}")
    print(f"CI gate: {'PASS' if report['pass'] else 'FAIL'}")
\end{minted}
\end{codebox}

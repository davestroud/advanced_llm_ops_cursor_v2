...
\midrule
1 & Batch size tuning + continuous batching & 2--8$\times$ throughput & Low & Near-free performance; enable in vLLM/TGI. Validate latency SLOs. \\
2 & Quantization (INT8 / FP8) & 1.3--2$\times$ tokens/s; 50\% memory reduction & Moderate & Calibrate on representative data; validate with golden set. \\
3 & KV-cache optimization (paged attention, quantized KV) & 1.5--3$\times$ effective batch capacity & Moderate & Critical for long contexts and high concurrency. \\
4 & Model distillation & 2--10$\times$ smaller model; proportional cost reduction & High & Requires teacher--student training pipeline; best for stable workloads. \\
5 & Custom kernels / FlashAttention & 10--30\% end-to-end speedup & High & Hardware-specific; requires kernel expertise or framework support. \\
\bottomrule
\end{tabularx}
\end{table}

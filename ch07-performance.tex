...
\begin{codebox}{Mixed-precision quantization configuration with calibration data, quality validation gates, and per-layer fallback policies. This configuration balances memory reduction with accuracy preservation, enabling deployment of large models on production hardware.}{lst:ch07_quantization_config}
\begin{minted}{yaml}
...
\begin{codebox}{Comprehensive caching configuration for response and embedding caching with Redis and pgvector backends, TTL policies, eviction strategies, and cache warming schedules. This multi-layer caching approach reduces both latency and cost by avoiding redundant LLM calls and embedding computations.}{lst:ch07_caching_config}
\begin{minted}{yaml}
...
\begin{codebox}{Production vLLM inference engine configuration covering continuous batching, PagedAttention, KV-cache management, tensor parallelism, and Prometheus monitoring. This configuration optimizes throughput and latency for serving large language models at scale.}{lst:ch07_inference_engine}
\begin{minted}{yaml}

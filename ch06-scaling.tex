...
\begin{lstlisting}[
  ...
  style=springerfloat,
  language=Python,
  caption={SLO-driven capacity planning script for LLM deployments that estimates token demand from workload profiles, calculates required GPU replicas with utilization headroom, decomposes end-to-end latency into prefill, decode, and queueing components, and validates the plan against service level objectives. This structured approach prevents common scaling failures caused by incomplete workload analysis or missing latency components.},
...
\begin{lstlisting}[
  float=htbp,
  style=springerfloat,
  caption={Cost-aware routing configuration implementing hierarchical model selection across three tiers (lightweight, standard, premium) based on query complexity scoring, per-request and per-tenant cost budgets, latency SLO constraints, and explicit routing hints. This configuration enables production systems to balance quality, cost, and latency by routing each request to the most cost-effective model tier that meets its requirements.},

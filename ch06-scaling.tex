...
Scaling a Large Language Model deployment is a multifaceted challenge. While traditional applications often scale linearly with additional compute resources, LLM-based systems face unique constraints: massive model sizes, GPU memory limits, high operational costs, and variable demand patterns. These factors make even high-performance GPU servers struggle under LLM workloads \cite{Kwon2023vLLM}. As a result, scaling LLM deployments has become a major focus in both industry and academia—nearly every recent ML systems conference now features dedicated sessions on LLM serving and optimization \cite{Kwon2023vLLM}. Groundbreaking models like ChatGPT have demonstrated unprecedented utility and demand, but their inference is slow and resource-intensive: for example, generating each token may require reading on the order of a terabyte of model weights \cite{google2022_chip}. In this chapter, we present a detailed guide to scaling LLM systems from prototype to global production, with \ishtar{} as our primary reference case:\cite{vllm_pagedattention}.

\subsection{Operational Realities Beyond Baseline Constraints}\label{sec:scaling-operational-realities}
Beyond baseline resource limits, several operational realities make scaling LLM deployments uniquely challenging compared to traditional microservices (see Table~\ref{tab:ch06_scaling_challenges}):
...
In \ishtar{}, these constraints manifested most visibly during breaking news cycles, when concurrent journalist queries could spike $10\times$ above baseline. As summarized in Table~\ref{tab:ch06_ishtar_scaling_strategies}, our scaling strategy combined:
...
The challenges outlined above are driving intense research into LLM serving systems. A 2024 survey identified an explosion of system-level innovations published since 2023 aimed specifically at LLM inference scalability \cite{Kwon2023vLLM}. Solving these issues is not just about adding hardware; it requires rethinking system design for LLMs. In the following sections, we explore how vertical, horizontal, and hybrid scaling approaches address these challenges, and we delve into advanced techniques—distributed inference, batching, speculative decoding, caching, and more—that enable LLM deployments to operate efficiently at scale:\cite{vllm_pagedattention,tensorrt_llm_docs,tgi_docs}.
...
\begin{codebox}{SLO-driven capacity planning script for LLM deployments that estimates token demand from workload profiles, calculates required GPU replicas with utilization headroom, decomposes end-to-end latency into prefill, decode, and queueing components, and validates the plan against service level objectives. This structured approach prevents common scaling failures caused by incomplete workload analysis or missing latency components.}{lst:ch06_capacity_planning}
\begin{minted}{python}
...
class WorkloadProfile:
    ...
    context_class_distribution: Dict[str, float]
...
def estimate_token_demand(workload: WorkloadProfile) -> float:
    avg_total_tokens = workload.avg_prompt_tokens + workload.avg_decode_tokens
    return workload.arrival_rate_rps * avg_total_tokens

def calculate_required_replicas(token_demand, gpu: GPUConfig, headroom=0.7):
    effective_throughput = gpu.throughput_tokens_per_sec * headroom
    return int(np.ceil(token_demand / effective_throughput))

def decompose_latency(prompt_tokens, decode_tokens, gpu, batch_size=8):
    # Prefill scales with prompt length; decode benefits from batching;
    # queueing is approximated from a fixed 70% utilization assumption.
    prefill_ms = (prompt_tokens / gpu.throughput_tokens_per_sec) * 1000
    decode_ms = (decode_tokens / (gpu.throughput_tokens_per_sec * batch_size)) * 1000
    queueing_ms = 50 * 0.7
    return {
        "prefill_ms": prefill_ms,
        "decode_ms": decode_ms,
        "queueing_ms": queueing_ms,
        "total_ms": prefill_ms + decode_ms + queueing_ms,
    }

def validate_capacity(result: CapacityResult, slo: SLO, lat: Dict[str, float]) -> bool:
    ttft_ms = lat["prefill_ms"] + lat["queueing_ms"]
    return (ttft_ms <= slo.p95_ttft_ms
            and lat["decode_ms"] <= slo.p95_tbt_ms
            and result.utilization_percent <= 0.85)

def plan_capacity(workload, gpu, slo, headroom=0.7) -> CapacityResult:
    token_demand = estimate_token_demand(workload)
    replicas = calculate_required_replicas(token_demand, gpu, headroom)
    lat = decompose_latency(workload.avg_prompt_tokens,
                            workload.avg_decode_tokens, gpu)
    eff_throughput = gpu.throughput_tokens_per_sec * headroom
    utilization = token_demand / (replicas * eff_throughput)
    result = CapacityResult(
        required_replicas=replicas,
        estimated_cost_per_hour_usd=replicas * gpu.cost_per_hour_usd,
        utilization_percent=utilization * 100,
        latency_components=lat,
        validation_passed=False,
    )
    result.validation_passed = validate_capacity(result, slo, lat)
    return result

# ... (driver block: defines SLO(p95_ttft=300ms, p95_tbt=50ms),
#      WorkloadProfile(50 rps, 500 prompt, 200 decode), and compares
#      A100-80GB vs H100-80GB by calling plan_capacity for each.
#      Full source in companion repo.) ...
\end{minted}
\end{codebox}
...
\begin{itemize}[leftmargin=1.5em, itemsep=3pt]
    \item \textbf{Increased complexity:} Horizontal scaling requires distributed inference frameworks (e.g., PyTorch TensorParallel, DeepSpeed, Megatron-LM~\cite{megatron_lm_2019}, vLLM~\cite{Kwon2023vLLM}, or Petals~\cite{Borzunov2023Petals}) and orchestration logic for load balancing. Engineering effort is needed to manage inter-node communication, distributed state (like KV caches per node), and aggregate monitoring.
...
\begin{tikzpicture}[
  ...
  vbox/.style ={llm/box/blue,   minimum width=2.4cm, minimum height=1cm},
  hbox/.style ={llm/box/green,  minimum width=2.4cm, minimum height=1cm},
  hybox/.style={llm/box/purple, minimum width=2.4cm, minimum height=1cm},
  hybox2/.style={llm/box/orange, minimum width=2.4cm, minimum height=1cm},
  hybox3/.style={llm/box/gray,   minimum width=2.4cm, minimum height=1cm},
  labelbox/.style={
    llm/label, text width=3.2cm, inner sep=2pt
  }
]

% --- Vertical scaling (left column) ---
\node[vbox] (v1) at (0,3) {\textbf{A100-40GB}\\\footnotesize GPU Node};
\node[vbox, below=8mm of v1] (v2) {\textbf{A100-80GB}\\\footnotesize GPU Node};
\node[vbox, below=8mm of v2] (v3) {\textbf{H100-80GB}\\\footnotesize GPU Node};

\node[labelbox, right=10mm of v2] (vlabel) {
  \textbf{Vertical Scaling}\\[1pt]
  Upgrading a single node with more powerful GPUs and memory.
};

\draw[llm/arrow] (v1.south) -- (v2.north);
\draw[llm/arrow] (v2.south) -- (v3.north);

% --- Horizontal scaling (top right) ---
\node[hbox] (h1) at (7,3) {\footnotesize GPU Node};
\node[hbox, right=8mm of h1] (h2) {\footnotesize GPU Node};
\node[hbox, right=8mm of h2] (h3) {\footnotesize GPU Node};

\node[labelbox, below=6mm of h2] (hlabel) {
  \textbf{Horizontal Scaling}\\[1pt]
  Adding more nodes and distributing requests.
};

\draw[llm/arrow/dashed] (h1.east) -- (h2.west);
\draw[llm/arrow/dashed] (h2.east) -- (h3.west);
\draw[llm/arrow] (hlabel.north) -- (h2.south);

% --- Hybrid scaling (bottom right) ---
\node[hybox]  (hy1) at (7,0) {\footnotesize\textbf{H100}\\\footnotesize Node};
\node[hybox2, right=8mm of hy1] (hy2) {\footnotesize\textbf{A100}\\\footnotesize Node};
\node[hybox3, right=8mm of hy2] (hy3) {\footnotesize\textbf{CPU}\\\footnotesize Pool};

\node[labelbox, below=6mm of hy2] (hylabel) {
  \textbf{Hybrid Scaling}\\[1pt]
  Mixing high-end GPUs with cheaper GPUs/CPUs for different tiers.
};

\draw[llm/arrow/dashed] (hy1.east) -- (hy2.west);
\draw[llm/arrow/dashed] (hy2.east) -- (hy3.west);
\draw[llm/arrow] (hylabel.north) -- (hy2.south);
...
\section{Distributed Inference Techniques}\index{distributed inference}
...
\subsection{Tensor Parallelism}\index{tensor parallelism}
...
\subsection{Pipeline Parallelism}\index{pipeline parallelism}
...
\begin{codebox}{Hybrid tensor and pipeline parallelism configuration for deploying a 70B-parameter model across 32 GPUs, specifying tensor parallelism groups with NVLink communication, pipeline stage assignments with microbatch scheduling, paged attention KV-cache management, and performance tuning options including Flash Attention and dynamic batching. This configuration demonstrates how production systems combine parallelism strategies to serve large models within memory and latency constraints.}{lst:ch06_model_parallelism}
\begin{minted}{yaml}
# Hybrid TP+PP for LLaMA-3.1-70B on 32 GPUs (TP=8 x PP=4)
model_config: { name: "llama-3.1-70b", num_layers: 80, hidden_size: 8192 }

tensor_parallelism:
  enabled: true
  tp_size: 8                          # Split attn/FFN within an NVLink domain
  partition_strategy: "column_parallel"  # column for attn, row for FFN
  communication: { backend: "nccl", collective_type: "ring_allreduce",
                   enable_async: true }

pipeline_parallelism:
  enabled: true
  pp_size: 4                          # 4 stages of 20 contiguous layers each
  pp_stages:
  ...
  microbatch_size: 4
  num_microbatches: 8
  schedule: "1F1B"                    # inference variants: gpipe, interleaved

parallelism_strategy: "hybrid"        # 8 TP * 4 PP = 32 GPUs total
total_gpus: 32
...
topology:
  nvlink_groups: [[0, 1, 2, 3, 4, 5, 6, 7]]  # full mesh via NVSwitch
  stage_communication: { type: "p2p", overlap: true }
...
# (Monitoring + quantization knobs elided; see companion repo.)
\end{minted}
\end{codebox}
...
\begin{tikzpicture}[
  font=\small,
  gpu/.style={llm/box/blue,   minimum width=18mm, minimum height=10mm, font=\footnotesize},
  gpug/.style={llm/box/green, minimum width=14mm, minimum height=7mm, font=\footnotesize},
  gpuo/.style={llm/box/orange, minimum width=18mm, minimum height=10mm, font=\footnotesize},
  titlestyle/.style={font=\small\bfseries, align=center}
]

% ============ (a) Data Parallelism ============
\node[titlestyle] (dptitle) at (0, 3.2) {(a) Data Parallelism};

\node[gpu] (dp1) at (-1.8, 1.8) {GPU 0\\Full Model};
\node[gpu] (dp2) at (0, 1.8) {GPU 1\\Full Model};
\node[gpu] (dp3) at (1.8, 1.8) {GPU 2\\Full Model};

\node[llm/label, align=center] at (0, 0.8) {Each GPU holds a\\complete model replica};

% Request arrows
\node[font=\footnotesize] (req) at (0, 2.8) {Requests};
\draw[llm/arrow] (req.south) -- (dp1.north);
\draw[llm/arrow] (req.south) -- (dp2.north);
\draw[llm/arrow] (req.south) -- (dp3.north);

% ============ (b) Tensor Parallelism ============
\node[titlestyle] (tptitle) at (6, 3.2) {(b) Tensor Parallelism};

% Single layer split across GPUs
\node[gpug] (tp1) at (4.5, 2.2) {Slice 0};
\node[gpug] (tp2) at (6.0, 2.2) {Slice 1};
\node[gpug] (tp3) at (7.5, 2.2) {Slice 2};

\node[llm/label] at (6.0, 2.95) {Single Layer};

% All-reduce arrows (dashed = synchronization, not control flow)
\draw[llm/arrow/dashed] (tp1.east) -- (tp2.west);
\draw[llm/arrow/dashed] (tp2.east) -- (tp3.west);

\node[gpug] (tpl2a) at (4.5, 1.2) {Slice 0};
\node[gpug] (tpl2b) at (6.0, 1.2) {Slice 1};
\node[gpug] (tpl2c) at (7.5, 1.2) {Slice 2};

\draw[llm/arrow] (tp1.south) -- (tpl2a.north);
\draw[llm/arrow] (tp2.south) -- (tpl2b.north);
\draw[llm/arrow] (tp3.south) -- (tpl2c.north);
\draw[llm/arrow/dashed] (tpl2a.east) -- (tpl2b.west);
\draw[llm/arrow/dashed] (tpl2b.east) -- (tpl2c.west);

\node[llm/label, align=center] at (6.0, 0.4) {Matrix ops split across GPUs;\\all-reduce sync per layer};

% ============ (c) Pipeline Parallelism ============
\node[titlestyle] (pptitle) at (12, 3.2) {(c) Pipeline Parallelism};

\node[gpuo] (pp1) at (12, 2.4) {GPU 0\\Layers 0--7};
\node[gpuo] (pp2) at (12, 1.4) {GPU 1\\Layers 8--15};
\node[gpuo] (pp3) at (12, 0.4) {GPU 2\\Layers 16--23};

\draw[llm/arrow] (pp1.south) -- (pp2.north);
\draw[llm/arrow] (pp2.south) -- (pp3.north);

\node[llm/label, align=center, right=8mm of pp2] {Sequential stages;\\microbatches\\in flight};

\end{tikzpicture}
\end{llmfigbox}
\caption{Distributed inference methods enable large-model deployment but introduce communication overhead. \textbf{(a)} Data Parallelism replicates the full model on each GPU, routing different requests to different replicas for throughput scaling. \textbf{(b)} Tensor Parallelism splits individual layer operations (matrix multiplies) across GPUs, requiring all-reduce synchronization at every layer boundary. \textbf{(c)} Pipeline Parallelism assigns contiguous layer blocks to different GPUs, forming an assembly line where microbatches flow through sequential stages. Choose based on model size, hardware topology, and latency tolerance.}
...
\section{Batching and Throughput Optimization}\index{batching}\index{throughput optimization}
...
\begin{codebox}{Kubernetes Horizontal Pod Autoscaler configuration for LLM-aware autoscaling using custom metrics (token demand rate, request queue depth, P95 TTFT, GPU utilization) with asymmetric scaling policies---aggressive scale-up for burst handling and conservative scale-down for stability. Includes Prometheus adapter configuration that bridges Kubernetes custom metrics API to LLM-specific Prometheus queries.}{lst:ch06_hpa_config}
\begin{minted}{yaml}
apiVersion: autoscaling/v2
kind: HorizontalPodAutoscaler
metadata: { name: ishtar-llm-inference-hpa, namespace: production }
spec:
  scaleTargetRef:
    apiVersion: apps/v1
    kind: Deployment
  ...
  metrics:                              # All four are per-pod averages.
  - { type: Pods, pods: { metric: { name: token_demand_rate },
        target: { type: AverageValue, averageValue: "8000" } } }   # tokens/s
  - { type: Pods, pods: { metric: { name: request_queue_depth },
        target: { type: AverageValue, averageValue: "10" } } }     # requests
  - { type: Pods, pods: { metric: { name: p95_ttft_ms },
        target: { type: AverageValue, averageValue: "300" } } }    # ms
  - { type: Pods, pods: { metric: { name: gpu_utilization_percent },
        target: { type: AverageValue, averageValue: "70" } } }     # percent

  # Asymmetric: conservative scale-down (5 min stabilize, max 25% or 2 pods),
  # aggressive scale-up (0s stabilize, can double or +5 pods every 30s).
  behavior:
    scaleDown:
...
---
# Prometheus adapter exposes the four ishtar_* series as the custom metrics
# referenced above.  Sketch of one rule (full ConfigMap in companion repo):
apiVersion: v1
kind: ConfigMap
metadata: { name: adapter-config, namespace: kube-system }
data:
  config.yaml: |
    rules:
    - seriesQuery: 'ishtar_token_demand_rate{namespace!="",pod!=""}'
      resources: { overrides: { namespace: { resource: "namespace" },
                                pod:       { resource: "pod" } } }
      name: { matches: "^ishtar_token_demand_rate", as: "token_demand_rate" }
      metricsQuery: 'sum(rate(<<.Series>>{<<.LabelMatchers>>}[2m]))
                     by (<<.GroupBy>>)'
    # ... analogous rules for request_queue_depth (avg), p95_ttft_ms
    #     (histogram_quantile 0.95 over 5m), and gpu_utilization_percent (avg).
\end{minted}
\end{codebox}
...
% Demand
\addplot+[color=llmblue, solid, mark=*, mark size=2pt,
...
\addplot+[color=llmorange, dashed, dash pattern=on 4pt off 2pt,
...
% Horizontal reference line for single-replica capacity
\addplot+[domain=0:100, samples=2, color=llmgray, dashdotted,
          dash pattern=on 3pt off 2pt on 0.5pt off 2pt,
          line width=1pt, mark=none] {430};
\node[llm/label, fill=white, inner sep=2pt, rounded corners=2pt]
    at (axis cs:8,470) {$\gamma\,\hat{C}$ per replica};

% Annotations
\node[llm/label, anchor=west, fill=llmblue!10,
      inner sep=3pt, rounded corners=3pt, text=llmblue]
    at (axis cs:33,700) {\textbf{Spike}};
\draw[llm/arrow, color=llmblue]
    (axis cs:30,670) -- (axis cs:32,600);

\node[llm/label, anchor=west, fill=llmorange!10,
      inner sep=3pt, rounded corners=3pt, text=llmorange]
    at (axis cs:44,1000) {\textbf{$N\uparrow$} (scale out)};
\draw[llm/arrow, color=llmorange]
    (axis cs:41,950) -- (axis cs:42,860);

\node[llm/label, anchor=east, fill=llmorange!10,
      inner sep=3pt, rounded corners=3pt, text=llmorange]
...
% Event-based capacity (proactive)
\addplot+[color=llmgreen, dashed, dash pattern=on 4pt off 2pt,
          mark=square*, mark size=2pt,
...
% Metrics-based capacity (reactive reference)
\addplot+[color=llmorange, dotted, mark=diamond*, mark size=2pt,
          mark options={fill=llmorange, draw=white, line width=0.4pt},
          smooth] coordinates {
(0,430) (20,430) (30,430) (40,860) (55,860) (60,860) (70,600) (85,600) (100,430)
};
...
% Single-replica capacity reference
\addplot+[domain=0:100, samples=2, color=llmgray, dashdotted,
          dash pattern=on 3pt off 2pt on 0.5pt off 2pt,
          line width=1pt, mark=none] {430};
\node[llm/label, fill=white, inner sep=2pt, rounded corners=2pt]
...
\node[llm/label, anchor=south west, align=left, fill=llmgreen!10,
      inner sep=3pt, rounded corners=3pt, text=llmgreen]
    at (axis cs:1,720) {\textbf{Event trigger}\\(calendar/schedule)};
\draw[llm/arrow, color=llmgreen]
    (axis cs:1,700) -- (axis cs:20,600);

% Pre-warm annotation
\node[llm/label, anchor=east, align=right, fill=llmgreen!10,
      inner sep=3pt, rounded corners=3pt, text=llmgreen]
...
\subsection{Spot Instances and Preemption Strategies for LLM Workloads}\index{spot instances}\index{preemption}
\label{subsec:ch06-spot-instances}

Spot (AWS), preemptible (GCP), and low-priority (Azure) instances offer GPU capacity at 60--90\% discounts compared to on-demand pricing~\cite{spot-capacity}, but they can be reclaimed by the cloud provider with as little as 30 seconds notice. For LLM workloads, leveraging spot capacity requires careful engineering to handle preemption without losing user-visible work.

\subsubsection{When spot makes sense}
Spot instances are well-suited for LLM workloads that can tolerate interruption:

\begin{itemize}
    \item \textbf{Batch inference}: offline processing of document summarization, classification, or embedding generation, where jobs can be checkpointed and resumed on a replacement instance.
    \item \textbf{Non-latency-sensitive training and fine-tuning}: LoRA adapter training or continued pre-training, where checkpoints are written at regular intervals and the job resumes from the latest checkpoint on a new instance.
    \item \textbf{Burst capacity for non-critical traffic}: supplementing on-demand capacity during traffic surges for low-priority user tiers or background features (e.g., suggested follow-up questions, automated tagging).
    \item \textbf{Nightly evaluation suites}: running large-scale adversarial, fairness, or regression evaluation suites (Section~\ref{sec:cicd-continuous-eval} of Chapter~\ref{ch:cicd}) where jobs are idempotent and can be restarted.
\end{itemize}

\subsubsection{Preemption handling}
The key engineering challenge is graceful degradation when a spot instance is reclaimed:

\begin{itemize}
    \item \textbf{KV-cache checkpointing}: for serving workloads, save the KV-cache state at token boundaries so that in-flight requests can be migrated to a replacement instance. Systems like SpotServe demonstrate that partial decoding progress can be committed and resumed, achieving near-seamless failover for interactive sessions \cite{pope2023exegpt}.
    \item \textbf{Graceful shutdown hooks}: register cloud provider termination notifications (e.g., AWS EC2 Spot Instance interruption notice, 2-minute warning) and use the window to drain active requests, flush metrics, and persist state.
    \item \textbf{Request rerouting}: the load balancer must detect preemption within seconds and reroute in-flight requests to surviving instances. Health-check intervals should be shortened for spot pools (e.g., 5\,s instead of 30\,s).
    \item \textbf{Training checkpointing}: for fine-tuning workloads, write checkpoints every $N$ steps (calibrated so that the maximum lost work is bounded, e.g., at most 10 minutes of training). Store checkpoints on durable shared storage (S3, GCS, or a persistent volume) rather than local disk.
\end{itemize}

\subsubsection{Cost/latency trade-offs}
The decision to use spot capacity involves a trade-off between cost savings and tail-latency risk:

\begin{itemize}
    \item \textbf{Batch inference}: spot savings of 60--80\% with minimal risk, since jobs are idempotent and restartable. Total wall-clock time may increase by 10--20\% due to occasional preemptions and restart overhead.
    \item \textbf{Interactive serving}: spot capacity can deliver meaningful savings for burst tiers, but preemption introduces large tail-latency spikes during failover. Spot is therefore acceptable only for non-SLO-critical traffic.
    \item \textbf{Fine-tuning}: spot savings of 70--90\%, with wall-clock overhead of 5--15\% from checkpoint/restart cycles. The dominant risk is losing training state if checkpointing is infrequent.
\end{itemize}

\subsubsection{Integration with autoscaling}
Spot capacity should be integrated with the autoscaling framework (Section~\ref{sec:autoscaling}) as a fallback tier:

\begin{enumerate}
    \item Maintain a \emph{base pool} of on-demand instances sized to handle steady-state traffic at SLO targets.
    \item Configure the autoscaler to add \emph{spot instances first} when scaling out for burst capacity. If spot capacity is unavailable (capacity pool exhausted), fall back to on-demand instances.
    \item Tag spot instances with \texttt{instance\_lifecycle=spot} in Prometheus labels so that dashboards can track spot utilization, preemption rate, and cost savings separately from on-demand capacity.
    \item Set autoscaler policies that prefer draining spot instances first during scale-in, preserving on-demand instances for stability.
\end{enumerate}
...
\begin{codebox}{Cost-aware routing configuration implementing hierarchical model selection across three tiers (lightweight, standard, premium) based on query complexity scoring, per-request and per-tenant cost budgets, latency SLO constraints, and explicit routing hints. This configuration enables production systems to balance quality, cost, and latency by routing each request to the most cost-effective model tier that meets its requirements.}{lst:ch06_routing_config}
\begin{minted}{yaml}
routing_id: "ishtar_hierarchical_routing"
routing_version: "1.2.0"

# Three model tiers: cheap/fast -> standard -> high-accuracy.
# (cost_per_1k_tokens_usd in USD; latency_p95_ms is steady-state.)
model_tiers:
  tier_1_lightweight:
    model_id: "llama-3.1-8b-chat"
    cost_per_1k_tokens_usd: 0.010
    latency_p95_ms: 150
    max_context_tokens: 8192
    use_cases: ["simple_classification", "basic_summarization", "short_qa"]
  tier_2_standard:
    model_id: "llama-3.1-13b-chat"
    cost_per_1k_tokens_usd: 0.015
    latency_p95_ms: 250
    max_context_tokens: 16384
    use_cases: ["complex_qa", "multi_step_reasoning", "content_generation"]
  tier_3_premium:
    model_id: "llama-3.1-70b-chat"
    cost_per_1k_tokens_usd: 0.050
    latency_p95_ms: 600
    max_context_tokens: 32768
    use_cases: ["critical_fact_checking", "complex_analysis"]

# Four ordered routing rules; the first applicable rule wins.
routing_rules:
  complexity_based:                        # 1. Score -> tier
    enabled: true
    classifier_model: "complexity_classifier_v1.0"
    routing_logic:
      - { condition: "complexity_score < 0.3",
          route_to: "tier_1_lightweight", fallback: "tier_2_standard" }
      - { condition: "0.3 <= complexity_score < 0.7",
          route_to: "tier_2_standard",   fallback: "tier_3_premium" }
      - { condition: "complexity_score >= 0.7",
          route_to: "tier_3_premium",    fallback: null }

  cost_budget:                             # 2. Downgrade to fit budget
    enabled: true
    per_request_budget_usd: 0.10
    per_tenant_daily_budget_usd: 100.0
    routing_logic:
      - { condition: "estimated_cost > per_request_budget",
          action: "downgrade_tier", max_downgrades: 2 }

  latency_slo:                             # 3. Pick a faster tier if needed
    enabled: true
    max_latency_ms: 500
    routing_logic:
  ...
  explicit_hints:                          # 4. Caller-provided override
    enabled: true
    hint_field: "routing_hint"
    routing_map: { critical: "tier_3_premium", standard: "tier_2_standard",
                   fast: "tier_1_lightweight", cost_optimized: "tier_1_lightweight" }

fallback:
  strategy: "cascade"                      # cascade -> failover -> reject
  cascade_order: ["tier_3_premium", "tier_2_standard", "tier_1_lightweight"]
  max_retries: 2
  retry_on_errors: ["timeout", "rate_limit", "model_error"]

# (Cost-tracking and per-tier monitoring sections elided; see companion repo.)
\end{minted}
\end{codebox}
...
Using algorithms like HNSW~\cite{Malkov2018HNSW}, IVF, or product quantization~\cite{Jegou2011PQ} can dramatically speed up retrieval with minimal loss in relevance. At large scale, exact nearest neighbor search in high dimensions is often infeasible. ANN methods allow sub-linear time queries and much smaller memory footprints, trading a tiny bit of accuracy for huge speed gains. This is crucial for real-time RAG.
...
\begin{tikzpicture}[
  ...
  every node/.style={font=\small},
  box/.style   ={llm/box/purple, minimum width=28mm, minimum height=10mm},
  router/.style={llm/box/green,  minimum width=32mm, minimum height=10mm},
  hot/.style   ={llm/box/orange, minimum width=36mm, minimum height=12mm},
  dist/.style  ={llm/box/blue,   minimum width=28mm, minimum height=10mm},
  stor/.style  ={llm/box/gray,   minimum width=36mm, minimum height=10mm},
  merge/.style ={llm/box/green,  minimum width=32mm, minimum height=10mm},
  gen/.style   ={llm/box/green,  minimum width=32mm, minimum height=10mm},
  arrhit/.style ={llm/arrow, color=llmgreen},
  arrmiss/.style={llm/arrow/dashed, color=llmorange}
]
...
\draw[llm/arrow] (q) -- (route);

\draw[llm/arrow] (route) -- (hotidx);
\node[llm/label, right=3mm of route, anchor=west] (lookuplabel) {first lookup};
\node[llm/label, right=3mm of route, anchor=west, below=6mm of lookuplabel] {(in-memory / GPU)};

% Hit path: curves around Shard 2 to avoid passing through it
\draw[arrhit] (hotidx) .. controls +(0,-8mm) and +(0,8mm) .. (merge);
\node[below=12mm of sh2, right=3mm, font=\footnotesize\bfseries, text=llmgreen] {hit};

% Miss path to shards
\draw[arrmiss] (hotidx.west) .. controls +(-2.5,0.0) and +(0.0,1.5) ..
    node[pos=0.3,above,font=\footnotesize\bfseries]{miss} (sh1.north);
\draw[arrmiss] (hotidx) -- (sh2.north);
\draw[arrmiss] (hotidx.east) .. controls +(2.5,0.0) and +(0.0,1.5) .. (sh3.north);

\draw[llm/arrow] (sh1) -- (merge);
\draw[llm/arrow] (sh2) -- (merge);
\draw[llm/arrow] (sh3) -- (merge);

\node[llm/label, align=left, text=llmpurple]
    (note) at ($(embcache.north)+(0,12mm)$) {reuse vectors on\\ identical chunks};
\draw[llm/arrow, color=llmpurple] (note.south) -- (embcache.north);

\draw[llm/arrow] (merge) -- (gen);

% Legend box
\node[draw=black!40, rounded corners=4pt, line width=0.6pt, align=left,
      anchor=south east, fill=white, inner sep=4pt, text width=7cm,
      font=\footnotesize]
      at ($(gen.south east)+(25mm,-2mm)$) (leg) {%
...
\begin{tikzpicture}[
  ...
  every node/.style={font=\small},
  region/.style={llm/box, minimum width=25mm, minimum height=8mm},
  region/blue/.style={region, fill=llmblue!12,   draw=llmblue!70},
  region/green/.style={region, fill=llmgreen!12, draw=llmgreen!70},
  region/orange/.style={region, fill=llmorange!12, draw=llmorange!70},
  user/.style={llm/box/purple, minimum width=19mm, minimum height=7mm}
]
% Schematic world frame (purely illustrative)
\node[draw=none, rounded corners, minimum width=100mm, minimum height=45mm, fill=black!2] (frame) {};

\node[region/blue,   anchor=west] (na) at ($(frame.west)+(0.2,0)$) {North America\\(NA cluster)};
\node[region/green]              (eu) at ($(frame.center)+(0,1.0)$) {Europe\\(EU cluster)};
\node[region/orange, anchor=east] (ap) at ($(frame.east)+(-0.2,0)$) {Asia-Pacific\\(APAC cluster)};

\node[user, above=of na] (userna) {Users (NA)};
\node[user, above=of eu] (usereu) {Users (EU)};
\node[user, above=of ap] (userap) {Users (APAC)};

\draw[llm/arrow] (userna) -- node[llm/label,pos=0.5,right]{nearest} (na.north);
\draw[llm/arrow] (usereu) -- node[llm/label,pos=0.5,right]{nearest} (eu.north);
\draw[llm/arrow] (userap) -- node[llm/label,pos=0.5,right]{nearest} (ap.north);

\draw[llm/arrow/dashed] (na.east) .. controls +(2.0,0.8) and +(-2.0,0.8) ..
    (eu.west) node[llm/label,pos=0.52,above,sloped,yshift=2pt] {failover};
\draw[llm/arrow/dashed] (eu.east) .. controls +(2.0,-0.8) and +(-2.0,-0.8) ..
    (ap.west) node[llm/label,pos=0.52,below,sloped,yshift=-2pt] {failover};
\draw[llm/arrow/dashed] (na.south east) .. controls +(1.4,-1.2) and +(-1.4,-1.2) ..
    (ap.south west) node[llm/label,pos=0.5,below,sloped] {overflow};

\node[draw=black!40, rounded corners=4pt, line width=0.6pt, align=left,
      anchor=north, fill=white, inner sep=4mm, font=\footnotesize]
      at ($(frame.south)+(0,-0.5)$) {%
\textbf{Notes}\\[2pt]
...
\begin{llmfigbox}
\begin{tikzpicture}[
  initial/.style     ={llm/box/blue,   align=left, minimum width=4.1cm, minimum height=1.1cm, font=\footnotesize},
  intermediate/.style={llm/box/green,  align=left, minimum width=4.3cm, minimum height=1.2cm, font=\footnotesize},
  mature/.style      ={llm/box/orange, align=left, minimum width=4.5cm, minimum height=1.2cm, font=\footnotesize}
]

% Key milestones box - centered at top
\node[draw=black!40, rounded corners=4pt, line width=0.6pt, align=left,
      fill=white, inner sep=3mm, anchor=north, font=\footnotesize]
    at (7.5,2.2) {%
...
% Timeline axis
\draw[line width=0.7pt, color=black!75] (0,0) -- (15,0);

\foreach \x in {0, 7, 14}
  \draw[line width=0.7pt, color=black!75] (\x,0.15) -- (\x,-0.15);
...
\draw[llm/arrow] (0,-0.2)  -- (m0.north west) node[pos=0.5,right=2mm,font=\footnotesize\bfseries] {Initial Stage};
\draw[llm/arrow] (7,-0.2)  -- (m1.north)      node[pos=0.5,right=2mm,font=\footnotesize\bfseries] {Intermediate};
\draw[llm/arrow] (14,-0.2) -- (m2.north east) node[pos=0.5,right=2mm,font=\footnotesize\bfseries] {Mature};

\end{tikzpicture}
\end{llmfigbox}
\caption{Incremental scaling investments can deliver order-of-magnitude throughput gains while keeping cost growth sub-linear, as \ishtar{}'s timeline shows. The system progressed from a single-GPU prototype to a multi-region deployment by layering batching, caching, speculative decoding, and predictive autoscaling---each increment paying for itself before the next was added.}

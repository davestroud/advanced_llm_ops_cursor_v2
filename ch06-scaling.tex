...
Spot (AWS), preemptible (GCP), and low-priority (Azure) instances offer GPU capacity at 60--90\% discounts compared to on-demand pricing, but they can be reclaimed by the cloud provider with as little as 30 seconds notice. For LLM workloads, leveraging spot capacity requires careful engineering to handle preemption without losing user-visible work.
...
\begin{itemize}
    ...
    \item \textbf{Interactive serving}: spot savings of 40--60\% for burst capacity, but preemption can cause P99 latency spikes of 5--15 seconds during failover. Acceptable only for non-SLO-critical traffic tiers.

...
\begin{tikzpicture}[
  font=\small,
  gpu/.style={rectangle, rounded corners=3pt, draw=none, minimum width=18mm, minimum height=10mm, align=center, inner sep=3pt},
  layer/.style={rectangle, rounded corners=2pt, draw=none, minimum width=14mm, minimum height=7mm, align=center, inner sep=2pt, font=\footnotesize},
  arr/.style={-{Stealth[length=2mm]}, line width=0.8pt, draw=black!60},
  comm/.style={<->, line width=0.6pt, draw=red!60, dashed},
...
\node[gpu, fill=blue!20] (dp1) at (-1.8, 1.8) {GPU 0\\Full Model};
\node[gpu, fill=blue!20] (dp2) at (0, 1.8) {GPU 1\\Full Model};
\node[gpu, fill=blue!20] (dp3) at (1.8, 1.8) {GPU 2\\Full Model};

\node[font=\footnotesize, text=black!60, align=center] at (0, 0.8) {Each GPU holds a\\complete model replica};

% Request arrows
\node[font=\footnotesize] (req) at (0, 2.8) {Requests};
\draw[arr] (req.south) -- (dp1.north);
\draw[arr] (req.south) -- (dp2.north);
\draw[arr] (req.south) -- (dp3.north);

% ============ (b) Tensor Parallelism ============
\node[titlestyle] (tptitle) at (6, 3.2) {(b) Tensor Parallelism};

% Single layer split across GPUs
\node[layer, fill=green!25] (tp1) at (4.5, 2.2) {Slice 0};
\node[layer, fill=green!25] (tp2) at (6.0, 2.2) {Slice 1};
\node[layer, fill=green!25] (tp3) at (7.5, 2.2) {Slice 2};

\node[font=\footnotesize, text=black!60] at (6.0, 2.8) {Single Layer};

% All-reduce arrows
\draw[comm] (tp1.east) -- (tp2.west);
\draw[comm] (tp2.east) -- (tp3.west);

\node[layer, fill=green!15] (tpl2a) at (4.5, 1.2) {Slice 0};
\node[layer, fill=green!15] (tpl2b) at (6.0, 1.2) {Slice 1};
\node[layer, fill=green!15] (tpl2c) at (7.5, 1.2) {Slice 2};

\draw[arr] (tp1.south) -- (tpl2a.north);
\draw[arr] (tp2.south) -- (tpl2b.north);
\draw[arr] (tp3.south) -- (tpl2c.north);
\draw[comm] (tpl2a.east) -- (tpl2b.west);
\draw[comm] (tpl2b.east) -- (tpl2c.west);

\node[font=\footnotesize, text=black!60, align=center] at (6.0, 0.5) {Matrix ops split across GPUs;\\all-reduce sync per layer};

% ============ (c) Pipeline Parallelism ============
\node[titlestyle] (pptitle) at (12, 3.2) {(c) Pipeline Parallelism};

\node[gpu, fill=orange!20] (pp1) at (12, 2.4) {GPU 0\\Layers 0--7};
\node[gpu, fill=orange!25] (pp2) at (12, 1.4) {GPU 1\\Layers 8--15};
\node[gpu, fill=orange!30] (pp3) at (12, 0.4) {GPU 2\\Layers 16--23};

\draw[arr] (pp1.south) -- (pp2.north);
\draw[arr] (pp2.south) -- (pp3.north);

\node[font=\footnotesize, text=black!60, align=center, right=8mm of pp2] {Sequential stages;\\microbatches\\in flight};

...
def evaluate_ci_gate(
    ...
    """
    Evaluate multiple CI gates with configurable metric types.

    Supports three metric comparison modes:
      - "exact_match": binary pass/fail per example
      - "fuzzy_match": continuous scores with bootstrap
      - "semantic_similarity": embedding-based comparison

    Args:
        metrics: Dict mapping metric names to their data:
            {
              "groundedness": {
                "type": "fuzzy_match",
                "baseline": [...], "candidate": [...],
                "threshold": 0.5, "confidence": 0.95
              },
              "toxicity": {
                "type": "exact_match",
                "baseline": [...], "candidate": [...],
    ...
    Returns:
        Dict mapping metric names to GateResult objects

    Raises:
        ValueError: If metric type is unknown or data invalid
    """
    results: Dict[str, GateResult] = {}

    for name, spec in metrics.items():
        metric_type = spec.get("type", "fuzzy_match")
        baseline = spec.get("baseline", [])
        candidate = spec.get("candidate", [])

        if not baseline or not candidate:
            raise ValueError(
                f"Metric '{name}': baseline and candidate "
                f"data must be non-empty"
            )
        if len(baseline) != len(candidate):
            raise ValueError(
                f"Metric '{name}': baseline ({len(baseline)}) "
                f"and candidate ({len(candidate)}) length mismatch"
            )

        try:
            if metric_type == "exact_match":
                # Binary flags: use McNemar's test
                result = mcnemar_test(
                    baseline, candidate,
                    alpha=spec.get("alpha", 0.05)
                )
            elif metric_type == "fuzzy_match":
                # Continuous scores: use paired bootstrap
                result = paired_bootstrap_test(
                    baseline, candidate,
                    confidence_level=spec.get("confidence", 0.95),
                    min_improvement=spec.get("threshold", 0.5)
                )
            elif metric_type == "semantic_similarity":
                # Embedding similarity: bootstrap on cosine diffs
                result = paired_bootstrap_test(
                    baseline, candidate,
                    confidence_level=spec.get("confidence", 0.95),
                    min_improvement=spec.get("threshold", 0.02)
                )
            else:
                raise ValueError(
            ...
            # Override metric name from config key
            result.metric_name = name
            results[name] = result

        except Exception as e:
            # Return a failing gate on error
...
def summarize_gate_results(
    ...
    """
    Summarize all gate results into an overall decision.
...
if __name__ == "__main__":
    # Define metrics with types and thresholds
    gate_metrics = {
        "groundedness": {
            "type": "fuzzy_match",
            "baseline": [0.82, 0.85, 0.79, 0.88],  # 1000 scores
            "candidate": [0.87, 0.89, 0.84, 0.91],
            "threshold": 0.5,
            "confidence": 0.95
        },
        "toxicity": {
            "type": "exact_match",
            "baseline": [False, False, True, False],  # 1000 flags
            "candidate": [False, False, False, False],
            "alpha": 0.05
        },
        "semantic_coherence": {
            "type": "semantic_similarity",
            "baseline": [0.91, 0.89, 0.93, 0.87],
            "candidate": [0.93, 0.91, 0.94, 0.90],
            "threshold": 0.02
        }
    }

    results = evaluate_ci_gate(gate_metrics)
    overall, summary = summarize_gate_results(results)

    print(summary)
    print(f"\nOverall CI gate: {'PASSED' if overall else 'FAILED'}")
\end{minted}
\end{codebox}

...
\textbf{Close the loop from production} & Mine traces and failures into reusable CI datasets and ``hard'' slices, so that incidents become regression tests \cite{phoenix_rag,trulens}. \\
\bottomrule
\end{tabularx}
\end{table}
...
\rowcolor{blue!8}
\textbf{Post-merge on main (hours)} &
...
\rowcolor{orange!10}
\textbf{Nightly/Batch (cost-aware)} &
...
\rowcolor{red!8}
\textbf{Canary/Shadow (real traffic)} &
...
\begin{llmfigbox}
% Eval pipeline: stage colors carry semantics from the book contract.
%   blue   = repository / source-of-truth event (commit, baseline)
%   green  = control gate (pre-merge guards)
%   orange = decision-gating evaluation (post-merge, callouts)
%   purple = batch / fairness sweep
%   gray   = production-facing canary; dashed = feedback loop.
\begin{tikzpicture}[
  node distance=24mm and 26mm,
  stage/.style={align=center, minimum width=60mm, minimum height=42mm, inner sep=12pt, font=\normalsize},
  note/.style={align=left, font=\normalsize},
  callout/.style={rounded corners=5pt, inner sep=8pt, draw=black!20, line width=0.6pt,
                  fill=llmgray!8},
]

\node[llm/box/blue,   stage]                          (commit) {\bfseries /commit\\code, prompts, data diffs};
\node[llm/box/green,  stage, right=of commit]         (pre)    {\bfseries Pre-merge\\\(\sim\)50--200 det. cases\\guards \& format checks};
\node[llm/box/orange, stage, right=of pre]            (post)   {\bfseries Post-merge\\1--5k cases\\stat. gating vs. baseline};
\node[llm/box/purple, stage, right=of post]           (night)  {\bfseries Nightly/Batch\\5k--20k cases\\adv., fairness, long ctx};
\node[llm/box/gray,   stage, right=of night]          (canary) {\bfseries Canary/Shadow\\live slice\\drift, safety, SLOs};

\draw[llm/arrow] (commit) -- (pre);
\draw[llm/arrow] (pre)    -- (post);
\draw[llm/arrow] (post)   -- (night);
\draw[llm/arrow] (night)  -- (canary);

\node[note, right=10mm of canary, callout,
      text width=50mm, align=left, anchor=west] (feedback) {%
    \textbf{Feedback:}\\
...
\draw[llm/arrow/dashed]
  (canary.east) -- ++(10mm,0)
  |- (looptop)
  -| (commit.north);

% Artifact bundle: dashed envelope around the gating stages.
\node[draw=llmblue!60, dashed, line width=0.8pt, fill=none, rounded corners=6pt,
      fit=(pre)(post)(night), inner sep=14pt,
      label={[note, below=8mm, font=\normalsize]south:%
        \textbf{Artifacts:} versioned datasets, judges \& prompts; metric code; thresholds; HTML/JSON reports}] (bundle) {};

\node[note, above=12mm of pre,    callout, text width=55mm, align=center] {regex PII / inj.; schema};
\node[note, above=12mm of post,   callout, text width=60mm, align=center] {helpfulness, groundedness; McNemar, bootstrap};
\node[note, above=12mm of night,  callout, text width=65mm, align=center] {red-team, NLI contradictions, fairness slices};
\node[note, above=12mm of canary, callout, text width=58mm, align=center] {online evals; SLO monitors; rollback};

\node[llm/box/gray, stage, below=28mm of bundle, minimum width=80mm, minimum height=28mm]
      (legend) {\textbf{Legend}};
\node[note, below=6mm of legend, text width=80mm, align=left] {
...
\subsubsection{From Failing Slices to Regression Tests}\label{subsec:ch04-failing-slices-regression}

The feedback loop shown in Fig.~\ref{fig:ch04_eval_stages_pipeline} is the engine that converts production incidents into durable test coverage. Concretely, the mechanism works as follows. When an evaluation slice fails during canary or shadow deployment---for example, groundedness drops below threshold on a ``high-stakes policy'' slice---the failing examples (input prompts, retrieved contexts, model outputs, and evaluator scores) are automatically captured and tagged with the failure mode, slice identifier, and timestamp. These examples are then appended to a growing \emph{regression test suite} that lives alongside the golden and adversarial datasets in the CI pipeline.

On every subsequent CI run, the pipeline replays these regression examples against the candidate release. Because the regression suite only grows (new failures are added; passing examples are never removed), it creates a \emph{monotonically improving test baseline}: the system must demonstrate that every previously observed failure mode has been addressed or at minimum not reintroduced. Over time, this transforms ad hoc incident responses into systematic, evidence-backed quality assurance. Teams that adopt this pattern report that within three to six months, their regression suites capture the long tail of domain-specific edge cases that static benchmark suites miss, making the evaluation pipeline increasingly representative of real-world failure modes.
...
One promising development is the use of strong models (e.g., GPT-4 class) as ``judges.'' These model-graded evaluations approximate human ratings at scale by providing pairwise comparisons or rubric-based scores \cite{zheng2023judge}. While efficient, they are not without weaknesses. Studies show that model judges exhibit position bias, verbosity bias, and a tendency to reward surface polish over factual rigor \cite{chen2024humansJudge}. To mitigate these effects, best practice is to randomize answer ordering, enforce symmetric prompting, and adopt blind judging protocols. Requiring structured rationales, as in G-Eval-style rubrics, further reduces spurious preference. Using multiple independent judges and analyzing disagreement provides another safeguard, while periodic calibration against human anchor sets helps track drift. For auditability, judge prompts, model versions, seeds, and rationales should be stored, and inter-rater reliability (e.g., Krippendorff's $\alpha$) should be reported.

\subsubsection{Quantifying LLM-as-Judge Biases}\index{LLM-as-judge!bias}
Empirical studies have identified several systematic biases in model-graded evaluation that practitioners must account for:

\begin{itemize}
    \item \textbf{Position bias} (approximately 10--15\% score variance): when comparing two answers, judge models systematically prefer the answer presented first (or second, depending on the model). Randomizing presentation order and averaging scores across orderings reduces this effect but does not eliminate it entirely.
    \item \textbf{Verbosity bias}: longer, more detailed responses receive higher scores even when the additional content is redundant or tangential. This can be mitigated by including explicit rubric criteria that penalize unnecessary verbosity and by normalizing scores by response length.
    \item \textbf{Self-enhancement bias}: models tend to rate their own outputs higher than outputs from other models, particularly when the judge and the candidate share the same base architecture. Using a different model family for judging, or employing multiple judges from different providers, reduces this confound.
\end{itemize}

\subsubsection{Judge Calibration Protocol}
To ensure that model-graded scores are meaningful and stable, teams should implement a calibration protocol before relying on LLM-as-judge in CI gates:

\begin{enumerate}
    \item \textbf{Anchor set construction.} Assemble a set of 50--200 examples with known quality levels: clearly good examples (human-validated, factually correct, well-cited), clearly bad examples (hallucinated, off-topic, unsafe), and borderline cases. Human annotators should independently score these examples using the same rubric the judge will use.
    \item \textbf{Judge scoring.} Run the judge model on the anchor set, recording scores, rationales, and confidence signals. Use identical prompting and decoding parameters that will be used in CI.
    \item \textbf{Correlation analysis.} Compute Pearson or Spearman correlation between judge scores and human scores. A correlation below 0.7 indicates that the rubric or judge model needs adjustment. Report Krippendorff's $\alpha$ or Cohen's $\kappa$ for inter-rater reliability.
    \item \textbf{Rubric refinement.} If correlation is insufficient, adjust the scoring rubric: add explicit criteria, provide more exemplars in the judge prompt, or switch to a more capable judge model. Re-run until correlation exceeds the threshold.
    \item \textbf{Periodic re-calibration.} Schedule quarterly re-calibration against fresh human annotations to detect judge drift caused by model version updates or distribution shifts in production traffic.
\end{enumerate}

\PitfallBox{Over-reliance on model-graded evaluation can create a false sense of quality assurance. LLM judges share systematic biases (position, verbosity, self-enhancement) that can inflate scores by 10--15\% on specific answer types. Never use a single judge model as the sole arbiter of quality in CI gates. Instead, triangulate with classical metrics (ROUGE, exact match), multiple judge models from different providers, and periodic human calibration. Treat judge scores as scalable \emph{signals}, not ground truth---and always report the calibration correlation alongside evaluation results so reviewers can assess confidence.}

\subsection{Evaluating Groundedness and Hallucination in RAG Systems}
In RAG systems, evaluation must specifically address groundedness. This requires operationalizing three dimensions: context relevance (did retrieval bring the right material?), answer relevance (does the response address the question?), and answer faithfulness (is the response supported by the retrieved evidence?). Emerging frameworks such as RAGAS and ARES formalize reference-free and reference-light metrics, while annotated corpora like RAGTruth enable supervised training of hallucination detectors \cite{ragas_eacl2024,ares,ragtruth}. A best practice is to combine these model-graded scores with retrieval metrics such as hit rate, MRR, or NDCG, and to monitor attribution fidelity through overlap of evidence spans and citations.

\subsection{Adversarial and Metric-Based Checks in CI}
Robust evaluation must go beyond general quality judgments to include adversarial and metric-based checks. For toxicity and bias, nightly suites should include datasets such as RealToxicityPrompts, CrowS-Pairs, and BBQ \cite{gehman2020realtoxicity,crowsPairs2020,bbq2022}. For security, evaluations should simulate attacks from the OWASP LLM Top-10, including prompt injection and insecure output handling, and incorporate red-team playbooks aligned with MITRE ATLAS patterns such as model extraction and evasion \cite{owasp_llm}. Grounding can be checked via retrieval coverage thresholds, compliance with citation requirements, and contradiction detection between answers and retrieved passages, often through natural language inference (NLI) classifiers.
...
\begin{codebox}{Complete evaluation harness configuration implementing staged CI/CD validation with deterministic inputs, statistical significance gates, and automated rollback triggers. This configuration ensures that model updates pass rigorous quality checks across pre-merge, post-merge, and nightly stages before reaching production.}{lst:ch04_eval_harness}
\begin{minted}{yaml}
...
\begin{codebox}{CI gate implementation combining paired bootstrap confidence intervals for continuous metrics (e.g., groundedness scores) with McNemar's test for binary safety metrics (e.g., toxicity flags). This statistical gating logic ensures that model promotions are backed by rigorous evidence rather than point estimates, preventing regressions from reaching production.}{lst:ch04_ci_gate}
\begin{minted}{python}
import numpy as np
...
def paired_bootstrap_test(
    ...
    return GateResult(
        ...
        p_value=None,
        message=f"diff={mean_diff:.3f} CI=[{lower_bound:.3f},{upper_bound:.3f}]",
    )

def mcnemar_test(baseline_flags, candidate_flags, alpha=0.05) -> GateResult:
    # ... (analogous: count discordant pairs (b, c), compute chi2 = (b-c)^2/(b+c),
    #      p_value = 1 - chi2.cdf(chi2, df=1); pass if rate non-increasing AND
    #      p_value > alpha. Full source in companion repo.) ...
    ...

def evaluate_ci_gate(metrics: Dict[str, Dict]) -> Dict[str, GateResult]:
    # Dispatcher: routes "fuzzy_match"/"semantic_similarity" -> paired_bootstrap_test,
    # "exact_match" -> mcnemar_test. See companion repo for the dispatcher body and
    # error envelope. Returns one GateResult per metric.
    ...

def summarize_gate_results(results) -> Tuple[bool, str]:
    overall = all(r.passed for r in results.values())
    lines = [f"  [{'PASS' if r.passed else 'FAIL'}] {n}: {r.message}"
             for n, r in results.items()]
    return overall, "\n".join(lines)
\end{minted}
\end{codebox}

\subsection{Cost Management}\index{cost management!CI/CD}
...
\section{Fine-Tuning-Aware Workflows}\index{fine-tuning!workflows}
...
\section{CI/CD for Fine-Tuned Models}\index{fine-tuning!CI/CD}\index{LoRA!evaluation gates}
\label{sec:ch04-finetuning-cicd}

When organizations adopt parameter-efficient fine-tuning methods such as LoRA (Low-Rank Adaptation)~\cite{Hu2022LoRA}, the CI/CD pipeline must evolve to accommodate a new class of release artifacts: adapter weights. Unlike full model updates, LoRA adapters are small delta files layered on top of a frozen base model, making them fast to train and deploy---but they still carry the same risks of behavioral regression, safety drift, and prompt–evaluation coupling that full fine-tunes do. This section describes the evaluation gates, prompt versioning practices, and pipeline integration patterns needed to ship adapter updates safely.

\subsection{LoRA Update Evaluation Gates}
\label{subsec:ch04-lora-eval-gates}

Every LoRA adapter update must pass through a series of evaluation gates before it is promoted to production. These gates are designed to catch regressions that are unique to adapter-based fine-tuning, where narrow domain gains can mask broader capability losses.

\begin{enumerate}
    \item \textbf{Baseline regression tests.}\index{regression testing!LoRA} Before any domain-specific evaluation, the candidate adapter is tested against a general-capability benchmark suite (e.g., a curated subset of MMLU, HellaSwag, or internal reasoning tasks). The goal is to verify that fine-tuning has not degraded the base model's general reasoning, instruction following, or safety behaviors---a phenomenon known as \emph{catastrophic forgetting}~. The gate requires that no metric drops by more than a pre-registered threshold (e.g., 1\% absolute on accuracy) relative to the base model without the adapter.

    \item \textbf{Domain-specific evaluation suite.} The adapter is then evaluated on the domain datasets it was trained to improve. These suites should be stratified by difficulty, topic, and risk level, with slice-level reporting. For \ishtar{}, domain slices include conflict-zone reporting, NGO bulletins, and high-stakes policy summaries. Promotion requires statistically significant improvement on the target domain (paired bootstrap, 95\% CI excluding zero) without degradation on adjacent domains.

    \item \textbf{Safety regression check.} Fine-tuning can subtly shift the model's refusal boundaries and toxicity profile. A dedicated safety gate runs adversarial probes (jailbreak attempts, toxic completion prompts, PII extraction scenarios) and compares incident rates against the previous adapter version using McNemar's test at $\alpha = 0.05$. Any statistically significant increase in safety violations blocks promotion.

    \item \textbf{Comparison against previous adapter version.} The candidate adapter is evaluated side-by-side with the currently deployed adapter on the same evaluation sets. This paired comparison prevents ``version drift,'' where incremental improvements against the base model mask regressions relative to the prior production adapter. Reports include per-slice deltas with confidence intervals.
\end{enumerate}

\subsection{Prompt Versioning Impact on Evaluations}
\label{subsec:ch04-prompt-versioning-impact}

Prompt templates and LoRA adapters are tightly coupled: changing a prompt can invalidate evaluation results obtained with a previous prompt version, and vice versa. Managing this coupling is critical for reproducible CI/CD.

\medskip
\noindent\textbf{Prompt--evaluation coupling.} A LoRA adapter trained with prompt template v1.3 may perform differently under prompt template v1.4, even if neither artifact changed in isolation. Evaluation results are therefore valid only for the specific \emph{(adapter, prompt)} pair under which they were obtained. CI pipelines must track and enforce this pairing: an evaluation report should record both the adapter version and the prompt template version used, and promotion gates should reject results produced with mismatched versions.

\medskip
\noindent\textbf{Prompt versioning strategy.} Adopt semantic versioning for prompt templates, following the convention established in Section~\ref{sec:cicd-structured-prompt-testing}:
\begin{itemize}
    \item \textbf{Major version} (breaking): changes to system message structure, output schema, or safety guardrails that require adapter re-evaluation.
    \item \textbf{Minor version}: additions to few-shot examples, formatting refinements, or expanded instructions that may affect quality but do not break compatibility.
    \item \textbf{Patch version}: typo fixes, whitespace normalization, or comment-only changes with no behavioral impact.
\end{itemize}
Major prompt version bumps should trigger mandatory re-evaluation of all active adapters. Minor bumps should trigger re-evaluation if the adapter's domain overlaps with the changed prompt sections.

\medskip
\noindent\textbf{Automated re-evaluation triggers.}\index{prompt versioning} When a prompt template change is merged, the CI pipeline should automatically determine whether dependent adapters need re-evaluation. A practical implementation maintains a dependency graph: each adapter declares which prompt template versions it has been validated against. When a prompt template is updated beyond a patch version, the pipeline enqueues re-evaluation jobs for all adapters that depend on that template, blocking their deployment until fresh evaluation results confirm continued compliance with promotion criteria.

\subsection{Fine-Tuning Pipeline Integration}
\label{subsec:ch04-finetune-pipeline}

Listing~\ref{lst:ch04_lora_eval_pipeline} shows a CI pipeline stage that evaluates a LoRA adapter against all four gates before permitting promotion.

\begin{codebox}{CI pipeline stage that evaluates a LoRA adapter against four promotion gates: baseline regression testing for catastrophic forgetting, domain-specific evaluation with bootstrap confidence intervals, safety regression checks using McNemar's test, and side-by-side comparison against the previously deployed adapter. All gates must pass before adapter promotion is permitted.}{lst:ch04_lora_eval_pipeline}
\begin{minted}{python}
#!/usr/bin/env python3
"""
CI pipeline stage: LoRA adapter evaluation gates.
Runs baseline regression, domain-specific, safety, and
version-comparison checks before adapter promotion.
"""
from dataclasses import dataclass
...
) -> List[AdapterEvalResult]:
    """Run all four evaluation gates for a LoRA adapter."""
    results = []

    # Gate 1: Baseline regression (general capabilities)
    baseline_metrics = run_general_benchmarks(
        base_model, adapter_path, prompt_template_version,
        datasets=["mmlu_subset", "hellaswag_subset", "reasoning_v2"]
    )
    max_drop = max(
        base - cand
        for base, cand in zip(
            baseline_metrics["base_scores"],
            baseline_metrics["adapter_scores"]
        )
    )
    results.append(AdapterEvalResult(
        gate_name="baseline_regression",
        passed=max_drop <= 0.01,  # 1% absolute threshold
        metrics=baseline_metrics["summary"],
        details=f"Max drop: {max_drop:.4f} (threshold: 0.01)"
    ))

    # Gate 2: Domain-specific evaluation
    domain_metrics = run_domain_evaluation(
        base_model, adapter_path, prompt_template_version,
        slices=["conflict_reports", "ngo_bulletins", "policy"]
    )
    ci_excludes_zero = domain_metrics["bootstrap_ci_lower"] > 0
    results.append(AdapterEvalResult(
        gate_name="domain_specific",
        passed=ci_excludes_zero,
        metrics=domain_metrics["per_slice"],
        details=f"CI: [{domain_metrics['bootstrap_ci_lower']:.3f}, "
                f"{domain_metrics['bootstrap_ci_upper']:.3f}]"
    ))

    # Gate 3: Safety regression check
    safety_metrics = run_safety_probes(
        base_model, adapter_path, prompt_template_version,
        suites=["jailbreak_v2", "toxic_completions", "pii_extraction"]
    )
    results.append(AdapterEvalResult(
        gate_name="safety_regression",
        passed=safety_metrics["mcnemar_p"] > 0.05,
        metrics=safety_metrics["incident_rates"],
        details=f"McNemar p={safety_metrics['mcnemar_p']:.4f}"
    ))

    # Gate 4: Comparison vs. previous adapter
    if previous_adapter_path:
        comparison = run_paired_comparison(
            base_model, adapter_path, previous_adapter_path,
            prompt_template_version
        )
        results.append(AdapterEvalResult(
            gate_name="version_comparison",
            passed=comparison["no_regression"],
...
def promotion_decision(results: List[AdapterEvalResult]) -> bool:
    """All gates must pass for promotion."""
    all_passed = all(r.passed for r in results)
    for r in results:
        status = "PASS" if r.passed else "FAIL"
        print(f"  [{status}] {r.gate_name}: {r.details}")
    return all_passed
\end{minted}
\end{codebox}

\BestPracticeBox{
\textbf{Key principles for fine-tuning CI/CD:}
\begin{itemize}[leftmargin=1.5em, itemsep=3pt]
    \item Always evaluate adapters against both domain-specific and general-capability benchmarks to catch catastrophic forgetting early.
    \item Track prompt--adapter version pairs explicitly; invalidate stale evaluation results when either artifact changes.
    \item Use semantic versioning for prompt templates and automate re-evaluation triggers on version bumps.
    \item Gate promotion on statistical tests (paired bootstrap for continuous metrics, McNemar for binary safety flags), not on point estimates.
    \item Archive adapter weights, evaluation reports, prompt versions, and training data references as a single auditable promotion artifact.
\end{itemize}
}
...
Deploying LLMs requires staged risk mitigation, as outlined in Table~\ref{tab:ch04_deployment_risk_mitigation}:
...
\begin{codebox}{Argo Rollouts canary deployment configuration with LLM-specific analysis gates for groundedness, toxicity, and latency. Traffic is gradually shifted from 1\% to 100\% with Prometheus-backed quality checks at each step, enabling automated rollback if behavioral regressions are detected during progressive delivery.}{lst:ch04_deployment_strategy}
\begin{minted}{yaml}
  ...
  strategy:
    canary:
      ...
      analysis:
        ...
        # ... (failureCondition is the symmetric negation; omitted) ...
  # ... (selector, template, container resources omitted; standard Argo
  #      Rollout pod spec referencing image ishtar/llm-inference:v1.4.0
  #      with 1 GPU + 40 GiB memory request) ...
---
apiVersion: argoproj.io/v1alpha1
...
spec:
  ...
  # ... (toxicity metric: sum(rate(toxicity_flags)) / sum(rate(requests)),
  #      pass <= 0.05, fail > 0.10. Latency metric: histogram_quantile(0.95, ttft),
  #      pass <= 600 ms, fail > 1000 ms. Same provider/interval/count shape as
  #      groundedness above; full source in companion repo.) ...
\end{minted}
\end{codebox}

\noindent\textbf{From patterns to practice.}
Shadow testing (\emph{a.k.a.} traffic mirroring or dark launching) provides the lowest-risk starting point: production queries are duplicated to a candidate model, whose outputs are logged but never surfaced to users. For LLMs, shadowing must be \emph{side-effect safe}: tool invocations, database writes, or external API calls should be simulated or routed to sandboxes to avoid unintended actions. Shadow traffic is invaluable for catching domain-specific regressions, style drift, or retrieval mismatches before any user is affected, and it allows calibration of online evaluators (helpfulness, groundedness, safety) against real distributions \cite{mlflow_llm_eval,phoenix_rag,trulens}.
...
\item \textbf{Live A/B:} At 10--25\% traffic, run a pre-registered test plan with slice-level dashboards and online evaluators; stop for harm rate inflation or hallucination spikes beyond error budgets \cite{mlflow_llm_eval,phoenix_rag,trulens}.  
...
\begin{itemize}[leftmargin=1.5em, itemsep=4pt, topsep=2pt]
  ...
  \item \textbf{LangFuse~\cite{langfuse_docs}:} Open-source tracing and monitoring for multi-step chains; supports curating edge-case datasets for CI.
  \item \textbf{LangGraph~\cite{langgraph_docs}:} Explicit graph-based agent orchestration with LangFuse/LangSmith integration for dependency testing.
  \item \textbf{Other tools:} TruLens~\cite{trulens}, Ragas~\cite{ragas_eacl2024}, and PromptLayer provide prompt/version tracking, RAG-focused evaluation, and explainability metrics.
\end{itemize}

Table~\ref{tab:ch04_obs_tools} compares these observability tools and their roles in CI/CD workflows.
\end{tcolorbox}
...
\section{Structured Prompt Testing}\index{prompt testing!structured}
...
\begin{codebox}{Versioned prompt template with semantic versioning, changelog, test suite references, and regression baseline tracking. This structure enables reproducible CI/CD by coupling prompt versions to evaluation results, safety guardrails, and deployment criteria, ensuring that prompt changes are validated before production release.}{lst:ch04_prompt_versioning}
\begin{minted}{json}
{
...
\item \textbf{Canary:} ramp to 1--10\% sticky traffic with online evaluators; gate on safety incident budgets and latency/cost SLOs; roll back on breach \cite{mlflow_llm_eval}.  
\end{enumerate}
\end{tcolorbox}
...
(5) \emph{Missing fault injection}: chains pass only in happy paths; add dependency tests and record--replay harnesses \cite{phoenix_rag}.  
...
Multi-agent graphs risk \emph{runaway loops} (planner--retriever ping-pong), prompt verbosity cascades, and tool storms that inflate latency and spend. CI should enforce graph-level budgets: maximum steps per request, maximum cumulative tokens, and per-tool call ceilings. Online, error-budget style alerting (e.g., EWMA or CUSUM on loop rate, p95 latency, \$ per 1k tokens) triggers automated rollback or path gating. Observability should attribute costs to nodes and edges, so teams can identify the worst-offending interactions and re-tune prompts or introduce early-exit heuristics \cite{mlflow_llm_eval}.
...
\item \textbf{Node contracts:} Validate schema conformance and safety clauses on every edge; run adversarial inputs from OWASP/ATLAS playbooks on planner and executor boundaries \cite{owasp_llm}.  
\item \textbf{Path metrics:} Compute per-path helpfulness and groundedness via model-graded rubrics, calibrated to human anchors; compute retrieval hit rate and citation coverage for RAG steps \cite{trulens,phoenix_rag,ragas_eacl2024}.  
...
(3) \emph{Attribution fog}: lack of graph-aware tracing hides the failing node; require structured spans and replay bundles.  
(4) \emph{Adversarial gaps}: no targeted tests at agent boundaries; incorporate OWASP/ATLAS scenarios in nightly suites \cite{owasp_llm}.  
...
\textbf{Observability feeds CI} & Traces, retrieved contexts, and tool-call logs are captured with a replayable contract so production incidents become regression tests \cite{phoenix_rag,trulens}. \\
\textbf{Supply-chain integrity is enforced} & Provenance and signing (SLSA; signed images and attestations) and secretless cloud auth (OIDC) are required before promotion \cite{slsa_levels,sigstore_cosign,cosign_attestations,gha_oidc_concepts,gha_oidc_cloud}. \\

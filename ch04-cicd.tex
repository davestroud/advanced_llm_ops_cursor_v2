...
\begin{lstlisting}[
  ...
  style=springerfloat,
  language=Python,
  caption={CI pipeline stage that evaluates a LoRA adapter against four promotion gates: baseline regression testing for catastrophic forgetting, domain-specific evaluation with bootstrap confidence intervals, safety regression checks using McNemar's test, and side-by-side comparison against the previously deployed adapter. All gates must pass before adapter promotion is permitted.},
  label={lst:ch04_lora_eval_pipeline}
]
#!/usr/bin/env python3
"""
...
\begin{lstlisting}[
  float=htbp,
  style=springerfloat,
  caption={Argo Rollouts canary deployment configuration with LLM-specific analysis gates for groundedness, toxicity, and latency. Traffic is gradually shifted from 1\% to 100\% with Prometheus-backed quality checks at each step, enabling automated rollback if behavioral regressions are detected during progressive delivery.},
...
\begin{lstlisting}[
  float=htbp,
  style=springerfloat,
  caption={Versioned prompt template with semantic versioning, changelog, test suite references, and regression baseline tracking. This structure enables reproducible CI/CD by coupling prompt versions to evaluation results, safety guardrails, and deployment criteria, ensuring that prompt changes are validated before production release.},

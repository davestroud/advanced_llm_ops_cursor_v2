...
\begin{itemize}[leftmargin=1.5em, itemsep=2pt]
    ...
    \item \emph{Invariant:} The boolean/sentiment polarity of the output must flip.
    \item \emph{Programmatic check:}
\end{itemize}
\begin{minted}[numbers=none]{python}
assert extract_boolean(base_output) != extract_boolean(negated_output)
\end{minted}
...
\begin{itemize}[leftmargin=1.5em, itemsep=2pt]
    ...
    \item \emph{Invariant:} Semantic similarity between outputs exceeds a threshold (e.g., $> 0.85$).
    \item \emph{Programmatic check:}
\end{itemize}
\begin{minted}[numbers=none]{python}
sim = embedding_similarity(output_q, output_q_prime)
assert sim >= 0.85, f"Paraphrase invariance violated: sim={sim:.2f}"
\end{minted}
...
\begin{itemize}[leftmargin=1.5em, itemsep=2pt]
    ...
    \item \emph{Invariant:} The source output set is a subset of the follow-up output set.
    \item \emph{Programmatic check:}
\end{itemize}
\begin{minted}[numbers=none]{python}
assert set(output_3).issubset(set(output_5)), "Subset consistency violated"
\end{minted}
...
\begin{codebox}{Circuit breaker sketch for retrieval resilience. When retrieval failures exceed a threshold within a rolling window, the circuit opens and the system degrades gracefully by bypassing retrieval until the cooldown expires.}{lst:ch10_circuit_breaker}
\begin{minted}{python}

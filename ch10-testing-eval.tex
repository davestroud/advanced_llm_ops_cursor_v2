...
\begin{lstlisting}[
  ...
  style=springerfloat,
  language=Python,
  caption={Unit test implementation for prompt template validation demonstrating structural and semantic invariant checking. These tests verify JSON output format correctness and date conversion accuracy, providing fast targeted feedback during development.},
...
\begin{lstlisting}[
  ...
  style=springerfloat,
  language=Python,
  caption={Integration test for a RAG chain validating end-to-end answer structure, citation relevance, and hallucination detection. The test exercises the full retrieval-generation pipeline and asserts that every factual claim is supported by cited passages.},
...
\begin{lstlisting}[
  float=t,
  style=springerfloat,
  caption={Adversarial test suite structured as a YAML configuration covering the OWASP LLM Top 10. Each test case specifies a category, adversarial payload, expected safe behavior, and a machine-checkable validation rule.},
...
\begin{lstlisting}[
  ...
  style=springerfloat,
  language=Python,
  caption={Property-based metamorphic test sketch enforcing paraphrase invariance and schema validity. For each seed case, paraphrased inputs must produce structurally valid JSON with semantically similar answers above a tunable similarity threshold.},
...
\end{itemize}
\begin{lstlisting}[language=Python, style=springer, numbers=none]
...
\begin{lstlisting}[
  ...
  style=springerfloat,
  language=Python,
  caption={Metrics aggregation script for slice-based evaluation reporting. The function computes correctness, faithfulness, and p95 latency per demographic or difficulty slice, then produces global aggregates for dashboard and gate consumption.},
...
\begin{lstlisting}[
  float=t,
  style=springerfloat,
  caption={LLM-as-judge prompt template with structured JSON output. The rubric defines hard-fail criteria for faithfulness and safety violations alongside 1--5 scores for correctness, attribution, and clarity, enabling automated gate decisions.},
...
\begin{lstlisting}[
  float=t,
  style=springerfloat,
  caption={Evaluation harness configuration in YAML defining datasets, metrics, quality gates, and reporting options. This declarative format allows teams to version-control their evaluation criteria and run reproducible offline evaluations across model candidates.},
...
\begin{lstlisting}[
  ...
  style=springerfloat,
  language=Python,
  caption={Paired bootstrap gate that computes a confidence interval on the metric difference between a candidate and baseline model. The gate passes only when the lower bound of the interval exceeds a configurable regression tolerance.},
...
\begin{lstlisting}[
  float=t,
  style=springerfloat,
  caption={Minimal annotation record schema for human evaluation. Each record captures independent rater scores with hard-fail flags and an adjudicated consensus, enabling inter-rater reliability analysis and judge calibration.},
...
\begin{lstlisting}[
  float=t,
  style=springerfloat,
  caption={Load test sketch for an LLM endpoint using k6-style pseudocode. The script ramps virtual users from 10 to 50 over a soak period and enforces thresholds on error rate and p95 latency to validate production readiness under sustained load.},
...
\begin{lstlisting}[
  float=t,
  style=springerfloat,
  caption={CI job sketch that runs an offline evaluation suite as part of a GitHub Actions workflow. The job installs dependencies, executes the evaluation harness against a candidate model, and fails the build if quality gates do not pass.},
...
\begin{lstlisting}[
  ...
  style=springerfloat,
  language=Python,
  caption={Circuit breaker sketch for retrieval resilience. When retrieval failures exceed a threshold within a rolling window, the circuit opens and the system degrades gracefully by bypassing retrieval until the cooldown expires.},

...
% Dashboard layout - tighter outer box eliminates dead space below charts
\draw[fill=gray!5, draw=black!25, line width=1.2pt, rounded corners=6pt] (0,5.4) rectangle (18,14);
...
\draw[fill=white, draw=llmblue!60!black, line width=1.2pt, rounded corners=4pt] (0.8,9.5) rectangle (8.2,12.8);
\node[anchor=north west, font=\normalsize\bfseries, text=llmblue!90!black] at (1.0,12.6) {Correctness Trend};
% Y-axis scale labels - positioned inside chart area
...
\draw[llmblue!80!black, line width=3pt] (1.3,10.3) -- (2.5,10.5) -- (3.7,10.7) -- (4.9,10.9) -- (6.1,11.1) -- (7.3,11.3);
...
\node[anchor=east, font=\normalsize\bfseries, text=llmblue!90!black] at (7.8,10.8) {0.92 $\uparrow$};
% X-axis time labels - inside chart area, larger and more visible
...
\draw[fill=white, draw=llmgreen!60!black, line width=1.2pt, rounded corners=4pt] (9.8,9.5) rectangle (17.2,12.8);
\node[anchor=north west, font=\normalsize\bfseries, text=llmgreen!90!black] at (10.0,12.6) {Faithfulness Trend};
% Y-axis scale labels - positioned inside chart area
...
\draw[llmgreen!80!black, line width=3pt] (10.3,11.5) -- (11.5,11.3) -- (12.7,11.1) -- (13.9,10.9) -- (15.1,10.7) -- (16.3,10.5);
% Grid lines - adjusted for taller box
...
% Value indicator - larger font
\node[anchor=east, font=\normalsize\bfseries, text=llmgreen!90!black] at (16.8,10.8) {0.88 $\downarrow$};
% X-axis time labels - inside chart area, larger and more visible
...
\draw[fill=white, draw=llmorange!60!black, line width=1.2pt, rounded corners=4pt] (0.8,6.2) rectangle (8.2,9.5);
\node[anchor=north west, font=\normalsize\bfseries, text=llmorange!90!black] at (1.0,9.3) {Latency Trend};
% Y-axis scale labels (latency in seconds) - positioned inside chart area
...
\draw[llmorange!80!black, line width=3pt] (1.3,8.3) -- (2.5,8.1) -- (3.7,7.9) -- (4.9,7.7) -- (6.1,7.5) -- (7.3,7.3);
% Grid lines - adjusted for taller box
\foreach \x in {1.3,2.5,3.7,4.9,6.1,7.3} {
...
% Value indicator - larger font
\node[anchor=east, font=\normalsize\bfseries, text=llmorange!90!black] at (7.8,7.8) {1.1s $\downarrow$};
% X-axis time labels - inside chart area, larger and more visible
...
\draw[fill=white, draw=llmorange!60!black, line width=1.2pt, rounded corners=4pt] (9.8,6.2) rectangle (17.2,9.5);
\node[anchor=north west, font=\normalsize\bfseries, text=llmorange!90!black] at (10.0,9.3) {Cost Trend};
% Y-axis scale labels (cost in dollars) - positioned inside chart area
...
\draw[llmorange!80!black, line width=3pt] (10.3,8.3) -- (11.5,8.1) -- (12.7,7.9) -- (13.9,7.7) -- (15.1,7.5) -- (16.3,7.3);
% Grid lines - adjusted for taller box
\foreach \x in {10.3,11.5,12.7,13.9,15.1,16.3} {
  \draw[gray!30, line width=0.3pt] (\x,6.8) -- (\x,8.5);
}
...
% Value indicator - larger font
\node[anchor=east, font=\normalsize\bfseries, text=llmorange!90!black] at (16.8,7.8) {\$0.04 $\downarrow$};
% X-axis time labels - inside chart area, larger and more visible
...
\node[box, fill=llmblue!15, minimum width=2.7cm, minimum height=10mm] (input) {Input Query};
\node[box, fill=llmgreen!15, right=of input, minimum width=2.7cm, minimum height=10mm] (golden) {Golden Dataset\\Check};
\node[box, fill=llmorange!15, right=of golden, minimum width=2.7cm, minimum height=10mm] (judge) {LLM-as-Judge};
\node[box, fill=llmpurple!15, right=of judge, minimum width=2.7cm, minimum height=10mm] (semantic) {Semantic\\Similarity};
\node[box, fill=llmpurple!15, below=of judge, minimum width=2.7cm, minimum height=10mm] (aggregate) {Aggregated\\Score};
...
\node[box, fill=llmblue!15, minimum width=2.7cm, minimum height=10mm] (change) {Code Change};
\node[box, fill=llmgreen!15, below=of change, minimum width=2.7cm, minimum height=10mm] (tests) {Run Tests};
\node[box, fill=llmorange!15, below=of tests, minimum width=2.7cm, minimum height=10mm] (compare) {Compare\\Metrics};
\node[box, fill=llmpurple!15, below=of compare, minimum width=2.7cm, minimum height=10mm] (gate) {Gate\\Decision};
\node[box, fill=llmorange!15, left=of gate, minimum width=2.2cm, minimum height=10mm] (block) {Block};
\node[box, fill=llmgreen!15, right=of gate, minimum width=2.2cm, minimum height=10mm] (deploy) {Deploy};

\draw[arrow] (change) -- (tests);
\draw[arrow] (tests) -- (compare);
\draw[arrow] (compare) -- (gate);
\draw[arrow, color=llmorange!70!black] (gate) -- node[above, font=\footnotesize] {fail} (block);
\draw[arrow, color=llmgreen!70!black] (gate) -- node[above, font=\footnotesize] {pass} (deploy);
...
\node[box, fill=llmblue!15, minimum width=2.9cm, minimum height=9mm] (pr) {Pull Request\\(fast gate)};
\node[box, fill=llmgreen!15, right=of pr, minimum width=2.9cm, minimum height=9mm] (merge) {Merge to main\\(nightly gate)};
\node[box, fill=llmorange!15, right=of merge, minimum width=2.9cm, minimum height=9mm] (release) {Pre-release\\(full gate)};
\node[box, fill=llmpurple!15, right=of release, minimum width=2.9cm, minimum height=9mm] (canary) {Canary + monitor\\(online gate)};
...
\begin{llmfigbox}
\begin{tikzpicture}[font=\small, node distance=14mm,
  box/.style={draw=none, rounded corners=5pt, align=center, inner sep=5pt, font=\small\bfseries, minimum width=2.6cm, minimum height=10mm},
  arrow/.style={-{Latex}, line width=1.2pt, color=black!70},
  decision/.style={draw=none, rounded corners=5pt, align=center, inner sep=5pt, font=\small\bfseries, minimum width=2.6cm, minimum height=10mm, diamond, aspect=2}
]
% Main flow
\node[box, fill=llmblue!15] (run) {Run Evals};
\node[box, fill=llmblue!10, below=of run] (check) {All pass?};
\node[box, fill=llmgreen!15, right=35mm of check] (deploy) {Deploy};
\node[box, fill=llmgreen!15, right=25mm of deploy] (monitor) {Monitor};
\node[box, fill=llmblue!10, below=of check] (which) {Which failed?};
\node[box, fill=llmorange!15, left=25mm of which] (safety) {Safety};
\node[box, fill=llmorange!15, below=of which] (quality) {Quality};
\node[box, fill=llmpurple!15, right=25mm of which] (perf) {Performance};

% Outcomes
\node[box, fill=llmorange!25, below=of safety] (block) {Block\\deployment};
\node[box, fill=llmorange!25, below=of quality] (review) {Flag for\\human review};
\node[box, fill=llmpurple!25, below=of perf] (warn) {Warn; allow\\with override};

% Arrows
\draw[arrow] (run) -- (check);
\draw[arrow, color=llmgreen!70!black] (check) -- node[above, font=\footnotesize]{Yes} (deploy);
\draw[arrow] (deploy) -- (monitor);
\draw[arrow, color=llmorange!70!black] (check) -- node[right, font=\footnotesize]{No} (which);
\draw[arrow] (which) -- (safety);
\draw[arrow] (which) -- (quality);
\draw[arrow] (which) -- (perf);
\draw[arrow, color=llmorange!70!black] (safety) -- (block);
\draw[arrow, color=llmorange!70!black] (quality) -- (review);
\draw[arrow, color=llmpurple!70!black] (perf) -- (warn);
\end{tikzpicture}
\end{llmfigbox}
...
% Normal path
\node[lane, fill=llmgreen!15, minimum width=0.95\linewidth, minimum height=10mm] (normal) {Normal Path: Primary LLM → Response};
\node[lane, fill=llmorange!15, below=of normal, minimum width=0.95\linewidth, minimum height=10mm] (failure) {Failure Path: Timeout/Circuit Breaker → Fallback Model → Graceful Degradation};

% Decision points
\node[box, fill=llmorange!15, right=10mm of normal.east, anchor=west, minimum width=2.2cm, minimum height=9mm] (timeout) {Timeout?};
\node[box, fill=llmpurple!15, below=of timeout, minimum width=2.2cm, minimum height=9mm] (circuit) {Circuit\\Open?};
\node[box, fill=llmblue!15, below=of circuit, minimum width=2.2cm, minimum height=9mm] (fallback) {Fallback\\Model};
\node[box, fill=llmgreen!15, below=of fallback, minimum width=2.2cm, minimum height=9mm] (degrade) {Degraded\\Response};

\draw[arrow] (normal.east) -- (timeout);
\draw[arrow, color=llmorange!70!black] (timeout) -- node[right, font=\footnotesize] {yes} (circuit);
\draw[arrow, color=llmorange!70!black] (circuit) -- node[right, font=\footnotesize] {yes} (fallback);
\draw[arrow, color=llmorange!70!black] (fallback) -- (degrade);
\end{tikzpicture}
\end{llmfigbox}
...
\node[box, fill=llmblue!15, minimum width=3.2cm, minimum height=9mm] (logs) {Production logs\\(sampled, labeled)};
\node[box, fill=llmgreen!15, right=of logs, minimum width=3.2cm, minimum height=9mm] (datasets) {Eval datasets\\golden + incidents + slices};
\node[box, fill=llmorange!15, right=of datasets, minimum width=3.2cm, minimum height=9mm] (runner) {Eval runner\\(offline)};
\node[box, fill=llmpurple!15, right=of runner, minimum width=3.2cm, minimum height=9mm] (report) {Report + gates\\CI/CD decision};

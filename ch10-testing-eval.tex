...
\node[anchor=center, font=\scriptsize, text=black!70] at (2.9,9.9) {Week 1};
\node[anchor=center, font=\scriptsize, text=black!70] at (5.3,9.9) {Week 2};
\node[anchor=center, font=\scriptsize, text=black!70] at (7.3,9.9) {Week 3};
...
% X-axis time labels - inside chart area, larger and more visible
\node[anchor=center, font=\scriptsize, text=black!70] at (11.9,9.9) {Week 1};
\node[anchor=center, font=\scriptsize, text=black!70] at (14.3,9.9) {Week 2};
\node[anchor=center, font=\scriptsize, text=black!70] at (16.3,9.9) {Week 3};
...
% X-axis time labels - inside chart area, larger and more visible
\node[anchor=center, font=\scriptsize, text=black!70] at (2.9,6.6) {Week 1};
\node[anchor=center, font=\scriptsize, text=black!70] at (5.3,6.6) {Week 2};
\node[anchor=center, font=\scriptsize, text=black!70] at (7.3,6.6) {Week 3};
...
% X-axis time labels - inside chart area, larger and more visible
\node[anchor=center, font=\scriptsize, text=black!70] at (11.9,6.6) {Week 1};
\node[anchor=center, font=\scriptsize, text=black!70] at (14.3,6.6) {Week 2};
\node[anchor=center, font=\scriptsize, text=black!70] at (16.3,6.6) {Week 3};
...
\centering
\begin{llmfigbox}
\definecolor{evalblue}{RGB}{44,102,146}
\definecolor{passgreen}{RGB}{34,139,96}
\definecolor{safetyred}{RGB}{173,63,60}
\definecolor{qualityorange}{RGB}{201,111,29}
\definecolor{perfpurple}{RGB}{123,88,163}
\definecolor{monitorteal}{RGB}{0,128,128}
\begin{tikzpicture}[font=\small, node distance=14mm,
...
% Main flow
\node[box, fill=evalblue!15] (run) {Run Evals};
\node[box, fill=evalblue!10, below=of run] (check) {All pass?};
\node[box, fill=passgreen!15, right=35mm of check] (deploy) {Deploy};
\node[box, fill=monitorteal!15, right=25mm of deploy] (monitor) {Monitor};
\node[box, fill=evalblue!10, below=of check] (which) {Which failed?};
\node[box, fill=safetyred!15, left=25mm of which] (safety) {Safety};
\node[box, fill=qualityorange!15, below=of which] (quality) {Quality};
\node[box, fill=perfpurple!15, right=25mm of which] (perf) {Performance};

% Outcomes
\node[box, fill=safetyred!25, below=of safety] (block) {Block\\deployment};
\node[box, fill=qualityorange!25, below=of quality] (review) {Flag for\\human review};
\node[box, fill=perfpurple!25, below=of perf] (warn) {Warn; allow\\with override};

% Arrows
\draw[arrow] (run) -- (check);
\draw[arrow, color=passgreen!70!black] (check) -- node[above, font=\footnotesize]{Yes} (deploy);
\draw[arrow] (deploy) -- (monitor);
\draw[arrow, color=safetyred!70!black] (check) -- node[right, font=\footnotesize]{No} (which);
\draw[arrow] (which) -- (safety);
\draw[arrow] (which) -- (quality);
\draw[arrow] (which) -- (perf);
\draw[arrow, color=safetyred!70!black] (safety) -- (block);
\draw[arrow, color=qualityorange!70!black] (quality) -- (review);
\draw[arrow, color=perfpurple!70!black] (perf) -- (warn);
\end{tikzpicture}
\end{llmfigbox}
\caption{CI/CD gate decision flowchart. When evaluation failures are detected, the response is categorized by failure type: safety failures block deployment unconditionally, quality regressions are flagged for human review, and performance warnings allow deployment with an explicit override. This tiered response balances velocity with risk management.}
